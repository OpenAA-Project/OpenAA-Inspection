...
FoundPreE:;
	if(e!=NULL){
		if(e->GetImportanceLevel()<100){
			...
			MsgBox.setText	(LangSolver.GetString(NGListForm_LS,LID_1)/*"Is it really OK?"*/);				//�{����OK�ł����H
			MsgBox.addButton(LangSolver.GetString(NGListForm_LS,LID_2)/*"Yes"*/,QMessageBox::AcceptRole);	//�͂�
			...
			int Ret=MsgBox.exec();
			if(Ret==QMessageBox::RejectRole){	//�L�����Z��
				twNGList->SetIsSignalKeyDown(false);
				return;
			}
		}
