...
static	void EditLibraryFormFunctionDrawEnd(FunctionServerClass *Obj,GUIFormBase *TartgetForm)
{
...
//		return;
	...
	AutoAlignmentLibrary		*pAlignLib=dynamic_cast<AutoAlignmentLibrary *>(AutoAlignmentBasePointer->GetLibraryContainer()->CreateNew());
	if(pAlignLib==NULL){
		return;
	}
	AutoPCBHoleAlignerLibrary	*pPCBHoleAlignLib=dynamic_cast<AutoPCBHoleAlignerLibrary *>(AutoPCBHoleAlignerBasePointer->GetLibraryContainer()->CreateNew());
	if(pPCBHoleAlignLib==NULL){
		return;
	}
	DynamicClassifyLibrary		*pDClassifyLib=NULL;
	if(DynamicClassifyBasePointer!=NULL){
		pDClassifyLib=dynamic_cast<DynamicClassifyLibrary *>(DynamicClassifyBasePointer->GetLibraryContainer()->CreateNew());
		if(pDClassifyLib==NULL){
			return;
		}
	}

	SetAutoAlignmentLibNumb		(CmdAlignEnum.LibIDList.GetNumber());
	SetAutoPCBHoleAlignerLibNumb(CmdPCBHoleAlignEnum.LibIDList.GetNumber());
	if(DynamicClassifyBasePointer!=NULL){
		SetDynamicClassifyLibNumb	(CmdDClassifyEnum.LibIDList.GetNumber());
	}
	...
	for(IntClass *LibIDPoint=CmdAlignEnum.LibIDList.GetFirst();LibIDPoint!=NULL;LibIDPoint=LibIDPoint->GetNext()){
		...
		AutoAlignmentBasePointer->GetLibraryContainer()->GetLibrary(LibID,*pAlignLib);
		//画面表示
	...
	for(IntClass *LibIDPoint=CmdPCBHoleAlignEnum.LibIDList.GetFirst();LibIDPoint!=NULL;LibIDPoint=LibIDPoint->GetNext()){
		...
		//ライブラリの中身をＩＤ番号から取得
		//ライブラリの読込コードが無くても仮想関数で読込できる
		AutoPCBHoleAlignerBasePointer->GetLibraryContainer()->GetLibrary(LibID,*pPCBHoleAlignLib);
		//画面表示
	...
	if(DynamicClassifyBasePointer!=NULL){
		...
		for(IntClass *LibIDPoint=CmdDClassifyEnum.LibIDList.GetFirst();LibIDPoint!=NULL;LibIDPoint=LibIDPoint->GetNext()){
			...
			//ライブラリの中身をＩＤ番号から取得
			//ライブラリの読込コードが無くても仮想関数で読込できる
			DynamicClassifyBasePointer->GetLibraryContainer()->GetLibrary(LibID,*pDClassifyLib);
			//色の初期化
			pDClassifyLib->PickupColor.ClearBase();
			//PADのライブラリの色情報を取得する→DynamicClassifyのライブラリに適用する→DynamicClassifyのGenerateをこの処理の後に行うようにする
			if(CmdAlignEnum.LibIDList.GetNumber()>0 && AlignLib[0]!=NULL){
				for(ColorLogic *c=AlignLib[0]->PickupColor.GetFirst();c!=NULL;c=c->GetNext()){
			...
			//Update
			DynamicClassifyBasePointer->GetLibraryContainer()->Update(*pDClassifyLib);
			//画面表示
	...
	//最後にcreateしたものは明示的に削除する
	delete	pAlignLib;
	delete	pPCBHoleAlignLib;
	delete	pDClassifyLib;
	...
	for(int i=0;i<LibNumb;i++){
		if(tbAddColor[i]->isChecked()==true){
			if(i<AutoAlignmentLibNumb){
				...
				GUIFormBase	*AAlignImage=pLayersBase->FindByName(/**/"Inspection" ,/**/"AutoAlignmentImagePanel" ,/**/"");
...
///		TargetImage->ReleaseHook(this);
	...
	if(ui.tbEffectiveMask->isChecked()==true || ui.tbIneffectiveMask->isChecked()==true){
		...
		GUIFormBase	*AMaskImage=pLayersBase->FindByName(/**/"Inspection" ,/**/"AutoMaskingPIImagePanel" ,/**/"");
...
void EditLibraryForm::SetHoleGerberButton(int LibID,int Index)
{
	for(int i=AutoAlignmentLibNumb;i<LibNumb;i++){
		if(PCBHoleAlignLib[i-AutoAlignmentLibNumb]->GetLibID()==LibID){
			if(Index<4){
...
///					CmdSetAAlignShowState.ModeShowingSheet			=CmdSetAutoAlignmentShowingState::_Area;
					...
					CmdSetAAlignShowState.ShowingLibIDs.Add(AlignLib[i]->GetLibID(),false);
					...
					CmdSetAPCBHoleAlignShowState.ShowingLibIDs.Add(PCBHoleAlignLib[i-AutoAlignmentLibNumb]->GetLibID());
	...
	if(ui.tbShowMaskArea->isChecked()==true){
		...
		Index=LibNumb;
		////他のボタンを押し上げる
		//ui.tbEffectiveMask->setChecked(false);
		//ui.tbIneffectiveMask->setChecked(false);
			...
			if(tbColor[j][i]->isChecked()==true){
				if(j<iCLogic[i]){
					...
					if(Ret==QMessageBox::AcceptRole){
						...
						ColorLogic *CLogic;
						if(i<AutoAlignmentLibNumb){
							CLogic=&(AlignLib[i]->PickupColor);
						}
						else{
							CLogic=&(PCBHoleAlignLib[i-AutoAlignmentLibNumb]->PickupHoleColor);
							P.setColor(QPalette::WindowText,QColor(Qt::white));
						}
				...
				if(Ret==QMessageBox::AcceptRole){
					...
					if(i<AutoAlignmentLibNumb){
						...
						if(BasePointer!=NULL){
							...
							CmdPCBHoleAlignDelIndex.LibID		=PCBHoleAlignLib[i-AutoAlignmentLibNumb]->GetLibID();
...
void EditLibraryForm::sbNoiseShrinkValueChanged(int Value)
{
	for(int i=0;i<LibNumb;i++){
		if(i<AutoAlignmentLibNumb){
			AlignLib[i]->NoiseShrink=sbNoiseShrink[i]->value();
		}
	}
}
...
void EditLibraryForm::sbExpandForPickupValueChanged(int Value)
{
	for(int i=0;i<LibNumb;i++){
		if(i<AutoAlignmentLibNumb){
			AlignLib[i]->ExpandForPickup=sbExpandForPickup[i]->value();
		}
		else{
			PCBHoleAlignLib[i-AutoAlignmentLibNumb]->ExpandForPickup=sbExpandForPickup[i]->value();
		}
	}
}
...
//		return;
	...
	pLayersBase->AddMaxProcessing(0);

	//ライブラリの中身をデータベースに保存
	//ライブラリの上書保存コードが無くても仮想関数で上書保存できる
	for(int i=0;i<LibNumb;i++){
		if(i<AutoAlignmentLibNumb){
			int Numb=AlignLib[i]->PickupColor.GetBaseCount()-1;
			int j=iCLogic[i]-1;
			for(ColorLogic *c=AddCLogicList[i].GetFirst();c!=NULL;c=c->GetNext(),j--){
				...
				AlignLib[i]->PickupColor.Add(*c);
			}
			AutoAlignmentBasePointer->GetLibraryContainer()->Update(*AlignLib[i]);

			//DynamicClassifyライブラリ
			if(i<DynamicClassifyLibNumb){
				//色登録
				Numb=DClassifyLib[i]->PickupColor.GetBaseCount()-1;
				for(int j=0;j<iPickupDeleteColor[i];j++,Numb--){
					ColorSpace	*c=DClassifyLib[i]->PickupColor.GetBase(Numb);
					if(c!=NULL){
						DClassifyLib[i]->PickupColor.RemoveBase(c);
						delete c;
					}
				}
				j=iCLogic[i]-1;
				for(ColorLogic *c=AddCLogicList[i].GetFirst();c!=NULL;c=c->GetNext(),j--){
					DClassifyLib[i]->PickupColor.Add(*c);
				}
				//ShrinkDotとNoiseDotの設定
				DClassifyLib[i]->ShrinkDot	=-(sbExpandForPickup[i]->value());
				DClassifyLib[i]->NoiseDot	=sbExpandForPickup[i]->value();
				//Update
				DynamicClassifyBasePointer->GetLibraryContainer()->Update(*DClassifyLib[i]);
			}
			...
			int Numb=PCBHoleAlignLib[i-AutoAlignmentLibNumb]->PickupHoleColor.GetBaseCount()-1;
			int j=iCLogic[i]-1;
			for(ColorLogic *c=AddCLogicList[i].GetFirst();c!=NULL;c=c->GetNext(),j--){
				//画面表示の設定
				if(j<9){
				...
				PCBHoleAlignLib[i-AutoAlignmentLibNumb]->PickupHoleColor.Add(*c);
			}
			AutoPCBHoleAlignerBasePointer->GetLibraryContainer()->Update(*PCBHoleAlignLib[i-AutoAlignmentLibNumb]);
			iPickupAddColor[i]		=0;
			iPickupDeleteColor[i]	=0;
			AddCLogicList[i].RemoveAll();
	...
	//CmdReqAutoPCBHoleAlignerExecuteInitialAfterEdit
...
void EditLibraryForm::SetAutoAlignmentLibNumb(int Numb)
{
	...
	AlignLib=new AutoAlignmentLibrary*[AutoAlignmentLibNumb];
}

void EditLibraryForm::SetAutoPCBHoleAlignerLibNumb(int Numb)
{
	for(int i=0;i<AutoPCBHoleAlignerLibNumb;i++){
		delete PCBHoleAlignLib[i];
	}
	delete []PCBHoleAlignLib;

	AutoPCBHoleAlignerLibNumb=Numb;

	PCBHoleAlignLib=new AutoPCBHoleAlignerLibrary*[AutoPCBHoleAlignerLibNumb];
}

void EditLibraryForm::SetDynamicClassifyLibNumb(int Numb)
{
	for(int i=0;i<DynamicClassifyLibNumb;i++){
		delete DClassifyLib[i];
	}
	delete []DClassifyLib;

	DynamicClassifyLibNumb=Numb;

	DClassifyLib=new DynamicClassifyLibrary*[DynamicClassifyLibNumb];
}

void EditLibraryForm::ShowLibrary(int i,AutoAlignmentLibrary *Lib,AlgorithmBase	*AlgorithmBasePointer)
{
	AlignLib[i]=dynamic_cast<AutoAlignmentLibrary *>(AlgorithmBasePointer->GetLibraryContainer()->CreateNew());
	*AlignLib[i]=*Lib;
	leLibName[i]		->setText	(AlignLib[i]->GetLibName());
	QPalette P;
	int Numb=0;
	for(ColorSphere *pPickupColor=(ColorSphere *)AlignLib[i]->PickupColor.GetFirstBase();pPickupColor!=NULL && Numb<9;pPickupColor=(ColorSphere *)pPickupColor->GetNext()){
	...
	sbNoiseShrink[i]	->setValue	(AlignLib[i]->NoiseShrink);
	sbExpandForPickup[i]->setValue	(AlignLib[i]->ExpandForPickup);
	iCLogic[i]=Numb;
...
{
	PCBHoleAlignLib[i]=dynamic_cast<AutoPCBHoleAlignerLibrary *>(AlgorithmBasePointer->GetLibraryContainer()->CreateNew());
	*PCBHoleAlignLib[i]=*Lib;
	leLibName[AutoAlignmentLibNumb+i]			->setText	(PCBHoleAlignLib[i]->GetLibName());
	QPalette P;
	P.setColor(QPalette::WindowText,QColor(Qt::white));
	int Numb=0;
	for(ColorSphere *pPickupHoleColor=(ColorSphere *)PCBHoleAlignLib[i]->PickupHoleColor.GetFirstBase();pPickupHoleColor!=NULL && Numb<9;pPickupHoleColor=(ColorSphere *)pPickupHoleColor->GetNext()){
...
//	sbNoiseShrink[AutoAlignmentLibNumb+i]		->setValue	(PCBHoleAlignLib[i]->NoiseShrink);
	...
	sbExpandForPickup[AutoAlignmentLibNumb+i]	->setValue	(PCBHoleAlignLib[i]->ExpandForPickup);
...
{
	DClassifyLib[i]=dynamic_cast<DynamicClassifyLibrary *>(AlgorithmBasePointer->GetLibraryContainer()->CreateNew());
	*DClassifyLib[i]=*Lib;
/*
