...
void	EasyPropertyBCodeForm::ShowGrid(void)
{
	...
	for(BCRList	*c=GridList.GetFirst();c!=NULL;c=c->GetNext(),L++){
        ...
        if(c->CheckType==0){
            ::SetDataToTable(ui->tableWidgetItemList ,3,L,QString(LangSolver.GetString(EasyPropertyBCodeForm_LS,LID_1)/*"�ｽﾇ趣ｿｽ"*/));
        }
        else
        if(c->CheckType==1){
            ::SetDataToTable(ui->tableWidgetItemList ,3,L,QString(LangSolver.GetString(EasyPropertyBCodeForm_LS,LID_2)/*"�ｽi�ｽ�ｽ"*/));
        }
	}
}
...
void EasyPropertyBCodeForm::on_pushButtonQualityLoad_clicked()
{
    ...
    if(Row<0){
        if(GridList.GetCount()!=1){
            return;
        }
        Row=0;
        ::SetCurrentRow(ui->tableWidgetItemList, Row);
    }

    QString	FileName=QFileDialog::getOpenFileName(nullptr,LangSolver.GetString(EasyPropertyBCodeForm_LS,LID_3)/*"Load BCR Grade file"*/,QString()
...
{
    QString	FileName=QFileDialog::getSaveFileName(nullptr,LangSolver.GetString(EasyPropertyBCodeForm_LS,LID_4)/*"Save BCR Grade file"*/,QString()
                                            ,/**/"Grade file(BCRGrade.dat);;Data file(*.dat);;All files(*.*)");
    if(FileName.isEmpty()==false){
        QFile	File(FileName);
...
void EasyPropertyBCodeForm::on_tableWidgetItemList_clicked(const QModelIndex &index)
{
    on_tableWidgetItemList_itemSelectionChanged();
}
...
void	EasyPropertyBCodeForm::StartInitial(void)
{
	ShowGrid();
}
