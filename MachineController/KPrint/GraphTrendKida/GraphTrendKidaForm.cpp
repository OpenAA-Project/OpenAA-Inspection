...
void	GraphTrendKidaForm::StartLot(void)
{
	for(EachMaster *m=GetLayersBase()->GetIntegrationBasePointer()->MasterDatas.GetFirst();m!=NULL;m=m->GetNext()){
		if(m->GetIntegrationSlaveNo()==SlaveNo || SlaveNo==-1){
			SetBladeList(m);
		}
	}
	ShowGraph();
...
void	GraphTrendKidaForm::SlotInspectionDone(int CurrentSlaveNo,int64 InspectionID,bool OK)
{
	if(CurrentSlaveNo==SlaveNo || SlaveNo==-1){
		EachMaster	*M		=GetLayersBase()->GetIntegrationBasePointer()->GetMaster(CurrentSlaveNo);
		if(M!=NULL){
			SetBladeList(M);
		}
		ShowGraph();
