...
#include "ui_CreatePatternEdgeDialog.h"
#include "SelectLibrariesForAnyType.h"
#include "XParamDatabase.h"
#include "XDLLOnly.h"
#include "XRememberer.h"

CreatePatternEdgeDialog::CreatePatternEdgeDialog(LayersBase *Base,QWidget *parent) :
    QDialog(parent)
    ,ServiceForLayers(Base)
    ,ui(new Ui::CreatePatternEdgeDialog)
{
    ui->setupUi(this);
...
CreatePatternEdgeDialog::~CreatePatternEdgeDialog()
{
    delete ui;
}
void CreatePatternEdgeDialog::resizeEvent(QResizeEvent *event)
{
...
void CreatePatternEdgeDialog::on_pushButtonGenerate_clicked()
{
	QIODevice	*Buff=GetLayersBase()->GetLocalParamStocker()->CreateIODevice(objectName());
	Libs->Save(Buff);
	GetLayersBase()->GetLocalParamStocker()->DeleteIODevice(Buff);
...
void CreatePatternEdgeDialog::on_pushButtonCancel_clicked()
{
	done(false);
}


AlgorithmLibraryListContainer	CreatePatternEdgeDialog::GetSelectedList(void)
{
	return Libs->SelectedList;
}

void CreatePatternEdgeDialog::on_spinBoxWidthDot_valueChanged(int arg1)
{
	if(OnChanging==true)
		return;
	OnChanging=true;
...
void CreatePatternEdgeDialog::on_doubleSpinBoxWidthMM_valueChanged(double arg1)
{
	if(OnChanging==true)
		return;
	OnChanging=true;
