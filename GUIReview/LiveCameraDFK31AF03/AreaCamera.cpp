...
bool AreaCamera::init()
{
	...
	if( m_cGrabber.loadDeviceStateFromFile(/**/"AreaCamera_DFK31AF03.xml"/*confFileName.toStdString())*/))
	{
...
bool AreaCamera::StartLive(void)
{
	if( m_cGrabber.isDevValid()==true ){
		if( m_cGrabber.isDevOpen()==false && m_cGrabber.loadDeviceStateFromFile(/**/"AreaCamera_DFK31AF03.xml")==false ){
			return false;
		}
...
{
	if( m_cGrabber.isDevValid()==true && m_cGrabber.isLive()==true ){
		return m_cGrabber.stopLive();
	}
	return true;
}
...
bool AreaCamera::SingleShot(void)
{
	...
	DShowLib::MemBufferCollection::tMemBufferPtr pActiveBuf = getSingleFrame(m_cGrabber, DShowLib::eRGB24);
	
	if( true ){
		//QMessageBox::critical(m_parent, "SnapError", "SnapImage is Fault!");
		return false;
	}else{
		return true;
	}
...
bool AreaCamera::GetCameraImage(void)
{
	...
	DShowLib::tFrameHandlerSinkPtr sink = m_cGrabber.getSinkTypePtr();
	DShowLib::Error e = sink->snapImages(1);

	if(e.isError()==true){
		return false;
	}

	smart_ptr<DShowLib::MemBufferCollection> mem = sink->getMemBufferCollection();
	if(mem==NULL){
		return false;
	}
	DShowLib::MemBufferCollection::tMemBufferPtr buff = mem->getBuffer(0);
	buff->lock();

	SIZE size = buff->getSize();
	...
	if(preImage.size()!=QSize(width, height)){
		preImage = QImage(width, height, QImage::Format_RGB32);
	}

	RGB24Pixel *pixel24 = (RGB24Pixel*)buff->getPtr();
	RGB24Pixel *line;

	QImage orgImage(width, height, QImage::Format::Format_RGB32);
	
	#pragma omp for
	for(int y=0; y<height; y++){// reversed
		int startPos = y*width;
		QRgb *pLine = (QRgb*)orgImage.scanLine(height - 1 - y);
		for(int x=0; x<width; x++){
			RGB24Pixel pix = pixel24[startPos + x];
			pLine[x] = qRgb(pix.r, pix.g, pix.b);
		}
	}
	
	buff->unlock();

	//orgImage.save("CameraImage.png");

	if(preImage==orgImage){
		StopLive();
		Close();
		Open();
		StartLive();
	...
	m_paintWidget->setPixmap(QPixmap::fromImage(preImage).scaled(m_paintWidget->size(), Qt::KeepAspectRatio));
...
bool AreaCamera::GetCameraImage(QImage &image)
{
	...
	//if(m_pSink->snapImages(1, 2000).isError()==true){
	//	//QMessageBox::critical(m_parent, "Snap Error", "Get Image is Error");
	//	return false;
	//}
	...
	//RGB24Pixel *pixel24 = (RGB24Pixel*)mem->getPtr();
	//RGB24Pixel *line;
	...
	//	int startPos = y*width;
	//	QRgb *pLine = (QRgb*)image.scanLine(y);

	//	for(int x=0; x<orgImage.width(); x++){
	//		pLine[x] = qRgb(pixel24[startPos + x].r, pixel24[startPos + x].g, pixel24[startPos + x].b);
	//	}
	//}

	//image = orgImage.scaled(image.size(), Qt::KeepAspectRatio);
...
bool AreaCamera::Open(void)
{
	...
	// Try to load the previously used video capture device.
	if( m_cGrabber.loadDeviceStateFromFile(/**/"AreaCamera_DFK31AF03.xml")==true )
	{
		// Display the device's name in the caption bar of the application.
		//SetWindowText( "ICTest " + CString(m_cGrabber.getDev().c_str()));
		return true;
		...
		if(m_cGrabber.isDevValid()==true){
			return m_cGrabber.saveDeviceStateToFile(/**/"AreaCamera_DFK31AF03.xml");
		}else{
			return false;
		}
	}
