...
bool AreaCameraLib::GetCameraImage()
{
	...
	//if(width/(double)height < m_dWidth/(double)m_dHeight){// �������傫��
...
void fixAspect(int cur_width, int cur_height, int fmt_width, int fmt_height, int &fix_width, int &fix_height)
{
	...
	if(cur_width/(double)cur_height < fmt_width/(double)fmt_height){// �E�C���h�E�̍������傫��(�������̑傫���͕��Ɉˑ�)
		fix_width	=	cur_width;
		fix_height	=	(int)(fmt_height * cur_width / (double)fmt_width);
	}
	else{
		fix_height	=	cur_height;// ���̂܂�
...
bool AreaCameraLib::ShowSettingDialog()
{
	if(m_cGrabber.isDevValid())
	{
		m_cGrabber.showVCDPropertyPage(m_hWnd, LangSolver.GetString(AreaCameraLib_LS,LID_15)/*"�J�����ݒ�"*/);
