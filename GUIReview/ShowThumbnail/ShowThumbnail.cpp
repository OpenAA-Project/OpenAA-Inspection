...
int ShowThumbnail::getMoveDelay(){
	...
	int AnyDataLibWait=-1;// AnyDataInLibrary�ɂ����E�F�C�g�^�C���ݒ��l�̎擾]

	if(getMoveMode()==_moveMode::PerPage){// �y�[�W���[�h
		int priority = -1;// ���ݗD���x
		for(int k=0;k<getThumbnailPageList(getCurrentSide())[getCurrentPage()].count();k++){
			...
			for(int i=0; i<list.count(); i++){
				if(list[i].Tag.TCode!=-1 && list[i].LibCode!=-1 && list[i].Tag.NGPriority){//LC��TC���K���ŗD���x�������ꍇ
...
int ShowThumbnail::getPageDelay(const ThumbnailPage *pageWidget)
{
	if(pageWidget!=NULL){
		int sumDelay = 0;
		int defDelay = getMoveDelay();// �v���p�e�B�ݒ��ɂ����f�t�H���g�̑ҋ@����
		for(int i=0; i<pageWidget->count(); i++){// �e�T���l�C���ł̑ҋ@���Ԃ̑��a������
			if(pageWidget->item(i)!=NULL){
				int addDelay = defDelay;// �T���l�C���̊��{�ҋ@����
				int nowPriority = INT_MAX;// �D���x
				if(pageWidget->item(i)->isChecked()==true){// �`�F�b�N�ς݂Ȃ��ҋ@���ԂȂ�
					addDelay = 0;
				}else{// ���`�F�b�N�̃T���l�C��
					for(int j=0; j<pageWidget->item(i)->getNGNailItem().NGPointList.count(); j++){
						...
						if(NGWait>defDelay && NGWait>addDelay && priority<nowPriority){// �ҋ@���Ԃ��蒷���A�����݂̑ҋ@���Ԃ����傫���A���D���x�����荂���ꍇ
							addDelay = NGWait;
						}
					}
				}
...
ShowThumbnail::~ShowThumbnail()
{
	...
	delete infoLabel();
	delete mainLayout();
	//if(FrontThumbnailList.isEmpty()==false){// delete ThumbnailBook �ɂ����Ă��ł� delete �����Ă����炵��
...
void ShowThumbnail::createThumbnailPageList()
{
	...
	int globalIndex = 0;
	// �y�[�W�̍쐬
	int frontPageMax = getThumbnailList(Review::Front).count() / (getThumbnailCountInPage());// �����ׂ��y�[�W�̐�
	if((getThumbnailList(Review::Front).count() % getThumbnailCountInPage())!=0){
		frontPageMax++;
	}
	for(int page=0; page<frontPageMax; page++){
		getThumbnailPageList(Review::Front).append(ThumbnailPageItem(page));// �y�[�W�쐬
	}
	// �y�[�W���X�g�̍쐬
	for(int i=0; i<getThumbnailList(Review::Front).count(); i++, globalIndex++){
		int page = i / (getRowCount() * getColumnCount());// �y�[�W�ԍ�
		int row =  (i - page * (getRowCount() * getColumnCount())) / getColumnCount(); // �c�ʒu
		int column = (i - page * (getRowCount() * getColumnCount())) % getColumnCount();// ���ʒu
		getThumbnailList(Review::Front)[i]->setPosision(page,row, column);// �ʒu�����ݒ�
		getThumbnailList(Review::Front)[i]->setIndexInLocal(i);
		getThumbnailList(Review::Front)[i]->setIndexInGlobal(globalIndex);
		getThumbnailPageList(Review::Front)[page].append(getThumbnailList(Review::Front)[i]);// �y�[�W���X�g�ɒǉ�
	}
	
	// �y�[�W�̍쐬
	int backPageMax = getThumbnailList(Review::Back).count() / (getRowCount() * getColumnCount());// �����ׂ��y�[�W�̐�
	if((getThumbnailList(Review::Back).count() % getThumbnailCountInPage())!=0){
		backPageMax++;
	}
	for(int page=0; page<backPageMax; page++){
		getThumbnailPageList(Review::Back).append(ThumbnailPageItem(page));// �y�[�W�쐬
	}
	// �y�[�W���X�g�̍쐬
	for(int i=0; i<getThumbnailList(Review::Back).count(); i++){
		int page = i / (getRowCount() * getColumnCount());// �y�[�W�ԍ�
		int row =  (i - page * (getRowCount() * getColumnCount())) / getColumnCount(); // �c�ʒu
		int column = (i - page * (getRowCount() * getColumnCount())) % getColumnCount();// ���ʒu
		getThumbnailList(Review::Back)[i]->setPosision(page,row, column);// �ʒu�����ݒ�
		getThumbnailList(Review::Back)[i]->setIndexInLocal(i);
		getThumbnailList(Review::Back)[i]->setIndexInGlobal(globalIndex);
		globalIndex++;
		getThumbnailPageList(Review::Back)[page].append(getThumbnailList(Review::Back)[i]);// �y�[�W���X�g�ɒǉ�
	}
}
...
void ShowThumbnail::createThumbnailPageListByPiece()
{
	...
	if(FrontPieceList.count()!=0){
		...
		for(int t=0;t<FrontPieceList.count();t++){
			...
			for(int i=0; i<getThumbnailList(Review::Front).count(); i++){
				...
				Area=LList[0];
				if(FrontPieceList[t]==Area && getThumbnailList(Review::Front)[i]->getNGNailItem().phase==0){
					int page = TmpNailCountInPiece[t] / getThumbnailCountInPage();// �y�[�W�ԍ�
					int row =  (TmpNailCountInPiece[t] - page * getThumbnailCountInPage()) / getColumnCount(); // �c�ʒu
					int column = (TmpNailCountInPiece[t] - page * getThumbnailCountInPage()) % getColumnCount();// ���ʒu
					getThumbnailList(Review::Front)[i]->setPosision(GPage+page ,row, column);// �ʒu�����ݒ�
					getThumbnailList(Review::Front)[i]->setIndexInLocal(LocalIndex);
					getThumbnailList(Review::Front)[i]->setIndexInGlobal(globalIndex);
					getThumbnailPageList(Review::Front)[GPage+page].append(getThumbnailList(Review::Front)[i]);// �y�[�W���X�g�ɒǉ�
					TmpNailCountInPiece[t]++;
					globalIndex++;
					LocalIndex++;
				}
	...
	if(BackPieceList.count()!=0){
		...
		//for(int page=0; page<frontPageMax; page++){
		//	getThumbnailPageList(Review::Back).append(ThumbnailPageItem(page));// dummy �y�[�W�쐬
		//}
		for(int page=0; page<backPageMax; page++){
			getThumbnailPageList(Review::Back).append(ThumbnailPageItem(frontPageMax+page));// �y�[�W�쐬
		}
		for(int t=0;t<BackPieceList.count();t++){
			TmpNailCountInPiece[t]=0;
		}
		for(int t=0;t<BackPieceList.count();t++){
			...
			for(int i=0; i<getThumbnailList(Review::Front).count(); i++){
				...
				if(BackPieceList[t]==Area && getThumbnailList(Review::Front)[i]->getNGNailItem().phase==1){
					...
					getThumbnailList(Review::Front)[i]->setIndexInLocal(LocalIndex);
					getThumbnailList(Review::Front)[i]->setIndexInGlobal(globalIndex);
					getThumbnailPageList(Review::Back)[GPage+page].append(getThumbnailList(Review::Front)[i]);// �y�[�W���X�g�ɒǉ�
					TmpNailCountInPiece[t]++;
					globalIndex++;
					LocalIndex++;
					BackInPhase=true;
		...
		for(int t=0;t<BackPieceList.count();t++){
			...
			for(int i=0; i<getThumbnailList(Review::Back).count(); i++){
				...
				if(BackPieceList[t]==Area){
					...
					int row =  (TmpNailCountInPiece[t] - page * getThumbnailCountInPage()) / getColumnCount(); // �c�ʒu
					int column = (TmpNailCountInPiece[t] - page * getThumbnailCountInPage()) % getColumnCount();// ���ʒu
					getThumbnailList(Review::Back)[i]->setPosision(GPage+page,row, column);// �ʒu�����ݒ�
					getThumbnailList(Review::Back)[i]->setIndexInLocal(LocalIndex);
					getThumbnailList(Review::Back)[i]->setIndexInGlobal(globalIndex);
					getThumbnailPageList(Review::Back)[GPage+page].append(getThumbnailList(Review::Back)[i]);// �y�[�W���X�g�ɒǉ�
					TmpNailCountInPiece[t]++;
					globalIndex++;
					LocalIndex++;
				}
...
{
	if(row<0 || getRowCount()<=row || column<0 || getColumnCount()<=column)return false;// �����C���f�b�N�X
	//if((row+1)*(column+1)>getThumbnailList(side).count())return false;// �C���f�b�N�X�I�[�o�[

	ReviewPIBase *RBase = GetReviewAlgorithm();
	if(RBase!=NULL){
		CmdSetCurrentNGNail Send(GetLayersBase());
		Send.refType = Review::Ref_Index;// ���P�[�g�ɂ����w��
		Send.locate = Review::Manual;// �C���f�b�N�X�̒��ڎw��
		Send.Index = getGlobalIndex(page, row, column, side);// �C���f�b�N�X�ݒ�
		RBase->TransmitDirectly(&Send);// ���M

		if(Send.Ret==true){// �ړ�����
			Thumbnail	*Thm=getThumbnail(side,  page,  row,  column);
			if(Thm!=NULL){
				if(getCurrentThumbnail()!=NULL){// �O�̑I���T���l�C���������ꍇ
					getCurrentThumbnail()->setSelected(false);// ���I�����Ԃɖ߂�
				}
				...
				// �`�F�b�N�ςݕt�^ �������T���l�C�����[�h�̂Ƃ��̂�
				if(getMoveMode()==_moveMode::PerThumbnail && getCurrentThumbnail()!=NULL && getCurrentThumbnail()->isChecked()==false){
					CmdSetNGChecked NGCheckCmd(GetLayersBase());
					NGCheckCmd.side = getCurrentSide();
					if(getMoveMode()==_moveMode::PerThumbnail){// �T���l�C���P�ʃ��[�h�Ȃ猻��NGNail����
						NGCheckCmd.begin = getCurrentThumbnail()->getIndexInLocal();// ����NGNail
						NGCheckCmd.length = 1;// ����NG�̂�
						RBase->TransmitDirectly(&NGCheckCmd);
					}else{
						//NGCheckCmd.begin = getCurrentPage() * getThumbnailCountInPage();// ���݂̃y�[�W�ɂ������ŏ���NGNail
						//NGCheckCmd.length = getThumbnailCountInPage();// 1�y�[�W��
					}
				...
				// �����L�[�ɂ����ړ����T���l�C���P�ʂȂ���
				if(getMoveMode()==_moveMode::PerThumbnail){
					getCurrentThumbnail()->setSelected(true);// �I�����Ԃ̕\���`���֕ύX
				}
...
int ShowThumbnail::getGlobalIndex(int page, int row, int column, Review::SideType side) const
{
	...
	if(side==Review::Back){
		index += getThumbnailList(Review::Front).count();// ���̏ꍇ�A�\�̕������킹���C���f�b�N�X�ɂ���
	}
	return index;
}
...
void ShowThumbnail::createThumbnailBook()
{
	...
	for(int page=0; page<getThumbnailPageList(Review::Front).count(); page++){
		...
		// ������
		if(getThumbnailPageList(Review::Front)[page].count() != getThumbnailCountInPage()){// �y�[�W�̃T���l�C���g�ɋ󂫂������Ƃ�
			int fillCount = getThumbnailCountInPage() - getThumbnailPageList(Review::Front)[page].count();// �󂫂̐����v�Z
			int lastLocalIndex = getThumbnailPageList(Review::Front)[page].last()->getIndexInLocal();
			for(int i=0; i<fillCount; i++){
				int nowLocalIndex = lastLocalIndex + i + 1;
				int row    =  (nowLocalIndex % getThumbnailCountInPage()) / getColumnCount(); // �c�ʒu
				int column =  (nowLocalIndex % getThumbnailCountInPage()) % getColumnCount(); // ���ʒu
	...
	for(int page=0; page<getThumbnailPageList(Review::Back).count(); page++){
		...
		// ������
		if(getThumbnailPageList(Review::Back)[page].count() != getThumbnailCountInPage()){// �y�[�W�̃T���l�C���g�ɋ󂫂������Ƃ�
			int fillCount = getThumbnailCountInPage() - getThumbnailPageList(Review::Back)[page].count();// �󂫂̐����v�Z
...
void ShowThumbnail::showPage(Review::SideType side, int page, Review::ListLocate locate, int row, int column)
{
	...
	if(page!=getCurrentPage() || getCurrentSide()!=side){// �\�������y�[�W�E�\���̂ǂ��炩���Ⴄ�ꍇ
	...
	if(page!=getCurrentPage() || getCurrentSide()!=side || getCurrentRow()!=row || getCurrentColumn()!=column){// �T���l�C���̏ꏊ���Ⴄ�ꍇ
		// �T���l�C�����I������
		if(locate==Review::First){// ���ԑO
			...
			if(getLocalIndex(page, row, column, side)==-1){// �����C���f�b�N�X
				return;
			}
...
void ShowThumbnail::showPage(Review::ListLocate locate, int GlobalIndex)
{
	if(locate==Review::First){
		...
		if(GlobalIndex<0 || GlobalIndex>=getThumbnailList(Review::Front).count()+getThumbnailList(Review::Back).count())return;// �C���f�b�N�X�I�[�o�[
...
{
	return isCurrentHistoryAllChecked();// ���͑S�`�F�b�N�̂݊m�F
}
...
bool ShowThumbnail::moveHistory(Review::Direction direction, Review::ListLocate NGLocate, bool *historyExisted)
{
	...
	CmdReqAdjacentCurrentNG NGAdjacentCmd(GetLayersBase());// �J�����gNG�̈ړ����m�F
	...
	CmdMoveCurrentHistoryToNGBoard MoveHistoryCmd(GetLayersBase());// NGBoard�P�ʂňړ�
	MoveHistoryCmd.direction = direction;
	RBase->TransmitDirectly(&MoveHistoryCmd);
	if(MoveHistoryCmd.Ret==false)return false;

	if(MoveHistoryCmd.CurrentHistoryPtr == tmp_current)return true;// NGBoard�̈ړ��悪�Ȃ������̂ňړ����Ă��Ȃ��ꍇ�͈ȍ~�̏��������Ȃ�
