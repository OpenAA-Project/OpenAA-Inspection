...
void ShowHistoryListForReview::relativeCellClicked(int currentRow, int currentColumn, int previousRow, int previousColumn)
{
	...
	if(ui.cbOKVisible->isChecked()==true && NGIndexAtTableRow.contains(currentRow)==false){// NG�̃C���f�b�N�X�Ɋ܂܂��Ȃ��iOK�{�[�h�̏ꍇ�j
		repaint();
		ui.twHistoryList->setCurrentCell(previousRow, previousColumn, QItemSelectionModel::SelectionFlag::ClearAndSelect | QItemSelectionModel::SelectionFlag::Rows);// �O�̃C���f�b�N�X���w��
		update();
		return;
	}
	ReviewPIBase *RBase = GetReviewAlgorithm();
