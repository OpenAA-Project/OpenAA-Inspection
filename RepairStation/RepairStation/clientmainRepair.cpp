...
//QByteArray sClientNo ;		//�T�[�o�[���犄���U���ꂽ�N���C�A���g�ԍ�
ClientMain::ClientMain(const QString &sHost, const QString &sPort)
{
	...
	Port=sPort;
	ConnectFlag=false;
	//bool ret=(connect(socket, SIGNAL(disconnected()), 0, SLOT(Client_Disconnect())))?true:false;//���C�A���g���炢���Ȃ��؂��ꂽ���p
...
bool ClientMain::Client_Connect(QByteArray &sError)
{
	if(ConnectFlag==false){
		...
		socket->connectToHost(Host,Port.toInt(),QIODevice::ReadWrite) ;//�T�[�o�[�ڑ�
		if (!socket->waitForConnected(iTimeout)) {
			sError=QByteArray(socket->errorString().toLocal8Bit().data());//�ڑ��ł�Client_Start�Ȃ����΁A�G���[���e���Ԃ�
			return false;
		}
...
bool ClientMain::Client_Start(const QByteArray &FileName, QByteArray &Msg, int &Client) //"START"���v���𑗂��A�T�[�o�[�ڑ��̊m��
{
...
void ClientMain::Client_Send()
{
	...
	block.clear();
	QDataStream out(&block, QIODevice::WriteOnly);		//��������Text
