...
void MasterUI::view(QString &SELECT,QString &ORDER){
	...
	ui->MASTERView->clear();
	while (MasterModel->canFetchMore()){ // 256�s�ȏ��ǂݍ��ނ��߂ɕK�v
...
//Data�̎擾
	...
	parent->saveRelatedMastercode(file,MASTERCODE); //RELATEDMASTERCODE�̒ǉ��ɔ���
...
//�����̃e�[�u���̃t�B�[���h�擾
	...
	parent->loadRelatedMastercode(file); //RELATEDMASTERCODE�̒ǉ��ɔ���
}
...
void LibUI::Delete(){
	...
	if(LibJudge==0){
		...
		message(table);
		return;
	}else if(LibJudge==1){	//�t�H���_���I�������Ă����Ȃ�
		table=/**/"FolderDelete";
		...
		if(ui->LibTreeWidget->itemAt(0,0)->isSelected()){
			...
			for(int i=0;i<items.count();i++){
				...
				Lib->FolderDelete(FolderID);////InspectLib��LibFolder�폜
				Folder->Delete(FolderID);//���u�t�H���_�[�폜
				Folder->ChildDelete(FolderID);//�q���̍폜
			}
		}
		showTree();
		ui->INSPECTLIBView->clear();
...
void LibUI::Backup(){
	...
	if(FolderID!=/**/"-1"){
		Target=/**/"Parent";
		Folder->SubSave(Target, file, FolderID);//�e�̊K�w���ۑ�
		Target=/**/"Child";
		Folder->SubSave(Target, file, FolderID);//�q���̊K�w���ۑ�
	}
	Folder ->Save(file, FolderID);//�������g�̕ۑ�
	Lib	   ->Save(file, FolderID);//�������g�̒��g
	LibType->Save(file);
}
void LibUI::Restore(){
	...
	if(FolderID!=/**/"-1"){
		Folder->SubLoad(file, BFolder, NFolder, Fins);//�e�̊K�w�̃��X�g�A
		Folder->SubLoad(file, BFolder, NFolder, Fins);//�q���K�w�̃��X�g�A
	}
	Folder->RootLoad(file, BFolder, NFolder, Fins);//�������g�̃��X�g�A
...
void LibUI::loadRelatedMastercode(QIODevice &file){
	...
	for (int count=0; count<FolderCount; count++){
		...
		Lib->FieldGet(Target, NLibType);
		Lib->RestoreField(BLibType, NLibType, Target, Tins);
	//�e�̊K�w�����݂��Ȃ��ꍇRoot�̒����ɂ�
		if(FolderID!=/**/"-1"){
			Folder->RootSubLoad(file, BFolder, NFolder, Fins);//�e�̊K�w�̃��X�g�A
		//	Folder->SubLoad(file, BFolder, NFolder, Fins);//�q���K�w�̃��X�g�A
		}
		Folder->RootLoad2(file, BFolder, NFolder, Fins);//�������g�̃��X�g�A
		Lib->Load(file, BLib, NLib, Lins);
		LibType->Load(file, BLibType, NLibType, Tins);
	}
}
