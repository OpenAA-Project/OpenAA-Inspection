...
bool	XMLTag::Match(XMLTag &MatchPoint,XMLElement *e)
{
	...
	if(MatchPoint.TagName==NULL)
		return false;
	if(strcmp(MatchPoint.TagName->GetString(),"ANY")!=0 && strcmp(TagName->GetString(),MatchPoint.TagName->GetString())!=0)//ANY���͈��v�����^�O�������������ʉ�
		return false;
	for(XMLTagAttr *a=MatchPoint.Attrs.GetFirst();a!=NULL;a=a->GetNext()){//MachiPoint�����f�[�^���n��
		XMLTagAttr *b;
		for(b=Attrs.GetFirst();b!=NULL;b=b->GetNext()){//�������g�̃f�[�^���n��
//			if(a->AttrSign=="="){
				//if(*a == *b){
			if( a->hCompare(*b)){//���ڂ̏����𔻒f����
					if(e->Parent->GetTop()!=NULL){//XMLElement�������Ă���
						if(e->Parent->GetTop()->MatchXMLTag->count()==0)//�f�[�^������
							break;
				...
				if(strcmp(E->StartTag.TagName->GetString(),MatchPoint.at(Cnt)->TagName->GetString())==0){
					for(XMLTagAttr *TA=E->StartTag.Attrs.GetFirst();TA!=NULL;TA=TA->GetNext()){
						...
						if(strcmp(TA->AttrName->GetString(),MatchPoint.at(Cnt)->Attrs.GetFirst()->AttrName->GetString())==0){
							...
							//XMLTagAttr *b;
							//for(b=MatchPoint.at(Cnt)->Attrs.GetFirst();b!=NULL;b=b->GetNext()){//�������g�̃f�[�^���n��
							//	if( b->hCompare(*TA))
									return true;
							//}
						}
