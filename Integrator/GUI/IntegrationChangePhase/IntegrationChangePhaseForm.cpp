...
void IntegrationChangePhaseForm::ShowList(void)
{
	...
	if(f!=NULL){
		QStringList	Header;
		Header.append(LangSolver.GetString(IntegrationChangePhaseForm_LS,LID_1)/*"Phase"*/);
		if(ABase!=NULL){
			ui->tableWidget->setColumnCount(2);
			Header.append(LangSolver.GetString(IntegrationChangePhaseForm_LS,LID_2)/*"Enable"*/);
...
void IntegrationChangePhaseForm::on_pushButtonAddPhase_clicked()
{
	EachMaster	*f=GetLayersBase()->GetIntegrationBasePointer()->FindMaster(MachineCode);
	if(f!=NULL){
		InputPanelDialog	D(-1 ,QString(LangSolver.GetString(IntegrationChangePhaseForm_LS,LID_3)/*"Phase"*/)+QString::number(f->GetPhaseNumb()));
		if(D.exec()==(int)true){
		...
		if(m!=NULL && m->GetPhaseNumb()>1){
			...
			if(RCmd.Send(SlaveNo,0,ACmd)==true){
				if(ACmd.CurrentPhase==0){
					QMessageBox::warning(NULL,LangSolver.GetString(IntegrationChangePhaseForm_LS,LID_4)/*"�x��"
*/											,LangSolver.GetString(IntegrationChangePhaseForm_LS,LID_5)/*"�ŏ���Phase�͍폜�ł��܂���"*/);
				}
				else{
