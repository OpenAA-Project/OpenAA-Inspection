	...
	if(Visible==true){
		...
		if(Parent->ShowInformation==true){
			...
			}
			{
				...
				int row;
				for(row=0;row<CMessageRow;row++){
					Pnt.drawText(Mergin,H+MessageHeight+MessageHeight*row,Messages[row]);
				}
				if(Parent->ShowRemarkData==true){
					char	*fp=Result->RemarkData.data();
					Pnt.drawText(Mergin,H+MessageHeight+MessageHeight*row,QString(fp));
				}
			}
		}
...
void ThumbnailPanel::mouseDoubleClickEvent ( QMouseEvent * event )
{
	if(Visible==true){
		...
		if(Parent->LargeFlatInspectionWindow!=NULL){
			Parent->LargeFlatInspectionWindow->Parent=NULL;
			delete	Parent->LargeFlatInspectionWindow;
			Parent->LargeFlatInspectionWindow=NULL;
		}
		...
		if(RepresentativePoint->LibType==DefLibTypeFlatInspection){
			Parent->LargeFlatInspectionWindow=new LargeFlatInspectionForm(this);
			Parent->LargeFlatInspectionWindow->Initial();
			Parent->LargeFlatInspectionWindow->show();
		}
		else{
    ...
    ui(new Ui::IntegrationStackNGThumbnailForm)
{
	...
	LargeFlatInspectionWindow	=NULL;
	LastPressedTime		=0;
	EnableUpdate		=true;
	ShowRemarkData		=false;
...
void	IntegrationStackNGThumbnailForm::TransmitDirectly(GUIDirectMessage *packet)
{
	...
	if(CmdSetUpdateModeVar!=NULL){
		...
		if(CmdSetUpdateModeVar->StartUpdate==true){
			EachMaster *m=GetLayersBase()->GetIntegrationBasePointer()->GetMaster(SlaveNo);
			if(m==NULL){
				m=GetLayersBase()->GetIntegrationBasePointer()->FindMaster(MachineID);
			}
			if(m!=NULL){
...
void	IntegrationStackNGThumbnailForm::PressPanel(int index)
{
...
void	IntegrationStackNGThumbnailForm::GetInformation(InspectionList *L,NGPoint *Rp
...
QStringList	IntegrationStackNGThumbnailForm::GetInformation(InspectionList *L,NGPoint *Rp)
{
	...
	QString LibName;
	QString CauseStr;
	GetInformation(L,Rp ,StrLibID ,LibName ,CauseStr);
	...
	if(ShowInformationTime==true && ShowInformationNumber==true){
		...
		Ret.append(L->InspectionTime.toString(LangSolver.GetString(IntegrationStackNGThumbnailForm_LS,LID_6)/*"hh:mm:ss"*/)
					+QString(/**/" (")
					+QString::number(L->ID)
					+QString(/**/")"));
	...
	if(ShowInformationTime==true && ShowInformationNumber==false){
		...
		Ret.append(L->InspectionTime.toString(LangSolver.GetString(IntegrationStackNGThumbnailForm_LS,LID_7)/*"hh:mm:ss"*/));
		if(ItemDetailList.count()>0){
			DetailResultInfoList	*L=ItemDetailList.NPListPack<DetailResultInfoList>::GetFirst();
			Ret.append(L->MessageString+QString(/**/":")+L->GetInfoString());
		}
