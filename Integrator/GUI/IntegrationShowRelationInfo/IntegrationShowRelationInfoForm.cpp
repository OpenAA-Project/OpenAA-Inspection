...
void	IntegrationShowRelationInfoForm::Prepare(void)
{
	...
	if(ShowMasterID==true){		
		...
		LabelWidth=max(FontM.boundingRect(TitleMasterID).width(),LabelWidth);
	}
	if(ShowMasterName==true){
		ui->labelHeadMasterName	->setFont(TitleFont);
		LabelWidth=max(FontM.boundingRect(TitleMasterName).width(),LabelWidth);
	}
	if(ShowLotName==true){
		ui->labelHeadLotName	->setFont(TitleFont);
		LabelWidth=max(FontM.boundingRect(TitleLotName).width(),LabelWidth);
	}
	if(ShowWorkerID==true){
		ui->labelHeadWorkerID	->setFont(TitleFont);
		LabelWidth=max(FontM.boundingRect(TitleWorkerID).width(),LabelWidth);
	}
	if(ShowWorkerName==true){
		ui->labelHeadWorkerName	->setFont(TitleFont);
		LabelWidth=max(FontM.boundingRect(TitleWorkerName).width(),LabelWidth);
	}
