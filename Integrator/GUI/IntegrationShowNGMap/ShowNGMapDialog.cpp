	...
	if(m!=NULL){
		...
		for(int y=0;y<ImageH;y++){
			...
			if(0<=Y && Y<m->GetMaxLines(Phase,Page)){
				BYTE	*s[3];
				for(int Layer=0;Layer<m->GetLayerNumb(Phase,Page) && Layer<3;Layer++){
					ImageBuffer	*MBuff=m->GetMasterImage(Phase ,Page ,Layer);
					s[Layer]=MBuff->GetY(Y);
				}
...
void	ShowNGMapDialog::AllocateMap(void)
{
	...
	double	Pixel=GetParamGlobal()->TransformUnitToPixel(0,MM);
	XRes=Pixel;
	YRes=Pixel;
...
void ShowNGMapDialog::on_pushButtonReview_clicked()
{
	...
	if(m!=NULL){
		NGCount=0;
		ui->progressBar->setMaximum(m->GetCurrentInspection().GetCount());
		int	N=0;
		for(InspectionList *L=m->GetCurrentInspection().GetFirst();L!=NULL;L=L->GetNext(),N++){
