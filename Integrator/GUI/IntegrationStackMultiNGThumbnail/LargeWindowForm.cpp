	...
	,ui(new Ui::LargeWindowForm)
{
	...
	if(Parent->RepresentativePoint!=NULL){
		EachMaster *m=GetLayersBase()->GetIntegrationBasePointer()->GetMaster(Parent->GetMaster()->GetIntegrationSlaveNo());
		if(m==NULL){
			m=GetLayersBase()->GetIntegrationBasePointer()->FindMaster(Parent->GetMaster()->GetMachineCode());
		}
		if(m!=NULL){
	...
	QString LibName;
	QString CauseStr;
	Parent->Parent->GetInformation(Parent->GetMaster()->GetIntegrationSlaveNo()
								,Parent->Result,Parent->RepresentativePoint
