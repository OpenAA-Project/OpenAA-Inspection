...
**
** This file is C:\Regulus64v5\Filter\FilterColorShift\FilterColorShift.cpp
** Author : YYYYYYYYYY
****************************************************************************-**/


#include "FilterColorShift.h"
...
{
	return new FilterColorShift(base);
}
...
//	if dialog can't be shown, it returns false
{
	...
	SettingColorShiftDialog	D(((FilterSmoothingClass *)handle)->MethodValue,parent);
	if(D.exec()==(int)true){
...
//================================================================================================
FilterColorShift::FilterColorShift(LayersBase *base)
...
bool	FilterColorShift::Save(QIODevice *f)
{
	if(::Save(f,Length)==false)
		return false;
	return true;
}
bool	FilterColorShift::Load(QIODevice *f)
{
...
bool	FilterColorShift::Initial(void)
{
	if(MethodValue==1){
		AllocatedLayerCount=GetLayerNumb();
...
bool	FilterColorShift::Release(void)
{
	if(TmpImage!=NULL){
		for(int i=0;i<AllocatedLayerCount;i++){
			delete	TmpImage[i];
...
bool	FilterColorShift::Execute(ImageBuffer *Buff[],int BufferDimCounts)
{
	int	ThreadNumb;
	#pragma omp parallel
	{
	...
	if(MethodValue==0){
		...
		for(int i=0;i<GetLayerNumb() && i<BufferDimCounts;i++){
			*TmpImage[i]=*Buff[i];
		}
