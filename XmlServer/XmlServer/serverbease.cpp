...
ServerBase::ServerBase( ) //QWidget *parent, Qt::WindowFlags flags
{
	...
	wxAnswer=NULL;
	xml = new XmlAPI();
	//Server�N�����F�N���C���A���g�����ڑ�100�S�Ă����ɂ���
	iSeverNo = 0;

	for(int i=0; i<1000; ++i){  /* �c�����̓Y�� */
		xmlClient[i]=NULL;
		ServerData[i]=NULL;
		UpAppSize[i]=NULL;	/* �e�v�f�֐��l������ */
		UpAppXML[i]=NULL;
		ThreadDB[i]=NULL;
		ThreadStart[i]=NULL;
		MemRead[i]=NULL;
...
int ServerBase::Server_XnmNum(const char *DBbuff)
{
	...
	for (int i=1; i<1000; i++){
		...
		if(ThreadDB[i]!=NULL){//
			...
			if(stricmp(RedDB, RedThread)==0)//Thread������
#if 1 // 20100302
			{
				delete[] RedThread;
		...
		if(ServerData[i]!=NULL){//'\0'
			...
			if(stricmp(RedDB, RedSD)==0){
				...
				if(RDB.toUpper()==RSD.toUpper()){//���{�ꕶ���񂪓����\���������גʏ���strcmp��Error�ɂȂ�
				*/
				delete []RedSD;
				delete []RedDB;
				return	i;
...
bool ServerBase::Server_Select(QList<QByteArray> &List, int &No, XMLElement **RetAns)
{
	...
	if(strcmp(Target, "Select")==0 || strcmp(Target, "SelectNotNG")==0){
		...
		if( strcmp(Target,"TagToTagSelect")==0 || strcmp(Target,"TagToTagnotNG")==0 ){
			for(int i=4; i<List.count()-1; i++){//0:1:2:�Ō���Parent
				bool	wFinished;
...
{
	int  NGP=QByteArray(buff).count("<NGP");//NG�|�C���g�̐�
	int iP=QByteArray(buff).indexOf("<NGP");//�\���ʒu
	int iI=QByteArray(buff).indexOf("<NGI");//�\���ʒu
...
bool ServerBase::Server_UpApp(QList<QByteArray> &List)
{
	...
	if(strcmp(List.at(4).data(), "END")==0){
		...
		RetAns=xmlClient[CNum]->XMLMain[Num]->MatchFirst(MTag);	//Select�������T��
		xmlClient[CNum]->XMLMain[Num]->MatchXMLTag->clear();
		DeleteTagList();
		...
		if (n.tagName()=="IST"){
			...
			while (!child.isNull()) {
				if (child.tagName()=="INSPECT"){
					RetEID=child.attribute("EID");//5:����ID�ԍ�
					return ;
				}
		...
		if (n.tagName()=="IST"){
			...
			while (!child.isNull()) {
				if (child.tagName()=="LOT"){
					RetLotID=child.attribute("LID");//6:�������b�g�ԍ�
					RetLotName=child.text();//7:�������b�g����
				}
				child=child.nextSiblingElement();
			}
		}
...
void MemDeleteThread::run()
{
	...
	strcpy(ThreadDB[DDBNo], DB);//Thread�������̃f�[�^�ێ�
	delete []DB;
//File�ɏ������������ޏ���
	char IST[7];
	strcpy(IST, "IST");
