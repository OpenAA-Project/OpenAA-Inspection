...
{
	Root=/**/"";
	Name="ResultWriteCSVBarcodeLine";
	return true;
}
...
{
	return "ResultWriteCSVBarcodeLine";
}
DEFFUNCEX	const	char		*DLL_GetParamName(void)
{
	return "ResultWriteCSVBarcodeLine.dat";
}

DEFFUNCEX	ResultDLLBaseRoot	*DLL_Initial(LayersBase *Base)
{
	ResultWriteCSVBarcodeLine	*ResultPointer=new ResultWriteCSVBarcodeLine(Base);
...
{
	bool	ret=((ResultWriteCSVBarcodeLine *)ResultPointer)->OutputCommon(*RBase,ShadowResStocks);
	if(ret==false)
		return false;
	return true;
}
DEFFUNCEX	bool		DLL_OutputResultSlave (ResultDLLBaseRoot *ResultPointer
												...
												,EntryPointBase &entryPoint)
{
	bool	ret=((ResultWriteCSVBarcodeLine *)ResultPointer)->OutputResult(ShadowResStocks
...
{
	bool	ret=((ResultWriteCSVBarcodeLine *)ResultPointer)->RemoveResult(MachineID ,ResultTime ,InspectionID);
	if(ret==false){
		return false;
	}
	return true;
