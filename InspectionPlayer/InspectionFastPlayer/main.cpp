...
int main(int argc, char *argv[])
{
	...
	TestCount=PTest.GetCount();

	int	kt=0;
	for(IntClass *v=PTest.GetFirst();v!=NULL;v=v->GetNext(),kt++){
		TestCount++;
		if(v->GetValue()!=kt){
			TestCount++;
		}
	...
	TestCount=0;
	kt=499;
	for(IntClass *v=PTest.GetLast();v!=NULL;v=v->GetPrev(),kt--){
		TestCount++;
		if(v->GetValue()!=kt){
			TestCount++;
		}
	}
	IntClass *v1=PTest.GetItem(0);
	PTest.RemoveList(v1);
	IntClass *v2=PTest.GetItem(0);
	PTest.RemoveList(v2);
	IntClass *v3=PTest.GetItem(0);
	PTest.RemoveList(v3);
	IntClass *v4=PTest.GetItem(0);
	PTest.RemoveList(v4);
	IntClass *v5=PTest.GetItem(0);
	PTest.RemoveList(v5);
	IntClass *v6=PTest.GetItem(497);
	PTest.RemoveList(v6);
	bool b1=PTest.IsInclude(499);
	bool b2=PTest.IsInclude(496);
	kt=0;
	for(IntClass *v=PTest.GetFirst();v!=NULL;v=v->GetNext(),kt++){
		TestCount=v->GetValue();
	}
	PTest.Add(1000);
	kt=0;
	for(IntClass *v=PTest.GetLast();v!=NULL;v=v->GetPrev(),kt--){
...
*/
	...
	bool	Ret1=G->InitialAdditionalFieldFromGUI(Layers->GetDatabase(),ErrorMsg);
	bool	Ret2=Layers->GetGeneralStocker()->InitialAdditionalFieldFromGUI(Layers->GetDatabase(),ErrorMsg);
	...
	int	CameraCount=-1;
	ExecuteInspectFast	*ExeIns=new ExecuteInspectFast(MainForm,EntryPointToFuncGlobal,SeqControlParamData);
...
//#endif
	...
	if(MasterID>=0){
		...
		if(Layers->SQLLoadMasterData(MasterID,-1,true,CriticalErrorOccured,ErrorMessageOfFalse)==true){
			LoadMasterBeforeFinilizeSpecifiedBroadcaster	BData;
			MainForm->BroadcastSpecifiedDirectly(&BData);
			...
			MainForm->BroadcastSpecifiedDirectly(&SData);
		}
	}
