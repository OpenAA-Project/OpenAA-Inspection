...
{
//////////////////////////////////////////////////////
//	?R???3???E?A?I?A?��?n?a?@?E?a?e?P?x?a?3?d?l?|?e	//
//////////////////////////////////////////////////////
//	
//	???E ax + by + c = z ?A?��?A
//	                       2
//	S = ?��(ax + by + c - z)  ?dabc?��?e???e?A?I?��?a?��?��?e?d =0 ?A?��???A?��?u?o?R?d?d?-?iZ?2?u?u?A?I?�E?a?j
//	        2 2    2 2    2          2
//	  = ?��(a x  + b y  + c  - 2cz + z  + 2abxy + 2bcy - 2byz + 2acx - 2axz)
//	     2   2    2   2    2                 2
//	  = a ?��x  + b ?��y  + c ?��1 - 2c?��z + ?��z  + 2ab?��xy + 2bc?��y - 2b?��yz + 2ac?��x - 2a?��xz
//	
//	?YS        2
//	--- = 2a?��x  + 2b?��xy + 2c?��x - 2?��xz
//	?Ya
//	
//	?YS        2
//	--- = 2b?��y  + 2a?��xy + 2c?��y - 2?��yz
//	?Yb
//	
//	?YS
//	--- = 2nc - 2?��z + 2b?��y + 2a?��x
...
//
//            2     2                                           2                     2
//?@detA = ?��x ?E?��y ?En + ?��xy?E?��y?E?��x + ?��x?E?��xy?E?��y - ?��x ?E?��y?E?��y - ?��x?E?��y ?E?��x - ?��xy?E?��xy?En ?? 0
//
//	             2
//	         |?��x   ?��xy  ?��x|
//	         |         2     |
...
//	MaskBmp=GetMaskBitmap();
	...
	if(((StatisticImagerBase *)GetParentBase())->ShaderEnabled==true){
		if(CurrentMasterCnt==3){
			//?a??(Master)?I????
			if(MaskBmp!=NULL){
				//AutoAlignment???C?u?????c?c?c?��?h?b?g?d?a??
...
//	MaskBmp=GetMaskBitmap();
	...
	if(ExecuteInitialAfterEditFlag==false){
		//?e?s?N?Z???I?A?W?d?A???C?????g?a?3?A?E?u???i?1?d?��?A?A?��?I?s?N?Z???I?P?x?I
		//?u?A?Z?l?v?A?u?Q?a?a?v
		//?u?e?O???e?��?P?x?v?A?u?e?O?A?��?P?x?v
		//?u?n?O?U?E???e?��?P?x?v?A?u?n?O?U?E?A?��?P?x?v?d???s?e
		if(MaskBmp!=NULL){
			#pragma omp parallel
			{
				#pragma omp for
															...
															,ExecuteInitialAfterEditInfo &EInfo)
{
	...
	#pragma omp parallel
	{
		...
		for(int y=0;y<AllocYLen;y++){
			PixelStatisticInLayer	*d=StatisticBuff[y];
			for(int x=0;x<AllocXLen;x++,d++){
				//?e?O???e?��?P?x?A?e?O?A?��?P?x?d?o?-
				d->Added-=(d->Lightest+d->Darkest);
				//?e?O???e?��?P?x?I?Q?a?A?e?O?A?��?P?x?I?Q?a?d?o?-
				d->Added2-=(d->Lightest*d->Lightest+d->Darkest*d->Darkest);
			}
		}
	}
	//?e?O???e?��?I?A?e?O?A?��?I?d?o?��???I?A?A?��?I?a?J?E???g?d?]?Q?�E?e
	...
	if(((StatisticImagerBase *)GetParentBase())->ShaderEnabled==true){
		...
		if(CurrentMasterCnt>1 && CurrentMasterCnt==99999999){
			...
			if(MaskBmp!=NULL){
				...
				//?a??(Inspect)?I???I?P?x?a?3?d?s??	?��?�}?I?????d???A?c???��?��?I?I
				#pragma omp parallel
				{
					#pragma omp for
					for(int y=0;y<AllocYLen;y++){
