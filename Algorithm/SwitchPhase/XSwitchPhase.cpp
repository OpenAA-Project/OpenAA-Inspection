					...
					|| EInfo.CalledInLoadMaster==false){
						for(RootNameList *RList=ABase->CopiedAlgorithmDim.GetFirst();RList!=NULL;RList=RList->GetNext()){
							...
							if(tABase!=NULL && ABase!=AlignedBase){
								...
								if(tABase->GetLogicDLL()->CheckAlgorithmType(AlgorithmBit_TypePieceProcessing)==true
								|| (tABase->GetLogicDLL()->CheckAlgorithmType(AlgorithmBit_TypeAlignment)==true
								 && tABase->GetLogicDLL()->CheckAlgorithmType(AlgorithmBit_TypePieceProcessing)==false)){								DstAp->RemoveAllDatasWithoutManual();
									DstAp->RemoveAllDatas();
								}
								else{
									DstAp->RemoveAllDatasWithoutManual();
								}
...
:AlgorithmBase(Base)
{
	...
	ItemAdoptedRate					=0.2;
	ItemRateFromPeak				=0.2;

	SetParam(&ColorSwitchPhase		, /**/"Color" ,/**/"ColorSwitchPhase"	,LangSolver.GetString(XSwitchPhase_LS,LID_2)/*"Color for SwitchPhase area"*/);
	SetParam(&ColorSelected			, /**/"Color" ,/**/"ColorSelected"		,LangSolver.GetString(XSwitchPhase_LS,LID_3)/*"Color for Selected Piece"*/);
	SetParam(&ColorActive			, /**/"Color" ,/**/"ColorActive"		,LangSolver.GetString(XSwitchPhase_LS,LID_4)/*"Color for Active Piece"*/);
	SetParam(&TransparentLevel		, /**/"Color" ,/**/"TransparentLevel"	,LangSolver.GetString(XSwitchPhase_LS,LID_5)/*"Color for Transparent display level"*/);
	SetParam(&NegColorSwitchPhase	, /**/"Color" ,/**/"NegColorSwitchPhase",LangSolver.GetString(XSwitchPhase_LS,LID_6)/*"Color for Negative SwitchPhase area"*/);
	SetParam(&NegColorSelected		, /**/"Color" ,/**/"NegColorSelected"	,LangSolver.GetString(XSwitchPhase_LS,LID_7)/*"Color for Negative selected SwitchPhase"*/);

	SetParam(&SkipForRoughSearch			, /**/"Setting"	,/**/"SkipForRoughSearch"		,LangSolver.GetString(XSwitchPhase_LS,LID_8)/*"Skip dot for Rough-search"*/);
	SetParam(&SourcePhaseInInitial			, /**/"Setting"	,/**/"SourcePhaseInInitial"		,LangSolver.GetString(XSwitchPhase_LS,LID_9)/*"Source phase in ExecuteInitialAfterEdit, -1:No copy"*/);
	SetParam(&SamePositionAfter2ndPhase		, /**/"Setting"	,/**/"SamePositionAfter2ndPhase",LangSolver.GetString(XSwitchPhase_LS,LID_10)/*"Same mark position after 2nd Phase"*/);
	SetParam(&Only1MarkInPage				, /**/"Setting"	,/**/"Only1MarkInPage"			,LangSolver.GetString(XSwitchPhase_LS,LID_11)/*"Only 1 mark in page"*/);
	SetParam(&ModeCopyAlignment				, /**/"Setting"	,/**/"ModeCopyAlignment"		,LangSolver.GetString(XSwitchPhase_LS,LID_12)/*"Mode copy Alignment"*/);
	SetParam(&ModeCopyAlgorithmInLoadMaster	, /**/"Setting"	,/**/"ModeCopyAlgorithmInLoadMaster"	,LangSolver.GetString(XSwitchPhase_LS,LID_13)/*"Mode copy Algorithm in Loading master"*/);

	SetParam(&AutoItemSize			, /**/"Auto"		,/**/"AutoItemSize"			,LangSolver.GetString(XSwitchPhase_LS,LID_14)/*"Item size for Automatic generation"*/);
	SetParam(&AutoSearchDot			, /**/"Auto"		,/**/"AutoSearchDot"		,LangSolver.GetString(XSwitchPhase_LS,LID_15)/*"SearchDot for Automatic generation"*/);

	SetParam(&CopiedAlgorithmData, /**/"Setting", /**/"CopiedAlgorithm"		
											 , LangSolver.GetString(XSwitchPhase_LS,LID_16)/*"Copied Algorithm "*/		);
	SetParam(&ItemAdoptedRate	, /**/"Setting", /**/"Adopted rate in Item"	,LangSolver.GetString(XSwitchPhase_LS,LID_18)/*""*/);
	SetParam(&ItemRateFromPeak	, /**/"Setting", /**/"Adopted rate from Peak in Item"	,LangSolver.GetString(XSwitchPhase_LS,LID_19)/*""*/);
													...
													,ExecuteInitialAfterEditInfo &EInfo)
{
	...
	if(SamePositionAfter2ndPhase==true){
		LayersBase	*LBase=GetLayersBase();
		AlgorithmInPageInOnePhase	*Ph=GetPageDataPhase(0);
		if(Ph!=NULL){
			for(int page=0;page<GetPageNumb();page++){
				SwitchPhaseInPage	*ApSource=dynamic_cast<SwitchPhaseInPage *>(Ph->GetPageData(page));
...
{
	return LangSolver.GetString(XSwitchPhase_LS,LID_17)/*"Phase蛻�崛蜃ｦ逅�*/;
}
