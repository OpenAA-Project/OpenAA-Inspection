																...
																,ExecuteInitialAfterEditInfo &EInfo)
{
	...
	for(int Y=0; Y<LBlockVNumb; Y++){
		int YLBlockHNumb = Y*LBlockHNumb;	//�񎟌��z�����ꎟ���z���ɂ������Ƃł̏��Z���Ȃ�����
...
{
	brightnessvariance=0;	//�P�x�̕��U
	lightsidengdotcount=0;	//���e�͈͊O�̖��邢���̃h�b�g��
	xn=0;				//�ԍ�
	yn=0;
	X1=0;				//BlockSize�̍������W
	Y1=0;
...
/*
��(v-a)^4
=��v^4 - ��4v^3*a + ��6v^2*a^2 -��4v*a^3 +��a^4
=��v^4 - 4a*��v^3 + 6a^2��v^2 -4a^3��v +N*a^4
=��v^4 - 4a*��v^3 + 6a^2��v^2 -3Na^4
							...
							,RotatedXYBuffer &RotatedXY)
{
	...
	if(isinsidemasking==false){
		...
		//���őI�񂾂��̂́}(SkipTheta/2+SkipTheta%2)�̒������ł��ӂ��킵���p�x��������
...
ExeResult	AreaFilterItemLineEmphasize::ExecutePreProcessing	(int ExeID ,int ThreadNo,ResultInItemRoot *Res)
{
	...
	int		SkipTheta		= GetThresholdR()->SkipTheta;	//�Ԉ����p�x
	int		SkipKy			= GetThresholdR()->SkipKy;
	int		UnifiedLineNumb = GetThresholdR()->UnifiedLineNumb;
	double	UpperBlocksPercentageByVariance= GetThresholdR()->UpperBlocksPercentageByVariance;	//�����������ʁ�

	int	ThetaRangeForSearchBestTheta = (SkipTheta/2) + (SkipTheta%2);

	int	L		= BlockSize;	//����L�͂킩�肸�炢�̂ŏC������
	int HalfL=L>>1;
	int NumOfPixels=L*L;
	int	XLen	=GetDotPerLine();
	int	XByte	=(XLen+7)/8;
	...
	//�u���b�N���̑S�s�N�Z�����}�X�N���ɂ��邩�ǂ����̔��肨����
	//DynamicMaskingPI�ɂ������Ă��Ȃ������̃s�N�Z�������v�Z
...
void	AreaFilterItemLineEmphasize::DrawResultItem(ResultInItemRoot *Res,QImage &IData ,QPainter &PData ,int MovX ,int MovY ,double ZoomRate,bool OnlyNG)
{
	...
	if(OnlyNG==false){
		...
		//���ʍ��W���ϊ�: ���ʍ��W=(���ۂ̉摜�̍��W+Mov[XY])*ZoomRate
