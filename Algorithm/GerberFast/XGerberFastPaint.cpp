...
bool	GerberFastInPage::MakePaintArea(double GlobalX,double GlobalY ,AlgorithmItemPointerListContainer &ItemList
										,AlgorithmItemPointerListContainer &CreatedItems)
{
	...
	a->Simplize();
	GerberFastItemPolygon	*PolygonItem=new GerberFastItemPolygon();
	int	N=a->NPListPack<XYClass>::GetCount();
	PolygonItem->Points		=new GerberFastItemPolygon::PointStruct[N+1];
	...
	for(XYClass *p=a->NPListPack<XYClass>::GetFirst();p!=NULL;p=p->GetNext(),n++){
		...
		PolygonItem->Points[n].Data.PointType	=GerberFastItemPolygon::PointStruct::_Line;
	}
	...
	PolygonItem->OrgPoints=new GerberFastItemPolygon::PointStruct[PolygonItem->OrgPosCount+2];
	for(int i=0;i<PolygonItem->OrgPosCount;i++){
		PolygonItem->OrgPoints[i]=PolygonItem->Points[i];
	}
	PolygonItem->TrPoints=new GerberFastItemPolygon::PointStruct[PolygonItem->PosCount+2];
	PolygonItem->Initial(PolygonItem->Points,PolygonItem->PosCount);

	GerberFastItemBase	*ItemNow=dynamic_cast<GerberFastItemBase *>(ItemList.GetFirst()->GetItem());
	...
	for(XYClassArea *b=a->GetNext();b!=NULL;b=b->GetNext()){
		b->Simplize();
		GerberFastItemPolygon	*PolygonInsideItem=new GerberFastItemPolygon();
		int	N=b->NPListPack<XYClass>::GetCount();
		PolygonInsideItem->Points		=new GerberFastItemPolygon::PointStruct[N+1];
		...
		for(XYClass *p=b->NPListPack<XYClass>::GetFirst();p!=NULL;p=p->GetNext(),n++){
			...
			PolygonInsideItem->Points[n].Data.PointType	=GerberFastItemPolygon::PointStruct::_Line;
		}
		...
		PolygonInsideItem->OrgPoints=new GerberFastItemPolygon::PointStruct[PolygonInsideItem->OrgPosCount+2];
		for(int i=0;i<PolygonInsideItem->OrgPosCount;i++){
			PolygonInsideItem->OrgPoints[i]=PolygonInsideItem->Points[i];
		}
		PolygonInsideItem->TrPoints=new GerberFastItemPolygon::PointStruct[PolygonInsideItem->PosCount+2];
...
static	void	AddItemReEntrant(GerberFastInPage *APage ,AlgorithmItemPointerListContainer &AddedItems
...
void	GerberFastInPage::MakeDraw(int FileLayerID ,AlgorithmItemPointerListContainer &AddedItems)
{
	GerberFileLayer	*gf=FileLayerList.Find(FileLayerID);
	if(gf==NULL)
		return;
	...
	for(AlgorithmItemPointerList *k=AddedItems.GetFirst();k!=NULL;k=k->GetNext()){
		GerberFastItemPolygon	*Item=dynamic_cast<GerberFastItemPolygon *>(k->GetItem());
		if(Item!=NULL){
