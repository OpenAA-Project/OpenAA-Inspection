...
/////////
	...
	//?�E?a?E?a?e?s?N?Z???2?A?I?}?X?N?r?b?g?d?��?��
	#pragma omp parallel
	{
		#pragma omp for
		for(int y=0;y<YLen;y++){
			...
			for(int x=0;x<XLen;x++){
				...
				for(int L=0;L<LayerCount && L<3;L++){
					...
					if(TLine[L][x]<=StageShadeColor){
						//?i?A?3?��?a???A?X?e?[?W?a?E?C?A?A?�E?��?e???a?I???I?P?x?d?I?p?�E?e
	...
	//MinSlitDot?E?a?I?A?O?��?�E?��???X???b?g?d?n???3?1?e
	MatrixBuffCopy(TmpMap	 ,XByte, YLen
                 , (const BYTE **)MaskBitmap,XByte, YLen);

		//?�}?INotAnd?A?X???b?g?A???E?C?A?A?O?��?�E?��??Bitmap?d?��?��
	...
	//?C???I???e?I?d?c?A?�P?A?��?e?d?L?o?G???A?E?�E?e
...
*/
	...
	if(chkEnLargeDot==true){
		...
		//EnLargeDot?a???�P?u?k?3?1??Map?d???e
		MatrixBuffCopy(TmpMap2,XByte,YLen
					 , (const BYTE **)MaskBitmap,XByte, YLen);
