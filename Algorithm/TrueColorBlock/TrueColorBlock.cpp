...
**
** This file is C:\Regulus64v5\Algorithm\TrueColorBlock\TrueColorBlock.cpp
** Author : YYYYYYYYYY
****************************************************************************-**/
...
const	char	*sName=/**/"TrueColorBlock";
...
{
	return TrueColorBlockVersion;
}
DEFFUNCEX	const char	*DLL_GetExplain(void)
{
	return "TrueColorブロック検査";
}
...
{
	return /**/"TrueColorBlockIns";
}

DEFFUNCEX	AlgorithmBase	*DLL_InitialAlloc(LayersBase *Base)
{
	TrueColorBlockBase	*base=new TrueColorBlockBase(Base);
	base->Initial(Base);

	(*Base)=new SlaveCommReqLibraryInMaskOfTrueColorBlock(Base,sRoot,sName);
	(*Base)=new SlaveCommAckLibraryInMaskOfTrueColorBlock(Base,sRoot,sName);
...
{
	return new TrueColorBlockLibraryContainer(Base);
}
...
{
	TrueColorChangeShiftForm	*CForm=new TrueColorChangeShiftForm(InstBase->GetLayersBase());
...
{
	return ((TrueColorBlockItem *)item)->Save(f);
}
DEFFUNCEX	bool	DLL_LoadItem(QIODevice *f,AlgorithmItemRoot *item,LayersBase *LBase)
{
	return ((TrueColorBlockItem *)item)->Load(f,LBase);
}
