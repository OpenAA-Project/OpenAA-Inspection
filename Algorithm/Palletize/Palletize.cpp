...
{
	PalletizeBase	*base=new PalletizeBase(Base);
	base->Initial(Base);
...
{
	return new PalletizeLibraryContainer(Base);
}
...
{
	return ((PalletizeItem *)item)->Save(f);
}
DEFFUNCEX	bool	DLL_LoadItem(QIODevice *f,AlgorithmItemRoot *item,LayersBase *LBase)
{
	return ((PalletizeItem *)item)->Load(f,LBase);
}
