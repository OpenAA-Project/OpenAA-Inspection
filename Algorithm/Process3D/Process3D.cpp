...
**
** This file is C:\Regulus64v5\Algorithm\Process3D\Process3D.cpp
** Author : YYYYYYYYYY
****************************************************************************-**/
...
{
	Process3DBase	*base=new Process3DBase(Base);
	base->Initial(Base);
	return base;
}
DEFFUNCEX	AlgorithmLibraryContainer	*DLL_NewLibrary(LayersBase *Base)
{
	return new Process3DLibraryContainer(Base);
}
...
{
	Process3DHistgramForm	*DForm=new Process3DHistgramForm(InstBase->GetLayersBase());
...
{
	return ((Process3DItem *)item)->Save(f);
}
DEFFUNCEX	bool	DLL_LoadItem(QIODevice *f,AlgorithmItemRoot *item,LayersBase *LBase)
{
	return ((Process3DItem *)item)->Load(f,LBase);
}
