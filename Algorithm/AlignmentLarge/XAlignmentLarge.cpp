	...
	if(GetParamGlobal()->CalcSingleThread==false && N>1){
		...
		do{
			...
			//	#pragma omp for reduction(+:Ret)  
				for(int i=0;i<N;i++){
					XAlignmentLarge	*AItem=(XAlignmentLarge *)GlobalItemPointers[i];
					if(AItem->Priority==0){
...
void	AlignmentLargeInLayer::TransmitDirectly(GUIDirectMessage *packet)
{
	...
	CmdReqMakeAreaFromMask	*CmdReqMakeAreaFromMaskVar=dynamic_cast<CmdReqMakeAreaFromMask *>(packet);
	if(CmdReqMakeAreaFromMaskVar!=NULL){
		UndoElement<AlignmentLargeInLayer>	*UPointer=new UndoElement<AlignmentLargeInLayer>(this,&AlignmentLargeInLayer::UndoAppendManualArea);
		int	CountOfDeletedArea=0;
		for(XAlignmentLargeArea *A=Areas.GetFirst();A!=NULL;A=A->GetNext()){
			if(CmdReqMakeAreaFromMaskVar->AreaPriority.IsInclude(A->Priority)==true){
				CountOfDeletedArea++;
			}
		}
		::Save(UPointer->GetWritePointer(),CountOfDeletedArea);

		AlgorithmLibraryListContainer	ALibC;
		for(XAlignmentLargeArea *A=Areas.GetFirst();A!=NULL;){
			XAlignmentLargeArea *ANext=A->GetNext();
			if(CmdReqMakeAreaFromMaskVar->AreaPriority.IsInclude(A->Priority)==true){
				ALibC.Merge(A->LimitedLib);
				A->Save(UPointer->GetWritePointer());
				RemoveArea(A);
				delete	A;
			}
		...
		if(FPack.GetCount()>0){
			...
			a->AreaName		=/**/"";
			a->LimitedLib	=ALibC;
			a->Priority		=(XAlignmentLargeArea::_EnumPriority)CmdReqMakeAreaFromMaskVar->AreaPriority[0];
	...
	CmdReqMakeAreaFromCAD	*CmdReqMakeAreaFromCADVar=dynamic_cast<CmdReqMakeAreaFromCAD *>(packet);
	if(CmdReqMakeAreaFromCADVar!=NULL){
		UndoElement<AlignmentLargeInLayer>	*UPointer=new UndoElement<AlignmentLargeInLayer>(this,&AlignmentLargeInLayer::UndoAppendManualArea);
		int	CountOfDeletedArea=0;
		for(XAlignmentLargeArea *A=Areas.GetFirst();A!=NULL;){
			XAlignmentLargeArea *ANext=A->GetNext();
			if(CmdReqMakeAreaFromCADVar->AreaPriority.IsInclude(A->Priority)==true){
				CountOfDeletedArea++;
			}
		}
		::Save(UPointer->GetWritePointer(),CountOfDeletedArea);
...
bool	AlignmentLargeInLayer::ReallocXYPixels(int NewDotPerLine ,int NewMaxLines)
{
	CheckAgain:;
	for(XAlignmentLargeArea *A=Areas.GetFirst();A!=NULL;A=A->GetNext()){
		int	x1,y1,x2,y2;
	...
	return AlgorithmInLayerPLI::ReallocXYPixels(NewDotPerLine ,NewMaxLines);
}
...
void	AlignmentLargeInLayer::MakeArea(PureFlexAreaListContainer &FPack
...
:AlgorithmBase(Base)
{
	...
	SetParam(&DefaultLayer				, /**/"Threshold"	,/**/"DefaultLayer"			,LangSolver.GetString(XAlignmentLarge_LS,LID_63)/*"Use layer"*/);
	...
	SetParam(&MaxCountOfMatchingPoint	, /**/"Setting"		,/**/"MaxCountOfMatchingPoint"	,LangSolver.GetString(XAlignmentLarge_LS,LID_64)/*"Max count of MatchingPoint"*/);
	SetParam(&JudgeCharacterModeParam	, /**/"Setting"		,/**/"JudgeCharacterModeParam"	,LangSolver.GetString(XAlignmentLarge_LS,LID_65)/*"Param to judge CharacterMode"*/);
	SetParam(&SearchPointDot			, /**/"Setting"		,/**/"SearchPointDot"		,LangSolver.GetString(XAlignmentLarge_LS,LID_66)/*"Search-dot for Character alignment (0-4)"*/);
	...
	SetParam(&AutoGenAreaSize		, /**/"Generate",/**/"AutoGenAreaSize"			,LangSolver.GetString(XAlignmentLarge_LS,LID_67)/*"Area size for partial generation"*/);
	SetParam(&AutoGenItemSize		, /**/"Generate",/**/"AutoGenItemSize"			,LangSolver.GetString(XAlignmentLarge_LS,LID_68)/*"Item size for partial generation"*/);
	SetParam(&AutoGenSelfSearchArea	, /**/"Generate",/**/"AutoGenSelfSearchArea"	,LangSolver.GetString(XAlignmentLarge_LS,LID_69)/*"SearchArea for self search"*/);
	SetParam(&AutoGenMoveDotX		, /**/"Generate",/**/"AutoGenMoveDotX"			,LangSolver.GetString(XAlignmentLarge_LS,LID_70)/*"MoveDotX"*/);
	SetParam(&AutoGenMoveDotY		, /**/"Generate",/**/"AutoGenMoveDotY"			,LangSolver.GetString(XAlignmentLarge_LS,LID_71)/*"MoveDotY"*/);
	SetParam(&AutoGenMoveDotX2		, /**/"Generate",/**/"AutoGenMoveDotX2"			,LangSolver.GetString(XAlignmentLarge_LS,LID_72)/*"MoveDotX2"*/);
	SetParam(&AutoGenMoveDotY2		, /**/"Generate",/**/"AutoGenMoveDotY2"			,LangSolver.GetString(XAlignmentLarge_LS,LID_73)/*"MoveDotY2"*/);
	SetParam(&AutoGenMinVar			, /**/"Generate",/**/"AutoGenMinVar"			,LangSolver.GetString(XAlignmentLarge_LS,LID_74)/*"Minimum sqrt(Var)"*/	);
	SetParam(&AutoGenNeighborMatch	, /**/"Generate",/**/"AutoGenNeighborMatch"		,LangSolver.GetString(XAlignmentLarge_LS,LID_75)/*"Neighbor Matching rate(0-1.0)"*/	);
	SetParam(&AutoGenMinDistance	, /**/"Generate",/**/"AutoGenMinDistance"		,LangSolver.GetString(XAlignmentLarge_LS,LID_76)/*"Minimum distance between items"*/	);

	SetParam(&NotExpandX	, /**/"Calc"		,/**/"NotExpandX"	,LangSolver.GetString(XAlignmentLarge_LS,LID_77)/*"Never expand X axis in 2 points alignment"*/);
	SetParam(&NotExpandY	, /**/"Calc"		,/**/"NotExpandY"	,LangSolver.GetString(XAlignmentLarge_LS,LID_78)/*"Never expand Y axis in 2 points alignment"*/);
}
...
{
	return LangSolver.GetString(XAlignmentLarge_LS,LID_32)/*"鬯ｮ�｣陷ｴ�ｽ�ｽ�ｴ鬮ｯ�ｷ�･�ｽ�ｲ�ｽ�ｽ�ｽ�ｮ鬯ｮ�ｯ�ｷ�ｽ�ｷ鬮ｯ蜈ｷ�ｽ�ｹ�ｽ�ｻ髯ｷﾂ�隴会ｽｦ�ｽ�ｸ�ｽ�ｺ*/;
}
