...
EditMarkDialog::EditMarkDialog(LayersBase *Base,QWidget *parent) :
    QWidget(parent),ServiceForLayers(Base)
    ,ui(new Ui::EditMarkDialog)
{
    ui->setupUi(this);
...
void	EditMarkDialog::Initial(AlgorithmBase *InstBase ,AlgorithmItemIndependentPack &Data)
{
...
void	EditMarkDialog::GetDataFromWindow(void)
{
	for(AlgorithmItemIndependent *D=IData->Items.GetFirst();D!=NULL;D=D->GetNext()){
		AlgorithmItemRoot	*DA=D->Data;
		if(DA==NULL)
			continue;
		MeasureLineMoveMarkItem	*BData=dynamic_cast<MeasureLineMoveMarkItem *>(DA);
		if(BData==NULL)
			continue;
...
void EditMarkDialog::ShowThreshold(void)
{
	for(AlgorithmItemIndependent	*D=IData->Items.GetFirst();D!=NULL;D=D->GetNext()){
		AlgorithmItemRoot	*DA=D->Data;
		if(DA==NULL)
...
void EditMarkDialog::ShowLibrary(void)
{
	for(AlgorithmItemIndependent	*D=IData->Items.GetFirst();D!=NULL;D=D->GetNext()){
		AlgorithmItemRoot	*DA=D->Data;
		if(DA==NULL)
...
void EditMarkDialog::on_ButtonRelrectOnlyBlock_clicked()
{
...
void EditMarkDialog::on_ButtonReflectAllBlocks_clicked()
{
	GetLayersBase()->ClearAllAckFlag();
...
void EditMarkDialog::on_ButtonSaveToLibrary_clicked()
{
	GetDataFromWindow();
...
void EditMarkDialog::on_ButtonLoadFromLibrary_clicked()
{
	int	ThresholdLevel=GetLayersBase()->SelectThresholdLevel();
	for(AlgorithmItemIndependent *D=IData->Items.GetFirst();D!=NULL;D=D->GetNext()){
		AlgorithmItemRoot	*DA=D->Data;
...
void EditMarkDialog::on_pushButtonChangeLib_clicked()
{
...
void EditMarkDialog::on_pushButtonTest_clicked()
{
	for(AlgorithmItemIndependent	*D=IData->Items.GetFirst();D!=NULL;D=D->GetNext()){
		AlgorithmItemRoot	*DA=D->Data;
		if(DA==NULL)
...
void EditMarkDialog::on_ButtonClose_clicked()
{
	close();
}

void EditMarkDialog::on_EditSearchDot_valueChanged(int arg1)
{
	if(OnChanging==true)
		return;
	OnChanging=true;
	ui->doubleSpinBoxSearchDot	->setValue(TransformPixelToUnit(HistgramPage,arg1));
	OnChanging=false;
}

void EditMarkDialog::on_doubleSpinBoxSearchDot_valueChanged(double arg1)
{
	if(OnChanging==true)
		return;
	OnChanging=true;
	ui->EditSearchDot	->setValue(TransformUnitToPixel(HistgramPage,arg1));
	OnChanging=false;
}

void EditMarkDialog::on_doubleSpinBoxThresholdM_valueChanged(double arg1)
{
	if(OnChanging==true)
		return;
	OnChanging=true;
	ui->doubleSpinBoxThresholdM_2	->setValue(TransformPixelToUnit(HistgramPage,arg1));
	OnChanging=false;
}

void EditMarkDialog::on_doubleSpinBoxThresholdM_2_valueChanged(double arg1)
{
	if(OnChanging==true)
		return;
	OnChanging=true;
	ui->doubleSpinBoxThresholdM		->setValue(TransformUnitToPixel(HistgramPage,arg1));
	OnChanging=false;
}

void EditMarkDialog::on_doubleSpinBoxThresholdP_valueChanged(double arg1)
{
	if(OnChanging==true)
		return;
	OnChanging=true;
	ui->doubleSpinBoxThresholdP_2	->setValue(TransformPixelToUnit(HistgramPage,arg1));
	OnChanging=false;
}

void EditMarkDialog::on_doubleSpinBoxThresholdP_2_valueChanged(double arg1)
{
	if(OnChanging==true)
		return;
	OnChanging=true;
	ui->doubleSpinBoxThresholdP		->setValue(TransformUnitToPixel(HistgramPage,arg1));
	OnChanging=false;
}

void EditMarkDialog::on_pushButtonChangeItemID_clicked()
{
	for(AlgorithmItemIndependent	*D=IData->Items.GetFirst();D!=NULL;D=D->GetNext()){
		AlgorithmItemRoot	*DA=D->Data;
		if(DA==NULL)
