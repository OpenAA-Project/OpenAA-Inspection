...
void	EditArcDialog::Initial(AlgorithmBase *InstBase ,AlgorithmItemIndependentPack &Data)
{
	...
	for(int L=0;L<GetLayersBase()->GetMaxLayerNumb();L++){
...
void EditArcDialog::on_ButtonRelrectOnlyBlock_clicked()
{
	...
	GetLayersBase()->ShowProcessingForm(LangSolver.GetString(EditArcDialog_LS,LID_12)/*"Reflecting ONE item threshold"*/);
...
{
	GetLayersBase()->ClearAllAckFlag();
	GetLayersBase()->ShowProcessingForm(LangSolver.GetString(EditArcDialog_LS,LID_13)/*"Reflecting ALL items\' threshold"*/);
...
void EditArcDialog::on_pushButtonChangeItemID_clicked()
{
	for(AlgorithmItemIndependent	*D=IData->Items.GetFirst();D!=NULL;D=D->GetNext()){
		AlgorithmItemRoot	*DA=D->Data;
		if(DA==NULL)
