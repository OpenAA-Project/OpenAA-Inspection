...
void DentHistgramForm::on_ButtonRelrectOnlyBlock_clicked()
{
	...
	GetLayersBase()->ShowProcessingForm(LangSolver.GetString(DentHistgramForm_LS,LID_0)/*"Reflecting ONE block threshold"*/);
...
{
	GetLayersBase()->ClearAllAckFlag();
	GetLayersBase()->ShowProcessingForm(LangSolver.GetString(DentHistgramForm_LS,LID_1)/*"Reflecting ALL blocks\' threshold"*/);
	...
	for(AlgorithmItemIndependent	*D=IData->Items.GetFirst();D!=NULL;D=D->GetNext()){
		...
		*/
		...
		if(PacketReq.Send(D->GlobalPage,0,PacketSend)==true){
			...
			ui->labelResultNGSizeUnit	->setText(TransformPixelToUnitSquareStr(HistgramPage,R->ResultNGSize		));
...
{
	GetLayersBase()->ClearAllAckFlag();
	GetLayersBase()->ShowProcessingForm(LangSolver.GetString(DentHistgramForm_LS,LID_2)/*"Transmiting ItemName all"*/);
