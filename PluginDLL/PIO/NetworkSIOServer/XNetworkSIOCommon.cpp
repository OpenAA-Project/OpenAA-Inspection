...
SoESocket::SoESocket(QObject *parent ,const QString &_IPAddress,int _PortNumber)
{
	OnRBuff=NULL;
	LastReceivingTime=0;
	NowOnPoint		=0;
...
SoESocket::SoESocket(QObject *parent ,QTcpSocket *sv)
...
bool	SoESocket::IsConnected(void)
{
	if(Sock!=NULL){
...
bool	SoESocket::ConnectToHost(void)
{
	Sock->connectToHost(IPAddress,PortNumber);
	return Sock->waitForConnected(5000);
}

void	SoESocket::WaitReadyRead(int milisec)
{
	if(Sock!=NULL){
		if(Sock->state()==QAbstractSocket::ConnectedState){
...
bool	SoESocket::Send(int32 cmd ,QByteArray &Data)
{
	if(Sock!=NULL){
...
RecList	*SoESocket::PopRecDataTop(void)
{
	MuteRecData.lock();
...
void	SoESocket::SlotReceived(int Cmd)
{
	RecList	*r=PopRecDataTop();
	if(r!=NULL){
		int32	ArrayByte;
...
void	SoESocket::SlotDisconnected()
{
	if(Sock!=NULL){
		//delete	Sock;
		Sock->deleteLater();
