...
class CameraXtiumPx8Repeat;
class SapAcqCallbackInfo;

class featureCtrl;

#define	FixedCamLineCount	16025
#define	MaxGCount			2

class saperaCtrl : virtual public QThread ,virtual public CameraHandle
{
	Q_OBJECT
...
public:
	...
	int		StateInPage[100];	//-1:ñ¢ì«Ç›çûÇ›	1:éBâeäÆóπÅ@2:GetImageäÆóπ
	int		CurrentWPage;
	int		CurrentRPage;
	...
	QMutex	MutexStackedLineCount;
	QMutex	LockAccessImage;
	QMutex	LockGetImage;
	bool	Calibrated;
...
public:
	/* ----------------------------------------------------------------------------
	 PUIBLIC FUNCTIONS
	---------------------------------------------------------------------------- */
	saperaCtrl(int CamNo ,LayersBase *base ,CameraXtiumPx8Repeat *parent);
	~saperaCtrl(void);
	...
	int	getPartialImageInside(int Index ,ImageBuffer *Buff[]
									,int DMAStartLine
									,int SkipLine);
...
private:
	...
	static void CallBackByLine(SapAcqCallbackInfo *pInfo);
	...
	bool ComputeGain(const char* Name, SapAcqDevice* AcqDevice, SapFeature* featureInfo, double coefGain);
	...
	CameraXtiumPx8Repeat	*Parent;

public:
	featureCtrl *fc;
	int		Index;
	bool	FirstCapture;
	DWORD	*ClearCenterLine(int index);
