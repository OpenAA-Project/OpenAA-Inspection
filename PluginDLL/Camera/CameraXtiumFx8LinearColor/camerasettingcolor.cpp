...
CameraSettingColor::CameraSettingColor(CameraXtiumFx8Color *s ,featureCtrl *f ,QWidget *parent)
    : QDialog(parent)
{
	Cam		=s;
	Feature		=f;
	...
	ui.spinBoxLightOffMilisec		->setValue  (Cam->Setting.LightOffMilisec);
	ui.spinBoxLightOnMilisec		->setValue  (Cam->Setting.LightOnMilisec);

	for(int x=0;x<Cam->Setting.CamDepth;x++){
...
void CameraSettingColor::on_pbOK_clicked()
{
	...
	if(Cam->Setting.RedApply==true){
		for(int i=0; i<Cam->Setting.CamDepth; i++)
	...
	Cam->Setting.LightOffMilisec	=ui.spinBoxLightOffMilisec	->value();
	Cam->Setting.LightOnMilisec		=ui.spinBoxLightOnMilisec	->value();
	...
	switch(Color){
		case Red:
			...
			//Straight Line
				...
				for(int x=RedSecondX;x<Cam->Setting.CamDepth;x++)
			...
			//Hight Pass
				...
				for(int x=RedFirstPoint;x<Cam->Setting.CamDepth;x++)
			...
			//UpDown Pass
				...
				for(int x=RedFirstPoint;x<RedSecondPoint;x++)
					ui.bgRedBrightnessGraph->SetYData(x,255);
				for(int x=RedSecondPoint;x<Cam->Setting.CamDepth;x++)
					ui.bgRedBrightnessGraph->SetYData(x,0);
			}
			//DownUp Pass
			...
			//Logarithm
				...
				for(int x=1;x<Cam->Setting.CamDepth;x++)
		...
		case Green:
			...
			if(gIndex==StraightLine){
				...
				for(int x=GreenSecondX;x<Cam->Setting.CamDepth;x++)
			...
			//Hight Pass
				...
				for(int x=GreenFirstPoint;x<Cam->Setting.CamDepth;x++)
			...
			//UpDown Pass
				...
				for(int x=GreenFirstPoint;x<GreenSecondPoint;x++)
					ui.bgGreenBrightnessGraph->SetYData(x,255);
				for(int x=GreenSecondPoint;x<Cam->Setting.CamDepth;x++)
					ui.bgGreenBrightnessGraph->SetYData(x,0);
			}
			//DownUp Pass
			else if(gIndex==DownUpPass){
		...
		case Blue:
			...
			if(gIndex==StraightLine){
				...
				for(int x=BlueSecondX;x<Cam->Setting.CamDepth;x++)
			...
			//Hight Pass
				...
				for(int x=BlueFirstPoint;x<Cam->Setting.CamDepth;x++)
			...
			//UpDown Pass
				...
				for(int x=BlueFirstPoint;x<BlueSecondPoint;x++)
					ui.bgBlueBrightnessGraph->SetYData(x,255);
				for(int x=BlueSecondPoint;x<Cam->Setting.CamDepth;x++)
					ui.bgBlueBrightnessGraph->SetYData(x,0);
			}
			//DownUp Pass
			else if(gIndex==DownUpPass){
