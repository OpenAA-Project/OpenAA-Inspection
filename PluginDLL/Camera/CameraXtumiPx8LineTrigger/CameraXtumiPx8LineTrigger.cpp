...
CameraXtumiPx8LineTrigger::CameraXtumiPx8LineTrigger(int CamNo ,LayersBase *base ,bool original=false)
...
CameraXtumiPx8LineTrigger::~CameraXtumiPx8LineTrigger()
{
	if (_sapera) {
		delete _sapera;
		_sapera = NULL;
...
void	CameraXtumiPx8LineTrigger::InitialXTable(void)
{
	if(MatrixRX!=NULL){
		delete	[]MatrixRX;
	}
...
void	CameraXtumiPx8LineTrigger::AfterPrepare(void)
{
...
void	CameraXtumiPx8LineTrigger::AfterStartSequence(void)
{
...
void	CameraXtumiPx8LineTrigger::Calibrate(void)
{
	if(ioLightControlEnable!=NULL){
...
//				if proocess fails, return 0
{
	...
	CameraXtumiPx8LineTrigger		*Px=new CameraXtumiPx8LineTrigger(CameraNoInThisComputer,base);

	if(CameraParameter==/**/"DupLineMode")
		Px->DupLineMode=true;
	else if(CameraParameter==/**/"Shrink2")
		Px->Shrink2=2;
...
{
	CameraXtumiPx8LineTrigger		*Px=(CameraXtumiPx8LineTrigger *)handle;
	delete	Px;
	return(true);
}
...
//	if process fails, it returns false
{
	...
	if(Px!=NULL){
		...
		if(Ret==true){
			...
			Ret=Px->Setting.StoreToCam(Px->_sapera->fc);	//For Power ON
		}
		Px->InitialXTable();
	}
	return Ret;
