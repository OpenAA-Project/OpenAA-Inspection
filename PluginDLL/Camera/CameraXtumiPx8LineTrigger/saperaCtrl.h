...
class CameraXtumiPx8LineTrigger;
class SapAcqCallbackInfo;

class featureCtrl;

const int	CamStackNumb=8;

class saperaCtrl : public QThread ,public CameraHandle
{
	/* ----------------------------------------------------------------------------
	 CONST
	---------------------------------------------------------------------------- */
...
public:
	ImageBuffer		ImageStack[CamStackNumb][3];
	ImageBuffer		TRImageStack[CamStackNumb][3];
...
public:
	volatile	int		StackedNumb;
	volatile	int		CurrentWTop;
	volatile	int		CurrentRTop;
	int		GCount;
	int		CurrentGPos;
...
public:
	/* ----------------------------------------------------------------------------
	 PUIBLIC FUNCTIONS
	---------------------------------------------------------------------------- */
	saperaCtrl(int CamNo ,LayersBase *base ,CameraXtumiPx8LineTrigger *parent);
	~saperaCtrl(void);
	...
	bool getPartialImageInsideBasic(int Index ,ImageBuffer *Buff[],ImageBuffer *TRBuff[]
									...
									,int width
									,int height);
	bool getPartialImageInsideShrink2(int Index ,ImageBuffer *Buff[],ImageBuffer *TRBuff[]
									,int StartLine , int EndLine);
...
private:
	...
	CameraXtumiPx8LineTrigger	*Parent;
