...
#define	CamDeviceName	"Mono #2"

static	BOOL GetOptionsFromQuestions(char *acqServerName, UINT32 *pAcqDeviceIndex, char *configFileName)
{
...
/* //////////////////////////////////////////////////////////////////////////////////////////////////////////
 function : saperaCtrl(void)
  ...
  subject : constructor
////////////////////////////////////////////////////////////////////////////////////////////////////////// */
saperaCtrlMono::saperaCtrlMono(int CamNo ,LayersBase *base,CameraXtiumFx8Mono *parent)
...
/* //////////////////////////////////////////////////////////////////////////////////////////////////////////
 function : ~saperaCtrl(void)
  ...
  subject : destructor
////////////////////////////////////////////////////////////////////////////////////////////////////////// */
saperaCtrlMono::~saperaCtrlMono(void)
{
	Terminated=true;
	wait(1000);
	delete	fc;
...
bool saperaCtrlMono::link(char *pConfigFilename)
{
	...
	//Parent->Parent->fc->setFeatureValue(featureCtrl::EN_AcquisitionLineRate, (double)DEF_AcquisitionLineRate);
...
/*
	...
	_gainMax = floor(_gainMax);     // Round down to previous integer �W���֐��̂悤�ł��B�؂��̂āB
	_gainMin = ceil(_gainMin);      // Round up to next integer �W���֐��̂悤�ł��B�؂��グ�B
...
bool saperaCtrlMono::ControlCamera(void)
{
	bool	Ret;
	...
	if(HDiv>=2 && BinningHorizontal!=HDiv){
	...
	else if(HDiv==1 && BinningHorizontal!=1){
		bool	StopMode;
		StopMode = true;
		BOOL isAvailable;
	...
	}
	{
		...
		if(VDiv>=2 && BinningVertical!=VDiv){
...
bool	saperaCtrlMono::SetCalibration(void)
{
	bool	Ret=true;
...
bool saperaCtrlMono::ChangeLineCount(int YCount)
{
	Terminated=true;
	wait(1000);
...
void saperaCtrlMono::AllocateBuff(void)
{
	for(int i=0;i<CamMonoStackNumb;i++){
#if defined(HalfMode)
		ImageStack[i].Set(0,0,GetDotPerLine()/2,GetMaxLines()/2);
		if(GetParamGlobal()->AllocateTargetTRBuff==true){
			TRImageStack[i].Set(0,0,GetMaxLines()/2,GetDotPerLine()/2);
		}
#else
		ImageStack[i].Set(0,0,GetDotPerLine(),GetMaxLines());
		if(GetParamGlobal()->AllocateTargetTRBuff==true){
			TRImageStack[i].Set(0,0,GetMaxLines(),GetDotPerLine());
		}
#endif
	}
...
////////////////////////////////////////////////////////////////////////////////////////////////////////// */
bool saperaCtrlMono::startCapture(void)
{
	Halted=false;
	return true;
}

bool saperaCtrlMono::startCaptureInside(void)
{
	if (_bLinked) {
		// start reserve.
...
/* //////////////////////////////////////////////////////////////////////////////////////////////////////////
 function : bool startCapture(void)
  ...
  subject : Start capture
////////////////////////////////////////////////////////////////////////////////////////////////////////// */
bool saperaCtrlMono::stopCapture(void)
{
	Halted=true;
	return true;
	/*
...
/* //////////////////////////////////////////////////////////////////////////////////////////////////////////
 function : bool unlink(void)
  ...
  subject : Sapera ���������s���܂��B
////////////////////////////////////////////////////////////////////////////////////////////////////////// */
bool saperaCtrlMono::unlink(void)
{
	Terminated=true;
...
/* //////////////////////////////////////////////////////////////////////////////////////////////////////////
 function : void acqCallback(SapXferCallbackInfo *pInfo)
   ...
   result : nil
  subject : �J�����f�[�^������Callback�����܂��B
////////////////////////////////////////////////////////////////////////////////////////////////////////// */
void saperaCtrlMono::acqCallback(SapXferCallbackInfo *pInfo)
{
	saperaCtrlMono *pSapera = (saperaCtrlMono *)pInfo->GetContext();

	// Resfresh view
//	pSapera->_View->Show();		// �����̂���???
...
////////////////////////////////////////////////////////////////////////////////////////////////////////// */
void saperaCtrlMono::gioCallback(SapGioCallbackInfo *pInfo)
{
	saperaCtrlMono *pSapera = (saperaCtrlMono *)pInfo->GetContext();;
...
/* //////////////////////////////////////////////////////////////////////////////////////////////////////////
 function : bool updateLUT(SapBuffer* Buffers, SapLut* m_pLut, char *chAcqLutName)
  ...
  subject : LUT update
////////////////////////////////////////////////////////////////////////////////////////////////////////// */
bool saperaCtrlMono::updateLUT(SapBuffer* Buffers, SapLut* m_pLut, char *chAcqLutName, int unLutMode)
{
#if 0
...
/* //////////////////////////////////////////////////////////////////////////////////////////////////////////
 function : bool updateGain(double dBlue, double dGreen, double dRed)
  ...
  subject : Gain update
////////////////////////////////////////////////////////////////////////////////////////////////////////// */
bool saperaCtrlMono::updateGain(double dBlue, double dGreen, double dRed)
{
...
/* //////////////////////////////////////////////////////////////////////////////////////////////////////////
 function : int saperaAutoWhiteBalance(void)
  ...
  subject : Sapera initialyze
////////////////////////////////////////////////////////////////////////////////////////////////////////// */
bool saperaCtrlMono::saperaAutoWhiteBalance(char *pConfigFilename)
{
	char     acqServerName[CORSERVER_MAX_STRLEN];
	...
	UINT32 colorType = 0;
	isAvailable = false;
	if (acqDevice.IsFeatureAvailable("ColorType",&isAvailable) && isAvailable) {	// �g�p�\��?
...
/* //////////////////////////////////////////////////////////////////////////////////////////////////////////
 function : bool :ComputeGain(char* Name, SapAcqDevice* AcqDevice, SapFeature* featureInfo, double coefGain)
     arg1 : char* Name				�Q�C����
     arg2 : SapAcqDevice* AcqDevice	AcqDevice�|�C���^
     arg3 : SapFeature* featureInfo	featureInfo�|�C���^
     arg4 : double coefGain			�Q�C��
   result : true	false
  subject : �w���̒l���]�����A���̌��ʂ��߂��l�ɕԂ��܂��B
////////////////////////////////////////////////////////////////////////////////////////////////////////// */
bool saperaCtrlMono::ComputeGain(char* Name, SapAcqDevice* AcqDevice, SapFeature* featureInfo, double coefGain)
{
	...
	if (gainCameraValue == 0) {
		powValue = pow((float)10, featureExponent);	// �ׂ���
		NewGainValuePrecision = (coefGain * powValue);
	}
	else {
		NewGainValuePrecision = (coefGain * gainCameraValue);
	}
	NewGainRounded = (INT32)(NewGainValuePrecision + 0.5);	// �����ȉ��l�̌ܓ�
...
/* //////////////////////////////////////////////////////////////////////////////////////////////////////////
 function : bool ValidateWhiteBalance(int value, int minValue, int maxValue)
     arg1 : int value		�]�������l
     arg2 : int minValue	�ŏ��l
     arg3 : int maxValue	�ő��l
   result : true	false
  subject : �w���̒l���]�����A���̌��ʂ��߂��l�ɕԂ��܂��B
////////////////////////////////////////////////////////////////////////////////////////////////////////// */
bool saperaCtrlMono::ValidateWhiteBalance(int value, int minValue, int maxValue)
{
...
/* //////////////////////////////////////////////////////////////////////////////////////////////////////////
 function : int prepareCapture(void)
  ...
  subject : �L���v�`���J�n����
////////////////////////////////////////////////////////////////////////////////////////////////////////// */
bool saperaCtrlMono::prepareCapture(void)
{
	if(isRunning()==false){
		Terminated=false;
		start();
...
/* //////////////////////////////////////////////////////////////////////////////////////////////////////////
 function : int getStatus(void)
     arg1 : nil
   result : _nStatus
  subject : ���݂̃X�e�[�^�X���߂��l�ɕԂ��܂��B
////////////////////////////////////////////////////////////////////////////////////////////////////////// */
int saperaCtrlMono::getStatusInside(void)
{
	if (_nStatus == EN_CTRLSTS_GRABED) return 0;
	return 1;
}

int saperaCtrlMono::getStatus(void)
{
	int	Ret;
...
BYTE	*saperaCtrlMono::ClearCenterLine(int index)
{
...
void	saperaCtrlMono::run()
{
	...
	while(Terminated==false){
		...
		GainRedL	=Parent->Setting.GainRedL	;
		OffsetRedL	=Parent->Setting.OffsetRedL	;		 			
		GainRedR	=Parent->Setting.GainRedR	;
		...
		int height	= min(Buff->GetHeight(),YN);	//CONST_MAX_HEIGHT;
		...
		if(Halted==false){
			...
			CurrentWTop++;
			if(CurrentWTop>=CamMonoStackNumb){
				CurrentWTop=0;
			}
			MutexStackImage.lock();
			StackedNumb++;
