...
bool	CLHS_PX8Setting::LoadFromCam(featureCtrl *f)
{
	...
	if(ColorMode==false){
		...
		if(f->getFeatureValue(featureCtrl::EN_BlackLevel, &OffsetBlue)==false)
			return false;
		if(f->getFeatureValue("sensorTDIModeSelection", featureString, sizeof(featureString))==false)
			return false;
		QString	qFeatureString(featureString);
		if(qFeatureString==/**/"TdiOff")
			TDIMode=0;
		if(qFeatureString==/**/"TdiSum")
			TDIMode=1;
		if(qFeatureString==/**/"TdiAvg")
			TDIMode=2;
		if(qFeatureString==/**/"TdiHdr")
			TDIMode=3;
		if(qFeatureString==/**/"MultiLight")
			TDIMode=4;
...
bool	CLHS_PX8Setting::StoreToCam(featureCtrl *f)
{
	...
	if(ColorMode==false){
		...
		if(f->setFeatureValue(featureCtrl::EN_BlackLevel, OffsetRed)==false)
			return false;
		if(TDIMode==0){
			if(f->setFeatureValue("sensorTDIModeSelection", "TdiOff")==false)
				return false;
		}
		if(TDIMode==1){
		...
		if(TDIMode==2){
			if(f->setFeatureValue("sensorTDIModeSelection", "TdiAvg")==false)
				return false;
		}
		if(TDIMode==3){
			if(f->setFeatureValue("sensorTDIModeSelection", "TdiHdr")==false)
				return false;
		}
		if(TDIMode==4){
			if(f->setFeatureValue("sensorTDIModeSelection", "MultiLight")==false)
				return false;
		}
