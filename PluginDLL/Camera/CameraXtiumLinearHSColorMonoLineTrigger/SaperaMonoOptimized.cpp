...
bool saperaCtrlMono::getImage(ImageBuffer *Buff,ImageBuffer *TRBuff)
{
	MutexSwapImage.lock();
	...
	CurrentRTop++;
	if(CurrentRTop>=CamMonoStackNumb){
		CurrentRTop=0;
	}
...
bool saperaCtrlMono::getImageInside(ImageBuffer *Buff,ImageBuffer *TRBuff
							,void	*SrcAddress
							,int	BuffWidth
							,int width
							,int height)
...
bool saperaCtrlMono::getImageInsideBasic(ImageBuffer *Buff,ImageBuffer *TRBuff
									,void	*SrcAddress
									,int	BuffWidth
									,int width
									,int height)
...
bool saperaCtrlMono::RemoveLineNoise(int x1, int XLen ,ImageBuffer *Buff
...
bool saperaCtrlMono::RemoveVerticalStripe(ImageBuffer *Buff)
{
	int	StartX	=1024;
	int	EndX	=7168;
	int	YBand	=30;
...
bool saperaCtrlMono::getPartialImageInside(int Index ,ImageBuffer *Buff,ImageBuffer *TRBuff
									,int StartLine , int EndLine
									,void	*SrcAddress
									,int	BuffWidth
									,int width
			...
			#pragma omp parallel
			{
				...
				for (int i=0; i< YN; i++) {
					...
					if(LeftRight==false){
						for (int j=0; j< width12; j++) {
							//*pR++ = GainOffsetTableRL[*s];		// red
							*pR++ = Clipping((int)(*s*GainRedL+OffsetRedL)	,0,255);
							s++;				// mono
						}

						for (int j=0; j< width22; j++) {
							//*pR++ = GainOffsetTableRR[*s];		// red
							*pR++ = Clipping((int)(*s*GainRedR+OffsetRedR)	,0,255);
							s++;				// mono
						}
					}
					else{
...
bool saperaCtrlMono::getPartialImageInsideBasic(int Index ,ImageBuffer *Buff,ImageBuffer *TRBuff
									,int StartLine , int EndLine
									,void	*SrcAddress
									,int	BuffWidth
									,int width
...
void saperaCtrlMono::RemoveCenterBlack(ImageBuffer *Buff)
{
	...
	#pragma omp parallel
	{
		#pragma omp for
		for(int y=0;y<YN;y++){
			BYTE	*tR=Buff->GetYWithoutDepended(y);
			if(tR[BuffWidth2-1]!=0 && tR[BuffWidth2]==0 && tR[BuffWidth2+1]!=0){
				tR[BuffWidth2]=(tR[BuffWidth2-1]+tR[BuffWidth2+1])>>1;
			}
		}
	}
}