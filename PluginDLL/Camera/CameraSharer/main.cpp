...
int main(int argc, char *argv[])
{
	...
	int		CountOfCamera=1;
	int		CountOfShare=2;
	QString	CameraSettingFile=/**/"CameraSharerSetting.dat";
	...
	Layers->GetLayersBase()->SetLanguageCode();
	Layers->GetLayersBase()->InitialAlgorithmLibrary();
	Layers->GetLayersBase()->SetMainForm(G->GetGUIInstanceRoot()->GetFirstForm());
	Layers->GetLayersBase()->ReadAllSettingFiles(true);


	CameraSharer w(CountOfCamera,CountOfShare
					,CameraDLLFileName
					,CameraSettingFile
					,Layers);
    w.show();
