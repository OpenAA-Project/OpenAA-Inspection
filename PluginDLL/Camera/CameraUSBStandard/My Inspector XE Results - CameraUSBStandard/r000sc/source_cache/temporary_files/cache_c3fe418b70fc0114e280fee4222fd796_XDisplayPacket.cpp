...
bool	GUICmdSendBmp::Load(QIODevice *f)
{
	...
	if(IData==NULL){
		IData =new QImage(Data.W,Data.H,QImage::Format_ARGB32_Premultiplied);
		NGData=new QImage(Data.W,Data.H,QImage::Format_ARGB32_Premultiplied);
...
bool	GUICmdSendBmp::Save(QIODevice *f)
{
	...
	for(int h=0;h<Data.H;h++){
		char	*s=(char *)IData->scanLine(h);
		if(f->write(s,Data.W*4)!=Data.W*4){
			return false;
		}
	}
	for(int h=0;h<Data.H;h++){
...
void	GUICmdSendBmp::MakeImageNGCircle(QImage	*tNGData ,QPainter *tPntFromNGData
...
{
	RetNGData=false;
	if(RedCircleMode==true && dtype==DisplayImage::__Target && ShowNGMarkInTarget==true){
		if(ZoomRate>0.3 || GetParamGlobal()->ShowToBindNGForReviewArea==false){
			for(ResultBaseForAlgorithmRoot *r=LocalLBase.GetCurrentResultForDraw().GetResultBaseDimFirst();r!=NULL;r=r->GetNext()){
				IntList PhaseCodes;
				...
				for(IntClass *v=PhaseCodes.GetFirst();v!=NULL;v=v->GetNext()){
					...
					L->GetPageDataPhase(phase)->GetPageData(localPage)->DrawResult(r->GetPageDataPhase(phase)->GetPageData(localPage)
																						,*tNGData ,*tPntFromNGData,MovX-Gx1/ZoomRate ,MovY-Gy1/ZoomRate ,ZoomRate);
					RetNGData=true;
				}
			}
		}
		else{
			NGImageContainerInPhase	*Ph=LocalLBase.GetCurrentResultForDraw().GetNGImageInPhase(LocalLBase.GetCurrentPhase());
			if(Ph!=NULL){
				QPen	dpen(QColor(GetParamGlobal()->NGCircleColor),(GetParamGlobal()->NGMarkRadius+1)/2);
...
void	GUICmdReqPixelColor::Receive(int32 localPage, int32 cmd ,QString &EmitterRoot,QString &EmitterName)
{
	...
	if(GetLayersBase()->GetPageData(localPage)!=NULL){
		DataInPage	*P=GetLayersBase()->GetPageData(localPage);
		if(0<=LocalX && LocalX<GetLayersBase()->GetParamGlobal()->DotPerLine
			&& 0<=LocalY && LocalY<GetLayersBase()->GetParamGlobal()->MaxLines){
			if(P->GetLayerNumb()==1){
				ImageBuffer	*SBuff;
				if((DType&DisplayImage::__Master)!=0){
					SBuff=&P->GetLayerData(0)->GetMasterBuff();
				}
...
void	GUICmdDrawDot::Receive(int32 localPage, int32 cmd ,QString &EmitterRoot,QString &EmitterName)
{
	...
	for(int layer=min(P->GetLayerNumb(),3)-1;layer>=0;layer--){
		DataInLayer	*L=P->GetLayerData(layer);
		if((DType&DisplayImage::__Master)!=0){
			L->GetMasterBuff().GetY(LocalY)[LocalX]=m&0xFF;
								 ...
								 ,QString &EmitterRoot,QString &EmitterName)
{
	...
	if(DMode==DisplayImage::_DrawUncoveredArea){
		int XByte ,YLen;
		const BYTE	**BMap=LocalLBase.GetPageData(localPage)->GetUncoveredMap(XByte ,YLen);
		if(BMap==NULL){
			LocalLBase.GetPageData(localPage)->MakeUncoveredMap();
		}
		if(MovX>0){
			MovX=0;
		}
...
void	DisplayImageCopyStructure::Capture(FlexArea &area ,DataInPage *PageData ,DisplayImage::DisplayType &dtype)
{
	...
	for(int layer=0;layer<PageData->GetLayerNumb();layer++){
		...
		if((dtype&DisplayImage::__Master)!=0){
			v->Set(area,PageData->GetLayerData(layer)->GetMasterBuff());
		}
		else if((dtype&DisplayImage::__Target)!=0){
...
DisplayImageCopyStructure	&DisplayImageCopyStructure::operator=(DisplayImageCopyStructure &src)
{
	QBuffer	Buff;
...
bool	GUICmdReqPasteRectPacket::Load(QIODevice *f)
{
	...
	for(int i=0;i<N;i++){
		DisplayImageCopyStructure *a=new DisplayImageCopyStructure();
		if(a->Load(f)==false){
			return false;
		}
...
void	GUICmdReqPasteRectPacket::Receive(int32 localPage, int32 cmd ,QString &EmitterRoot,QString &EmitterName)
{
	...
	for(a=ImageClipData.GetFirst();a!=NULL;a=a->GetNext()){
		...
		x2=x2+c->x+dx-q->x;
		y2=y2+c->y+dy-q->y;
		if(x2<0 || x1>=GetDotPerLine() || y2<0 || y1>=GetMaxLines()){
			continue;
		}
		int	Layer=0;
		...
		for(FlexAreaImageList *L=a->ImageWithAreas.GetFirst();L!=NULL;L=L->GetNext(),Layer++){
			...
			FlexAreaImage	UndoAreaImage;
			FlexArea	aArea=*L;
			aArea.MoveToClip(Dx,Dy,0,0,GetDotPerLine(),GetMaxLines());
...
void	GUICmdReqClippedImage::Receive(int32 localPage, int32 cmd ,QString &EmitterRoot,QString &EmitterName)
{
	...
	SendBack->CreateImageData(GetLayersBase(),RealArea,ZoomRate,Source);
	SendBack->LayerList=LayerList;
...
void	GUICmdOperationMove::Receive(int32 localPage, int32 cmd ,QString &EmitterRoot,QString &EmitterName )
{
	...
	AlgorithmBase	*Ab=GetLayersBase()->GetAlgorithmBase(AlgoRoot,AlgoName);
	if(Ab!=NULL){
		Ab->ExecuteMove(localPage,GlobalDx,GlobalDy,Items);
	}
	SendAck(localPage);
}
...
void	GUICmdMoveImage::Receive(int32 localPage, int32 cmd ,QString &EmitterRoot,QString &EmitterName)
{
	...
	for(int i=0;i<GetLayerNumb();i++){
		...
		if(FlagMaster==true){
			if(XDir<0){
				FlexArea	aArea;
				aArea.SetRectangle(0,0,-XDir,GetMaxLines());
				FlexAreaImage	UndoAreaImage;
				...
				FlexArea	aArea;
				aArea.SetRectangle(GetDotPerLine()-XDir,0,GetDotPerLine(),GetMaxLines());
				FlexAreaImage	UndoAreaImage;
				UndoAreaImage.Set(aArea,L->GetMasterBuff());
				UndoAreaImage.Save(UndoPointer->GetWritePointer());
			}
			if(YDir<0){
				FlexArea	aArea;
				aArea.SetRectangle(0,0,GetDotPerLine(),-YDir);
				FlexAreaImage	UndoAreaImage;
				UndoAreaImage.Set(aArea,L->GetMasterBuff());
				UndoAreaImage.Save(UndoPointer->GetWritePointer());
			}
				...
				FlexArea	aArea;
				aArea.SetRectangle(0,GetMaxLines()-YDir,GetDotPerLine(),GetMaxLines());
				FlexAreaImage	UndoAreaImage;
				UndoAreaImage.Set(aArea,L->GetMasterBuff());
				UndoAreaImage.Save(UndoPointer->GetWritePointer());
			}
...
void	GUICmdRotateImage::Receive(int32 localPage, int32 cmd ,QString &EmitterRoot,QString &EmitterName)
{
	...
	for(int i=0;i<GetLayerNumb();i++){
		...
		if(FlagMaster==true){
			...
			L->GetMasterBuff().RotateImage(GetDotPerLine()/2.0,GetMaxLines()/2.0,Angle*2.0*M_PI/360.0);
			L->GetMasterBuff().SetChanged(true);
		}
		if(FlagTarget==true){
			L->GetTargetBuff().Save(UndoPointer->GetWritePointer());
			L->GetTargetBuff().RotateImage(GetDotPerLine()/2.0,GetMaxLines()/2.0,Angle*2.0*M_PI/360.0);
			L->GetTargetBuff().SetChanged(true);
		}
		GetLayersBase()->GetUndoStocker().SetElementToNewTopic(UndoPointer);
	}
...
OriginNames	&OriginNames::operator=(OriginNames &src)
{
	DLLRoot=src.DLLRoot;
	DLLName=src.DLLName;
	Numb   =src.Numb;
...
void	GUICmdSelectByOrigin::Receive(int32 localPage, int32 cmd ,QString &EmitterRoot,QString &EmitterName)
{
	...
	if(Ab!=NULL && Sb!=NULL){
		AlgorithmInPageRoot	*Ap=Ab->GetPageData(localPage);
		if(Ap!=NULL){
			Ap->SelectOriginRootNames(Sb ,LayerList);
		}
	}
	SendAck(localPage);
}
...
void	GUICmdReqDrawItemListsForPaste::Receive(int32 localPage, int32 cmd ,QString &EmitterRoot,QString &EmitterName)
{
	...
	if(Ab!=NULL){
		...
		SendBack->Image=new QImage(SendBack->Width,SendBack->Height,QImage::Format_ARGB32_Premultiplied);
...
void	GUICmdRegulateBrightness::Receive(int32 localPage, int32 cmd ,QString &EmitterRoot,QString &EmitterName)
{
	for(AvrVarListByLayer *c=AvrDatas.GetFirst();c!=NULL;c=c->GetNext()){
		...
		if((ImageType&DisplayImage::__Master)!=0){
			Area.RegulateBrightness(L->GetMasterBuff(),c->Average,c->Dispersion,BlockSize
...
void	GUICmdGetAvrVar::Receive(int32 localPage, int32 cmd ,QString &EmitterRoot,QString &EmitterName)
{
	...
	for(IntClass *c=LayerList.GetFirst();c!=NULL;c=c->GetNext()){
		...
		DataInLayer	*L=GetLayersBase()->GetPageData(localPage)->GetLayerData(c->GetValue());
		double Avr ,Var;
		a->Average=0;
		a->Dispersion=0;
		if((ImageType&DisplayImage::__Master)!=0){
			Area.CalcAvrVar(0,0,L->GetMasterBuff(),a->Average ,a->Dispersion
							,GetParamGlobal()->AdoptRateLForAvr,GetParamGlobal()->AdoptRateHForAvr);
			Area.MakeBrightList(h->BrightList ,GetDotPerLine(),GetMaxLines() ,L->GetMasterBuff());
		}
		else if((ImageType&DisplayImage::__Target)!=0){
			Area.CalcAvrVar(0,0,L->GetTargetBuff(),a->Average ,a->Dispersion
							,GetParamGlobal()->AdoptRateLForAvr,GetParamGlobal()->AdoptRateHForAvr);
			Area.MakeBrightList(h->BrightList ,GetDotPerLine(),GetMaxLines(),L->GetTargetBuff());
		}
		else if((ImageType&DisplayImage::__TargetTR)!=0){
			Area.CalcAvrVar(0,0,L->GetTargetTRBuff(),a->Average ,a->Dispersion
							,GetParamGlobal()->AdoptRateLForAvr,GetParamGlobal()->AdoptRateHForAvr);
			Area.MakeBrightList(h->BrightList ,GetDotPerLine(),GetMaxLines(),L->GetTargetTRBuff());
		}
		else if((ImageType&DisplayImage::__DelayedView)!=0){
			Area.CalcAvrVar(0,0,L->GetDelayedViewBuff(),a->Average ,a->Dispersion
							,GetParamGlobal()->AdoptRateLForAvr,GetParamGlobal()->AdoptRateHForAvr);
			Area.MakeBrightList(h->BrightList ,GetDotPerLine(),GetMaxLines(),L->GetDelayedViewBuff());
		}
		else if((ImageType&DisplayImage::__RawTarget)!=0){
			Area.CalcAvrVar(0,0,L->GetRawTargetBuff(),a->Average ,a->Dispersion
							,GetParamGlobal()->AdoptRateLForAvr,GetParamGlobal()->AdoptRateHForAvr);
			Area.MakeBrightList(h->BrightList ,GetDotPerLine(),GetMaxLines(),L->GetRawTargetBuff());
		}
		else if((ImageType&DisplayImage::__CamTarget)!=0){
			Area.CalcAvrVar(0,0,L->GetCamTargetBuff(),a->Average ,a->Dispersion
							,GetParamGlobal()->AdoptRateLForAvr,GetParamGlobal()->AdoptRateHForAvr);
			Area.MakeBrightList(h->BrightList ,GetDotPerLine(),GetMaxLines(),L->GetCamTargetBuff());
		}
...
{
	ImageBuffer	TmpBuff(0,GetDotPerLine(),GetMaxLines());
	for(int Ly=0;Ly<GetLayerNumb();Ly++){
			...
			if(p->GlobalPage==GPage){
				...
				if(p->Layers.GetFirst()==NULL){
					for(ClipboardAlgorithmItem *item=C->Items.GetFirst();item!=NULL;item=item->GetNext()){
						...
						Area.MoveToClip(p->XPos-x, p->YPos-y,0,0,GetLayersBase()->GetDotPerLine(),GetLayersBase()->GetMaxLines());
