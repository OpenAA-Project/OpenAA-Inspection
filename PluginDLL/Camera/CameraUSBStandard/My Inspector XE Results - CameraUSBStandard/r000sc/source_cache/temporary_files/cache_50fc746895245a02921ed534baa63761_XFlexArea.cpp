...
void	FlexAreaBasePure::SetRefereneFrom(FlexAreaBasePure *src)
{
	if(OwningFLines==true && FLines!=NULL){
		delete	[]FLines;
	}
	FLines		=src->FLines;
	Len			=src->Len;
...
int     FlexAreaBasePure::GetHeight(void)
{
	if(Len!=0){
...
int     FlexAreaBasePure::GetMinY(void)
{
	if(Len!=0){
...
int     FlexAreaBasePure::GetMaxY(void)
{
	if(Len!=0){
...
void	  FlexAreaBasePure::PointerCopy(FlexAreaBasePure *src)
{
	if(OwningFLines==true && FLines!=NULL){
		delete	[]FLines;
	}
...
struct  BlockShape  &FlexAreaBasePure::GetShape(void)
{    
	if(AShape!=NULL)
		return(*AShape);
	else
...
int		FlexAreaBasePure::FindIndex(int AbsY ,int &IndexStart ,int &IndexEnd)
{
	...
	struct FlexLine *k=(struct FlexLine *)bsearch(&m,GetFLinePoint(),GetFLineLen(),sizeof(struct FlexLine),YOnlySortFunc);
	if(k==NULL){
		return(0);
	}
...
struct BlockShape	&BlockShape::operator=(struct BlockShape &src)
{
	if(Kind==_sPoly){
...
//
/*----------------------------------------------------------------------------*/
bool	BlockShape::operator==(struct BlockShape &src)
{
	int	i;
	if(Kind ==src.Kind){
		switch(Kind){
...
//
/*----------------------------------------------------------------------------*/
bool	  FlexArea::IsOvered(int DotPerLine, int MaxLines)
{
...
//
/*----------------------------------------------------------------------------*/
void	  FlexArea::GetXY(int &x1 ,int &y1 ,int &x2 ,int &y2)
{
	x1=GetMinX();
	y1=GetMinY();
	x2=GetMaxX();
...
void	FlexAreaBasePure::Swap(FlexAreaBasePure &dest)
{
	...
	swap(Len,dest.Len);
	swap(OwningFLines,dest.OwningFLines);
	swap(AddX,dest.AddX);
	swap(AddY,dest.AddY);
	swap(MinX,dest.MinX);
	swap(MaxX,dest.MaxX);
...
bool	  FlexAreaBasePure::WriteBase(QIODevice *str)
{
	...
	if(str->write((const char *)&iDotPerLine,sizeof(iDotPerLine))!=sizeof(iDotPerLine)){
		return(false);
	}
	if(str->write((const char *)&iMaxLines,sizeof(iMaxLines))!=sizeof(iMaxLines)){
		return(false);
	}
	return(true);
}
...
//
/*----------------------------------------------------------------------------*/
FlexArea	&FlexArea::operator=(FlexArea &src)
{
	PointerCopy(&src);
	return(*this);
}
...
{
	PointerCopy((FlexAreaBasePure *)&src);
	return(*this);
}
/*----------------------------------------------------------------------------*/
//
...
//
/*----------------------------------------------------------------------------*/
bool		FlexArea::operator==(FlexArea &src)
{
...
//
/*----------------------------------------------------------------------------*/
bool	  FlexArea::IsInRectangle(int x1 ,int y1 ,int x2 ,int y2)
{
	for(int i=0;i<GetFLineLen();i++){
...
//
/*----------------------------------------------------------------------------*/
bool	  FlexArea::CheckOverlap(FlexArea *b)
{
...
bool	  FlexArea::CheckOverlapNeighbor(FlexArea *b)
{
...
//
/*----------------------------------------------------------------------------*/
bool	  FlexArea::CheckOverlapRectangle(int x1 ,int y1 ,int x2 ,int y2)
{
	if(x1>x2){
		swap(x1,x2);
	}
...
//
/*----------------------------------------------------------------------------*/
bool	  FlexArea::CheckOverlapLine(int x1 ,int y1 ,int x2 ,int y2)
{
	long X;
	long Y;
	if(y1!=y2){
...
bool	  FlexArea::CheckOverlapLine(double x1 ,double y1 ,double x2 ,double y2)
{
	double	X;
	double	Y;
	if(y1!=y2){
...
//
/*----------------------------------------------------------------------------*/
int 	  FlexArea::SearchFirst(int y)
{
	struct FlexLine m;
	m._SetAbsY(this,y);
...
bool	  FlexArea::IsInclude(int x ,int y)
{
	if(GetFLineLen()==0){
		return false;
	}
	...
	int MaxY=GetFLineAbsY(GetFLineLen()-1);
	...
	struct	 FlexLine	Key;
	Key._SetAbsY(this,y);
	struct FlexLine *c=(struct FlexLine *)bsearch(&Key, GetFLinePoint(), GetFLineLen(), sizeof(struct FlexLine), IsIncludeSearch);
	if(c==NULL){
		return(false);
	}
	...
	for(int i=LIndex+1;i<GetFLineLen();i++){
		if(GetFLineAbsY(i)!=y){
			break;
		}
		if(GetFLineLeftX(i)<=x && x<GetFLineRightX(i)){
...
//
/*----------------------------------------------------------------------------*/
void	  FlexArea::GetCenter(int &cx ,int &cy)
{
	cx=(GetMinX()+GetMaxX())/2;
	cy=(GetMinY()+GetMaxY())/2;
}

void	FlexArea::GetCenterOrOnIt(int &x ,int &y)
{
	GetCenter(x ,y);
...
void	  FlexArea::GetCenter(double &cx ,double &cy)
{
	cx=(GetMinX()+GetMaxX())/2;
	cy=(GetMinY()+GetMaxY())/2;
}

void    FlexArea::GetCenter(DualIntClass &xy)
{
										...
										,int px ,int py)
{
	...
	Len=N;
	register	struct FlexLine *MFLines=new struct FlexLine[Len];
...
//
/*----------------------------------------------------------------------------*/
void  FlexArea::MakeBitData(BYTE *data ,int xbyte,int DotPerLine, int MaxLines)
{
	for(int i=0;i<GetFLineLen();i++){
		if(GetFLineAbsY(i)<0 || GetFLineAbsY(i)>=MaxLines){
			continue;
...
void  FlexArea::MakeBitData(BYTE **data ,int XDotLen ,int YDotLen ,bool EnableThread)
{
	if(GetFLineLen()>=50){
...
void  FlexArea::MakeNotBitData(BYTE **data ,int XDotLen ,int YDotLen ,int mx ,int my)
{
	if(GetFLineLen()>=50){
		struct	FlexAreaYDim	YDim[1000];
		struct	FlexAreaYDim	*YDimPoint;
...
//
/*----------------------------------------------------------------------------*/
void  FlexArea::MakeMaskBitData(BYTE **data,int DotPerLine, int MaxLines)
{
	if(GetFLineLen()>=50){
		struct	FlexAreaYDim	YDim[1000];
		struct	FlexAreaYDim	*YDimPoint;
...
//
/*----------------------------------------------------------------------------*/
bool  FlexArea::CheckOverlapBit(BYTE *data ,int xbyte,int DotPerLine, int MaxLines)
{
	for(int i=0;i<GetFLineLen();i++){
		int Y=GetFLineAbsY(i);
		if(Y<0 || Y>=MaxLines){
...
//
/*----------------------------------------------------------------------------*/
bool  FlexArea::CheckOverlapBit(BYTE **data , int XDot ,int YDot)
{
	for(int i=0;i<GetFLineLen();i++){
		int Y=GetFLineAbsY(i);
		if(Y<0 || Y>=YDot){
...
//
/*----------------------------------------------------------------------------*/
//void	 ClusterizeLocal(FlexArea *parent ,int index1 ,int index2 ,NPListPack<IntClass> &PathIndex)
bool	  FlexArea::NearBy(struct FlexLine &L)
{
	...
	struct FlexLine *src=(struct FlexLine *)bsearch(&LL,GetFLinePoint(),GetFLineLen(),sizeof(LL),NearByFunc);
	if(src==NULL){
		return(false);
	}
...
//
/*----------------------------------------------------------------------------*/
void	  FlexArea::EatArea(FlexArea *food)
{
...
void	FlexArea::ChopRect(PureFlexAreaListContainer &Piece ,int ChopSize)
{
...
//
/*----------------------------------------------------------------------------*/
bool	  FlexArea::IsInclude(FlexArea *b)
{
...
//
/*----------------------------------------------------------------------------*/
int 	  FlexArea::GetSumColor(int dx,int dy,ImageBuffer &src ,int xdotperline ,int ymaxlines)
{
	int D=0;
...
int 	  FlexArea::GetSumColorSep4(int dx,int dy,ImageBuffer &src ,int xdotperline ,int ymaxlines)
{
	int D=0;
...
//
/*----------------------------------------------------------------------------*/
int64 RFlexAreaList::GetPatternByte(void)
{
	int64 tByte=0;
...
//
/*----------------------------------------------------------------------------*/
FlexArea	&FlexArea::operator=(RFlexAreaList &src)
{
...
void	  FlexArea::Sub(FlexArea &src1 , FlexArea &src2)
{
...
void	FlexArea::Sub(FlexArea &src, int dx ,int dy)
{
...
void	FlexArea::Add(FlexArea &src1 , FlexArea &src2)
{
...
bool	FlexArea::SubInside(FlexArea &src, int dx ,int dy ,float Multiply)
{
	if(GetFLineLen()==0){
		return(true);
	}
...
void	FlexArea::ThinAreaPartial(uchar **bmpdata,uchar **tmpdata,int xbyte ,int YLen)
{
	...
	#pragma omp parallel
	{
		...
		for(int i=0;i<NNumb;i++){
			...
			if(y>=1 && (y+1)<YLen){
				register	uchar	*p2=  &bmpdata[y][KMinXb];
				register	uchar	*p1=  &tmpdata[y][KMinXb];
				register	uchar	*p1p= &tmpdata[y+1][KMinXb];
				register	uchar	*p1m= &tmpdata[y-1][KMinXb];
...
PureFlexAreaList::PureFlexAreaList(PureFlexAreaList &src)
{
	FlexArea::operator=(src);
}

PureFlexAreaList::PureFlexAreaList(FlexArea &src)
{
	FlexArea::operator=(src);
}
...
PureFlexAreaList	&PureFlexAreaList::operator=(PureFlexAreaList &src)
{
	FlexArea::operator=(*((FlexArea *)&src));
	return(*this);
}
PureFlexAreaList	&PureFlexAreaList::operator=(FlexArea &src)
{
	FlexArea::operator=(src);
	return(*this);
}
...
bool	FlexArea::GetPeak(ImageBuffer &Buffer ,int dx ,int dy , int &PeakL ,int &PeakH)
{
	int	iPeakL=256;
	int	iPeakH=-1;
	for(int i=0;i<GetFLineLen();i++){
		...
		if(x2>=Buffer.GetWidth()){
			x2=Buffer.GetWidth()-1;
		}

		if(x1>x2){
			continue;
		}
		if(x2==0 || x1==(Buffer.GetWidth()-1)){
			continue;
		}
...
void	 FlexArea::MakeBitData(ImageBuffer &BitImg ,int dx, int dy)
{
	int	NNumb=GetFLineLen();
...
bool	FlexAreaBasePure::GetFlexLine(int index , struct FlexLine &Ret)
{
	if(0<=index && index<Len){
...
int FlexAreaBasePure::SearchNearBy(struct FlexLine Key)
{
	...
	struct FlexLine *ret=(struct FlexLine *)bsearch(&Key,GetFLinePoint(),GetFLineLen(),sizeof(struct FlexLine),NearByFunc);
	if(ret==NULL)
		return -1;
...
double	 FlexArea::GetAverageHalf(ImageBuffer &Data,int dx ,int dy ,bool SkippingMode)
{
	int 	N=0;
	double	D=0.0;
	int 	RFrac=1;
...
double	 FlexArea::GetAverage(ImageBuffer &Data,int dx ,int dy)
{
	...
	for(int i=0;i<GetFLineLen();i++){
		...
		register	BYTE	*p=Data.GetY(y)+x1;
		int Di=0;
		for(int n=0;n<Len;n++){
		   Di+= *p;
		   p++;
...
double	 FlexArea::GetAverageRough(ImageBuffer &Data,int dx ,int dy)
{
	int 	N=0;
	double	D=0.0;
	int 	RFrac=1;
...
void	FlexArea::GetMinMaxBrightness(ImageBuffer &Data,int dx ,int dy ,int &MinBrightness,int &MaxBrightness)
{
	MinBrightness	=255;
	MaxBrightness	=0;
	for(int i=0;i<GetFLineLen();i++){
...
static	bool   MazeBoolLocal(FlexArea *parent ,int index1 ,int index2 ,NPListPack<IntClass> &PathIndex)
{
	IntClass *d;
	for(int i=index2;i>=0;i--){
		...
		if(ret==0){
			...
			PathIndex.AppendList(new IntClass(i));
...
{
	NPListPack<IntClass>	PathIndex;

	return(MazeBoolLocal(this ,index ,index ,PathIndex));
}

 static  void  ClusterizeLocal(FlexArea *parent ,int index1 ,int index2 ,NPListPack<IntClass> &PathIndex)
{
	IntClass *d;
	for(int i=index2;i<parent->GetFLineLen();i++){
		if(i==index1){
...
bool	  FlexArea::Clusterize(FlexArea &Remaining)
{
	...
	PathIndex.AppendList(new IntClass(0));
	ClusterizeLocal(this ,0,0 ,PathIndex);
	int FlexLineNumb=PathIndex.GetNumber();
...
double	FlexArea::GetVar(int mx1 ,int my1 ,int mx2 ,int my2
					,ImageBuffer &bufftop ,double &DAvr)
{
	uint	D=0;
	uint	Avr=0;
	uchar	*p1;
									 ...
									 ,double AdoptRateL,double AdoptRateH)
{
	...
	int	WCount=(GetWidth()+CellSize-1)/CellSize;
	int	HCount=(GetHeight()+CellSize-1)/CellSize;
...
double	FlexArea::GetCoeff(ImageBuffer &Src ,int dx ,int dy ,ImageBuffer &Dst,int Isolation ,bool ThreadMode)
{
	double	SumS=0;
	double	SumD=0;
	double	SumSS=0;
...
void  FlexArea::MakeBitmap(ColorLogic &Color ,int Mergin ,ImagePointerContainer &Images 
						,int MapDx ,int MapDy ,BYTE **DestMap ,int XLen ,int YLen)
{
	if(XLen==0){
...
int64		 FlexArea::GetCrossBitCount(BYTE **data)
{
	int64	Count=0;
	int	NLen=GetFLineLen();
...
int64		 FlexArea::GetCrossBitCount(BYTE **data,int dx ,int dy ,int srcxbyte ,int srcylen ,int LineIsolation)
{
	int64	Count=0;
	int	XLen=srcxbyte*8;
	int	NLen=GetFLineLen();
...
void	FlexArea::Rotate(int AngleDegree ,int Cx, int Cy)
{
	int	W=GetWidth();
	int	H=GetHeight();
	...
	BuildFromRaster(BMap,XByte,YLen,Cx+Rx-x2,Cy+Ry-x2);
...
int		FlexArea::GetPatternWidth(void)
{
	int	NLen=GetFLineLen();
	int	Width=0;
	for(int i=0;i<NLen;i++){
...
int		FlexArea::GetPatternHeight(void)
{
	int	Height=0;
...
PureFlexAreaListContainer	&PureFlexAreaListContainer::operator=(PureFlexAreaListContainer &src)
{
	RemoveAll();
...
bool	PureFlexAreaListContainer::IsInclude(int x ,int y)
{
	for(PureFlexAreaList *a=GetFirst();a!=NULL;a=a->GetNext()){
