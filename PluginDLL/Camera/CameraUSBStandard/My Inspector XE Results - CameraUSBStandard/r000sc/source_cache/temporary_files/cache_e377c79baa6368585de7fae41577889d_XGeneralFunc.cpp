...
bool	Save(QIODevice *f,char *d,int64 Len)
{
...
bool	Load(QIODevice *f,char *d,int64 &Len ,int64 maxbuffsize)
{
...
void	SetDataToTable(QTableWidget *W ,int col ,int row ,const QVariant &value ,Qt::ItemFlags flags)
{
...
void	SetDataToTableComboBox(QTableWidget *W ,int col ,int row ,QStringList &List ,int CurrnetIndex)
{
	//W->model()->setData(W->model()->index(row,col),value);
...
int		GetSelectedRows(QTableWidget *W ,IntList &Rows)
{
	...
	for(int i=0;i<N;i++){
		...
		if(Rows.IsInclude(r)==false){
			Rows.AppendList(new IntClass(r));
			Counter++;
		}
	}
	return Counter;
...
char	*ToStr(char Text[] ,double N ,int Decimal)
{
	char	*fp=Text;
	if(N<0){
		*fp='-';
...
bool	CheckHidden(QWidget *v)	//If v is overlapped by other widget or tab/stacked widget , it returns true
{
