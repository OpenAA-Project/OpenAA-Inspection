...
,Parent(NULL)
{
	...
	DLL_GetAlignmentForProcessingByHandle=NULL;
...
,Parent(parent)
{
	...
	DLL_GetAlignmentForProcessingByHandle			=Parent->DLL_GetAlignmentForProcessingByHandle			;
...
bool	AlgorithmDLLList::LoadDLL(const QString &filename)
{
	...
	DLL_GetExplain				=(char *(*)(void))DllLib.resolve("DLL_GetExplain");
	...
	DLL_GetBlobName				=(char *(*)(void))DllLib.resolve("DLL_GetBlobName");
	DLL_GetAlgorithmType		=(DWORD (*)(void))DllLib.resolve("DLL_GetAlgorithmType");

	DLL_InitialAlloc			=(AlgorithmBase *(*)(EntryPointBase *))DllLib.resolve("DLL_InitialAlloc");
	...
	DLL_ExecuteInitialAfterEdit		=(ExeResult	(*)(int,AlgorithmBase *,ResultBaseForAlgorithmRoot * ))DllLib.resolve("DLL_ExecuteInitialAfterEdit");
	...
	DLL_ShowAndSetItems				=(void  (*)(QWidget *, int32 ,AlgorithmBase *,AlgorithmItemIndependentPack &,ShowAndSetItemsBaseClass *Something))DllLib.resolve("DLL_ShowAndSetItems");
	...
	DLL_SetIndependentItemData		=(void	(*)(int32 ,AlgorithmBase *,int32 ,int32 ,int32 ,AlgorithmItemRoot *,QByteArray &))DllLib.resolve("DLL_SetIndependentItemData");
	DLL_DrawResultDetail			=(bool	(*)(AlgorithmItemRoot *Item ,ResultPosList *Pos ,QStringList &RetList , QRgb &NGColor))DllLib.resolve("DLL_DrawResultDetail");

	DLL_ChangeItemsAttr				=(void  (*)(QWidget *, int32 ,AlgorithmBase *,AlgorithmItemIndependentPack &))DllLib.resolve("DLL_ChangeItemsAttr");
	...
	DLL_GetAlignmentPointer			=(AlignmentPacketBase *(*)(AlgorithmBase *base ,int localPage, int localX ,int localY,AlgorithmItemRoot *Item))DllLib.resolve("DLL_GetAlignmentPointer");
	...
	DLL_GetAlignmentForProcessingByHandle	=(void (*)(AlgorithmBase *base ,int localPage, AlignmentPacketBase &Packet ,void *Handle))DllLib.resolve("DLL_GetAlignmentForProcessingByHandle");
	...
	DLL_ResultItemToString			=(bool (*)(ResultInItemRoot *ItemRes ,char StrBuf[] ,int ByteOfStrBuff))DllLib.resolve("DLL_ResultItemToString");
...
void	AlgorithmDLLList::GetExportFunctions(QStringList &Str)
{
	...
	if(DLL_GetAlignmentForProcessing!=NULL){
		Str.append("DLL_GetAlignmentForProcessingByHandle");
	}
...
char			*AlgorithmDLLList::GetExplain(void)
{
	if(DLL_GetExplain!=NULL){
		InFunc=true;
		char	*ret=DLL_GetExplain();
		InFunc=false;
		return ret;
	}
	return NULL;
...
char			*AlgorithmDLLList::GetBlobName(void)
{
	if(DLL_GetBlobName!=NULL){
		InFunc=true;
		char	*ret=DLL_GetBlobName();
		InFunc=false;
		return ret;
	}
	return NULL;
...
AlgorithmBase	*AlgorithmDLLList::InitialAlloc(EntryPointBase *pInspectionData)
{
	if(DLL_InitialAlloc!=NULL){
		InFunc=true;
		AlgorithmBase	*ret=DLL_InitialAlloc(pInspectionData);
		InFunc=false;
		return ret;
	}
	return NULL;
...
ExeResult	AlgorithmDLLList::ExecuteInitialAfterEdit	(int ExeID ,AlgorithmBase *base,ResultBaseForAlgorithmRoot *Res)
{
	if(DLL_ExecuteInitialAfterEdit!=NULL){
		InFunc=true;
		ExeResult	ret=DLL_ExecuteInitialAfterEdit	(ExeID,base,Res);
		InFunc=false;
		return ret;
	}
	return _ER_NoFunc;
...
void	AlgorithmDLLList::ShowAndSetItems(QWidget *parent, int32 Command ,AlgorithmBase *InstBase ,AlgorithmItemIndependentPack &Data,ShowAndSetItemsBaseClass *Something)
{
	if(DLL_ShowAndSetItems!=NULL){
		...
		if(Data.Items.GetNumber()!=0){
			InFunc=true;
			DLL_ShowAndSetItems(parent, Command ,InstBase ,Data,Something);
			InFunc=false;
		}
	}
}
...
void	AlgorithmDLLList::SetIndependentItemData(int32 Command,AlgorithmBase *InstBase ,int32 TargetPhase,int32 LocalPage,int32 Layer,AlgorithmItemRoot *Data,QByteArray &Something)
{
	if(DLL_SetIndependentItemData!=NULL){
		InFunc=true;
		DLL_SetIndependentItemData(Command,InstBase ,TargetPhase,LocalPage,Layer,Data,Something);
		InFunc=false;
	}
}
bool	AlgorithmDLLList::DrawResultDetail(AlgorithmItemRoot *Item ,ResultPosList *Pos ,QStringList &RetList , QRgb &NGColor)
{
	if(DLL_DrawResultDetail!=NULL){
		InFunc=true;
		bool	ret=DLL_DrawResultDetail(Item ,Pos ,RetList , NGColor);
		InFunc=false;
		return ret;
	}
	return false;
...
void	AlgorithmDLLList::ChangeItemsAttr(QWidget *parent, int32 Command ,AlgorithmBase *InstBase ,AlgorithmItemIndependentPack &Data)
{
	if(DLL_ChangeItemsAttr!=NULL){
		InFunc=true;
		DLL_ChangeItemsAttr(parent, Command ,InstBase ,Data);
		InFunc=false;
	}
}
...
bool	AlgorithmDLLList::ResultItemToString(ResultInItemRoot *ItemRes ,char StrBuf[] ,int ByteOfStrBuff)
{
	if(DLL_ResultItemToString!=NULL){
		InFunc=true;
		bool	ret=DLL_ResultItemToString(ItemRes ,StrBuf ,ByteOfStrBuff);
		InFunc=false;
		return ret;
	}
	return false;
...
AlignmentPacketBase	*AlgorithmDLLList::GetAlignmentPointer(AlgorithmBase *base ,int localPage, int localX ,int localY,AlgorithmItemRoot *Item)
{
	if(DLL_GetAlignmentPointer!=NULL){
		InFunc=true;
		AlignmentPacketBase	*ret=DLL_GetAlignmentPointer(base ,localPage, localX ,localY,Item);
		InFunc=false;
		return ret;
	}
	return NULL;
...
void		AlgorithmDLLList::GetAlignmentForProcessingByHandle(AlgorithmBase *base ,int localPage, AlignmentPacketBase &Packet ,void *Handle)
{
	if(DLL_GetAlignmentForProcessingByHandle!=NULL){
		InFunc=true;
		DLL_GetAlignmentForProcessingByHandle(base ,localPage, Packet,Handle);
		InFunc=false;
	}
}
...
void		AlgorithmDLLList::SetPriority(AlgorithmDefList &Def)
{
...
int	AlgorithmDLLContainer::SearchAddDLL(RootNameListContainer &AlgoList ,LayersBase *Base ,QString path,bool LoadAll)
{
	QDir	Dir;
	int	N=0;
...
LogicDLL	&LogicDLL::operator=(LogicDLL &src)
{
	...
	InitialAlloc(GetLayersBase()->GetEntryPoint());
...
bool	LogicDLL::InitialAlloc(EntryPointBase *PInspectionData)
{
	...
	char	*p=DLLPointer->GetBlobName();
	if(p!=NULL){
		BlobName=QString(p);
	}

	int	LanguageCode=0;
	if(PInspectionData!=NULL && PInspectionData->GetLayersBase()!=NULL && PInspectionData->GetLayersBase()->GetFRegistry()!=NULL){
		LanguageCode=PInspectionData->GetLayersBase()->GetFRegistry()->LoadRegInt("Language",0);
	}
	SetLanguageCode(LanguageCode);
	InstBase=AllocOnly(PInspectionData);
...
void	LogicDLL::ReallocXYPixels(int NewDotPerLine ,int NewMaxLines)
{
	GetInstance()->ReallocXYPixels(NewDotPerLine ,NewMaxLines);
}
void	LogicDLL::ChangeAlgorithmType(DWORD algorithmtype)
{
	AlgorithmType	=algorithmtype;
}
AlgorithmBase	*LogicDLL::AllocOnly(EntryPointBase *PInspectionData)
{
	AlgorithmBase	*p=DLLPointer->InitialAlloc(PInspectionData);
	if(p!=NULL){
...
void	LogicDLL::ReallocAlgorithmBase(EntryPointBase *PInspectionData)
{
	//DLLPointer->ReleaseAlgorithmBase(InstBase);
	//InstBase=DLLPointer->InitialAlloc(PInspectionData);
	InstBase->Initial(PInspectionData);
	QFile	FAlgprithm(GetLayersBase()->GetCurrentPath()
					 +GetSeparator()
...
int		LogicDLL::GetPriority(int PriorityTypeBit_Type)
{
...
AlignmentPacketBase	*LogicDLL::GetAlignmentPointer(int localPage, int localX ,int localY,AlgorithmItemRoot *Item)
{
	AlignmentPacketBase	*ret=DLLPointer->GetAlignmentPointer(InstBase,localPage, localX,localY,Item);
	return ret;
}
...
void		LogicDLL::GetAlignmentForProcessingByHandle(int localPage, AlignmentPacketBase &Packet ,void *Handle)
{
	DLLPointer->GetAlignmentForProcessingByHandle(InstBase,localPage, Packet ,Handle);
}
...
ExeResult	LogicDLL::ExecuteInitialAfterEdit	(int ExeID ,ResultBaseForAlgorithmRoot	*pResult)
{
	ExeResult	ret=DLLPointer->ExecuteInitialAfterEdit(ExeID ,InstBase,pResult);
...
bool	LogicDLL::DrawResultDetail(AlgorithmItemRoot *Item ,ResultPosList *Pos ,QStringList &RetList , QRgb &NGColor)
{
	bool	ret=DLLPointer->DrawResultDetail(Item ,Pos ,RetList , NGColor);
	if((GetParamGlobal()->DebugLevel & 0x08)!=0){
		if(_CrtCheckMemory()==false){
			ErrorOccurs();
		}
...
{
	DLLPointer->ShowAndSetItems(parent, Command ,InstBase ,Data,Something);
}
...
void	LogicDLL::SetIndependentItemData(int32 Command,int32 TargetPhase,int32 LocalPage,int32 Layer,AlgorithmItemRoot *Data,QByteArray &Something)
{
	DLLPointer->SetIndependentItemData(Command,GetInstance() ,TargetPhase,LocalPage,Layer,Data,Something);
}

void	LogicDLL::ChangeItemsAttr(QWidget *parent, int32 Command ,AlgorithmBase *InstBase ,AlgorithmItemIndependentPack &Data)
{
	DLLPointer->ChangeItemsAttr(parent, Command ,InstBase ,Data);
}
...
bool	LogicDLL::ResultItemToString(ResultInItemRoot *ItemRes ,char StrBuf[] ,int ByteOfStrBuff)
{
	return DLLPointer->ResultItemToString(ItemRes ,StrBuf ,ByteOfStrBuff);
}
...
bool	LogicDLL::GetMaskBitmap(ConstBitBuffer &MaskMap ,int page, int layer ,AlgorithmLibrary *LibData ,void *Anything)
{
	BYTE	**B=GetInstance()->GetPageData(page)->CreateMaskBitmapForMaskSide(layer ,LibData ,Anything);
	if(B==NULL){
		return false;
	}
	MaskMap.Set(B,(GetDotPerLine()+7)/8,GetMaxLines());
	return true;
}
bool	LogicDLL::GetMaskBitmap(ConstBitBuffer &MaskMap ,int page, int layer ,void *Anything)
{
	BYTE	**B=GetInstance()->GetPageData(page)->CreateMaskBitmapForMaskSide(layer ,Anything);
	if(B==NULL){
		return false;
	}
	MaskMap.Set(B,(GetDotPerLine()+7)/8,GetMaxLines());
...
bool	LogicDLL::GetMaskVector(VectorLineBase &MaskVector ,int page, int layer ,AlgorithmLibrary *LibData ,void *Anything)
{
	bool	ret=GetInstance()->GetPageData(page)->CreateMaskVectorForMaskSide(MaskVector ,layer ,LibData ,Anything);
	return ret;
}

bool	LogicDLL::GetMaskVector(VectorLineBase &MaskVector ,int page, int layer ,void *Anything)
{
	bool	ret=GetInstance()->GetPageData(page)->CreateMaskVectorForMaskSide(MaskVector ,layer ,Anything);
	return ret;
}
bool	LogicDLL::IncludeLibInMask(int page, int layer ,AlgorithmLibrary *LibData ,void *Anything)
{
	bool	ret=GetInstance()->GetPageData(page)->IncludeLibInMaskForMaskSide(layer,LibData,Anything);
	return ret;
}
...
LogicDLLBaseClass	&LogicDLLBaseClass::operator=(LogicDLLBaseClass &src)
{
	RemoveAll();
...
void	LogicDLLBaseClass::CopyFrom(LayersBase *ToBase,LogicDLLBaseClass &src)
{
	RemoveAll();
	for(LogicDLL *L=src.GetFirst();L!=NULL;L=L->GetNext()){
...
int	LogicDLLBaseClass::AddDLLs(AlgorithmDLLContainer &AlgoCont ,LayersBase *Base)
{
	int	N=0;
	...
	for(AlgorithmDLLList *L=AlgoCont.GetFirst();L!=NULL;L=L->GetNext()){
		...
		printf("InitialAlloc\n");
		DLLInst->InitialAlloc(Base->GetEntryPoint());
		AppendList(DLLInst);
		N++;
	}
	return N;
...
bool	LogicDLLBaseClass::CheckAndCreateBlobInMasterPageTable(QSqlDatabase &db)
{
	for(LogicDLL *L=GetFirst();L!=NULL;L=L->GetNext()){
...
ExeResult	LogicDLLBaseClass::ExecuteInitialAfterEdit	(int ExeID ,ResultInspection *Res)
{
	bool	ReTryMode;
	...
	if(Res!=NULL){
		...
		do{
			...
			for(int i=0;i<PNumb;i++){
				...
				ExeResult	RR=LDimP[i].Point->ExecuteInitialAfterEdit(ExeID,r);
		...
		do{
			...
			for(int i=0;i<ANumb;i++){
				...
				ExeResult	RR=LDimA[i].Point->ExecuteInitialAfterEdit	(ExeID,r);
				if(RR==_ER_ReqRetryLater){
					ReTryMode=true;
				}
				else if(RR!=_ER_true){
		...
		do{
			...
			for(int i=0;i<SNumb;i++){
				...
				ExeResult	RR=LDimS[i].Point->ExecuteInitialAfterEdit	(ExeID,r);
				if(RR==_ER_ReqRetryLater){
					ReTryMode=true;
				}
				else if(RR!=_ER_true){
...
bool	LogicDLLBaseClass::GetMaskBitmap(ConstBitBuffer &MaskMap ,int page, int layer ,AlgorithmLibrary *LibData ,void *Anything)
{
	bool	ret=false;
	for(LogicDLL *L=GetFirst();L!=NULL;L=L->GetNext()){
		if(L->CheckAlgorithmType(AlgorithmBit_TypeMasking)==true){
			ConstBitBuffer tMaskMap((GetDotPerLine()+7)/8,GetMaxLines());
			if(L->GetMaskBitmap(tMaskMap ,page, layer ,LibData ,Anything)==true){
				MaskMap|=tMaskMap;
				ret=true;
			}
		}
...
bool	LogicDLLBaseClass::GetMaskBitmap(ConstBitBuffer &MaskMap ,int page, int layer ,void *Anything)
{
	bool	ret=false;
	for(LogicDLL *L=GetFirst();L!=NULL;L=L->GetNext()){
		if(L->CheckAlgorithmType(AlgorithmBit_TypeMasking)==true){
			ConstBitBuffer tMaskMap((GetDotPerLine()+7)/8,GetMaxLines());
			if(L->GetMaskBitmap(tMaskMap ,page, layer ,Anything)==true){
				MaskMap|=tMaskMap;
				ret=true;
			}
		}
...
bool	LogicDLLBaseClass::GetMaskVector(VectorLineBase &MaskVector ,int page, int layer ,AlgorithmLibrary *LibData ,void *Anything)
{
	bool	ret=false;
	for(LogicDLL *L=GetFirst();L!=NULL;L=L->GetNext()){
		if(L->CheckAlgorithmType(AlgorithmBit_TypeMasking)==true){
			VectorLineBase *tMaskVector=MaskVector.CreateEmpty();
			if(L->GetMaskVector(*tMaskVector ,page, layer ,LibData,Anything)==true){
				MaskVector.AppendMoveBase(tMaskVector);
				ret=true;
			}
			delete	tMaskVector;
		}
...
bool	LogicDLLBaseClass::GetMaskVector(VectorLineBase &MaskVector ,int page, int layer ,void *Anything)
{
	bool	ret=false;
	for(LogicDLL *L=GetFirst();L!=NULL;L=L->GetNext()){
		if(L->CheckAlgorithmType(AlgorithmBit_TypeMasking)==true){
			VectorLineBase *tMaskVector=MaskVector.CreateEmpty();
			if(L->GetMaskVector(*tMaskVector ,page, layer ,Anything)==true){
				MaskVector.AppendMoveBase(tMaskVector);
				ret=true;
			}
			delete	tMaskVector;
...
bool	LogicDLLBaseClass::IncludeLibInMask(int page, int layer ,AlgorithmLibrary *LibData ,void *Anything)
{
	for(LogicDLL *L=GetFirst();L!=NULL;L=L->GetNext()){
		if(L->CheckAlgorithmType(AlgorithmBit_TypeMasking)==true){
			if(L->IncludeLibInMask(page, layer ,LibData ,Anything)==true){
				return true;
			}
		}
	}
