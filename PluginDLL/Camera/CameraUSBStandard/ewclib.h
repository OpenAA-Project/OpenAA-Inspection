...
int ewc_SetValueDirect(int num, int prop, long Val)
{
	...
	if(ewc_s[num].pCap!=NULL){
		if(prop<EWC_VPAMPMAX){
			...
			if(ewc_hr==S_OK){
				long tVal;
				ewc_hr= ewc_s[num].pVPAmp->Get(prop,&tVal,&Flags);	//現在値の取得
				ewc_hr= ewc_s[num].pVPAmp->Set(prop,Val,VideoProcAmp_Flags_Manual);
				ewc_release(ewc_s[num].pVPAmp);
				if(ewc_hr!=S_OK) return 5;
			}else{
			...
			if(ewc_hr==S_OK){
				long tVal;
				ewc_hr= ewc_s[num].pCamCtl->Get(prop,&tVal,&Flags);	//現在値の取得
				ewc_hr= ewc_s[num].pCamCtl->Set(prop,Val,CameraControl_Flags_Manual);
				ewc_release(ewc_s[num].pCamCtl);
				if(ewc_hr!=S_OK) return 5;
			}else{
...
cont:
	...
	//if(ewc_s[num].wx==0) ewc_s[num].wx= (ProductID==ProductID835 && ewc_wx==EWC_WX)?640:ewc_wx;		//デフォルト値の適用
	//if(ewc_s[num].wy==0) ewc_s[num].wy= (ProductID==ProductID835 && ewc_wy==EWC_WY)?480:ewc_wy;		//デフォルト値の適用
...
void	SetFrameRate(double FPS)
{
	...
	group->SetOutputFPS( FPS ); //現状よく分かってないので数値は適当です
	tline->AddGroup( obj );
