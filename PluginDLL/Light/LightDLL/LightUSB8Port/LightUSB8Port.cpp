...
{
	DirectLightControl	*h=new DirectLightControl(LightParamStr);
	LightControlFromUSB	*H=new LightControlFromUSB(base,8,64,64);
	H->Initial(h);
...
{
	LightControlFromUSB	*H=(LightControlFromUSB *)handle;

	if(H!=NULL){
		DirectLightControl	*h=H->GetDirectAccessor();
		h->Close();
		LightControlFromUSB	*p=static_cast<LightControlFromUSB *>(handle);
		delete	p;
	}
