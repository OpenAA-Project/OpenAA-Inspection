...
{
	LightBowl240RGBW	*H=new LightBowl240RGBW(base,LightParamStr);
	if(H->Initial()==false){
		QMessageBox::warning(NULL,"エラー"
							 ,"照明コントローラーと通信できません");
		return NULL;
	}
	return H;
...
{
	LightBowl240RGBW	*H=dynamic_cast<LightBowl240RGBW *>(handle);
	if(H!=NULL){
		H->Close();
		delete	H;
	}
...
//	if process fails, it returns false
{
	LightBowl240RGBW	*p=static_cast<LightBowl240RGBW *>(handle);
	bool	Ret=p->Load(&str);
	if(Ret==true){
		if(p->CurrentIndex>=0){
			p->Transfer(p->LightDataDim[p->CurrentIndex],p->ONMode);
		}
	}
	return Ret;
}
