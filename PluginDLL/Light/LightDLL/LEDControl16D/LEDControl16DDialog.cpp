...
#include "ui_LEDControl16DDialog.h"
...
LineGroup::LineGroup(LEDControl16DDialog *p ,QWidget *parent)
	:Parent(p)
	,Button(false,parent)
{
...
void	LineGroup::AddLine(LEDControl16DPanel::DefUnit::DefBoard::DefLine	*L ,QWidget *parent)
{
	for(int i=0;i<sizeof(LineData)/sizeof(LineData[0]);i++){
		if(LineData[i]==NULL){
...
LEDControl16DDialog::LEDControl16DDialog(LEDControl16DPanel *p,QWidget *parent) :
    QDialog(parent)
	,Panel(p)
    ,ui(new Ui::LEDControl16DDialog)
{
    ui->setupUi(this);
...
LEDControl16DDialog::~LEDControl16DDialog()
{
	for(int k=0;k<LineGroupDataNumb;k++){
...
void LEDControl16DDialog::SetPatternValue(int PatternNo)
{
	for(int k=0;k<LineGroupDataNumb;k++){
		for(int i=0;i<sizeof(LineGroupData[k]->LineData)/sizeof(LineGroupData[k]->LineData[0]);i++){
			if(LineGroupData[k]->LineData[i]==NULL)
				break;
			LEDControl16DPanel::DefUnit::DefBoard::DefLine	*L=LineGroupData[k]->LineData[i]->PointerForDefLine;

			for(int h=0;h<Panel->LightData[PatternNo].Pattern.AllocatedCountOfLines;h++){
				LEDControl16DPanel::DefUnit::DefBoard::DefLine	*D=Panel->LightData[PatternNo].Pattern.LineValueDim[h].PointerForDefLine;
				if(L!=NULL && D!=NULL 
				&& D->BoardNo==L->BoardNo
				&& D->UnitNo==L->UnitNo
				&& D->LineNo==L->LineNo){
...
void LEDControl16DDialog::GetPatternValue(int PatternNo)
{
	for(int k=0;k<LineGroupDataNumb;k++){
		for(int i=0;i<sizeof(LineGroupData[k]->LineData)/sizeof(LineGroupData[k]->LineData[0]);i++){
			if(LineGroupData[k]->LineData[i]==NULL)
...
void	LEDControl16DDialog::SlotClicked(int n)
{
	ui->stackedWidgetSetting->setCurrentIndex(n);
}

QProgressBar	*LEDControl16DDialog::GetBar(void)
{
	return ui->progressBar;
}

void LEDControl16DDialog::on_pushButtonOK_clicked()
{
	GetPatternValue(GetCurrentPatternNo());
	close();
}

void LEDControl16DDialog::on_pushButtonSetAll_clicked()
{
...
void LEDControl16DDialog::on_pushButtonTransfer_clicked()
{
	GetPatternValue(GetCurrentPatternNo());
	Panel->ReflectDataInDialog();
}

void LEDControl16DDialog::on_toolButtonON_clicked()
{
	GetPatternValue(GetCurrentPatternNo());
	Panel->ReflectOnOffInDialog(ui->toolButtonON->isChecked());
}

void LEDControl16DDialog::on_pushButtonDefinition_clicked()
{
...
void LEDControl16DDialog::on_pushButtonShowIO_clicked()
{
...
void LEDControl16DDialog::on_pushButtonShowSpecial_clicked()
{
...
void LEDControl16DDialog::on_pushButtonIODeliverer_clicked()
{
