...
{
	LEDControl16DPanel	*H=new LEDControl16DPanel(base ,LightParamStr);
...
{
	LEDControl16DPanel	*H=(LEDControl16DPanel *)handle;
	if(H!=NULL){
		H->Close();
		delete	H;
	}
...
//	In one case, this fuction is called from Sequence.dat
{
	LEDControl16DPanel	*p=static_cast<LEDControl16DPanel *>(handle);
	if(p!=NULL){
