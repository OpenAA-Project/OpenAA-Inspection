...
void    SocketClient::SlotReadyRead()
{
    ...
    if(Cmd=='A'){
        struct  KDSBeltPIOFixedStruct  Data;
        if(DataLen==sizeof(Data)){
            if(Socket->read((char *)&Data,sizeof(Data))==sizeof(Data)){
                Parent->GIOData.KDSBeltPIOFixedData=Data;
            }
        }
        else{
             QByteArray A=Socket->readAll();
        ...
        SendOut(-1,'B' ,(BYTE *)&Parent->GIOData.KDSBeltPIOFixedData ,sizeof(Parent->GIOData.KDSBeltPIOFixedData));
    }
    else if(Cmd=='C'){
        QByteArray Dummy=Socket->readAll();
        int Len;
...
void LEDControl16FRasPiKDSBelt::ReadGIO(int DipSW)
{
    ...
    ReadRegister4Bit(DipSW ,Data ,13);
