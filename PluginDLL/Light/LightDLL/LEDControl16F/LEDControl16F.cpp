...
{
	CopyrightString="Copyright(c) MEGATRADE 2020.08";
	return true;
}

DEFFUNCEX	const char	*DLL_GetExplain(void)
{
	return "LEDControl16F";
}
...
{
	LEDControl16FPanel	*H=new LEDControl16FPanel(base ,LightParamStr);
...
{
	LEDControl16FPanel	*H=(LEDControl16FPanel *)handle;
	if(H!=NULL){
		H->Close();
		delete	H;
	}
...
//	In one case, this fuction is called from Sequence.dat
{
	LEDControl16FPanel	*p=static_cast<LEDControl16FPanel *>(handle);
	if(p!=NULL){
