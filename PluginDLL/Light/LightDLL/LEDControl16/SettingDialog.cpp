...
SettingDialog::SettingDialog(LEDControl16Panel *p,QWidget *parent) :
    QDialog(parent)
	,Panel(p)
...
void	SettingDialog::InitialWindow	(LEDControl16Panel::PortData &Data)
{
	for(int i=0;i<sizeof(LineData)/sizeof(LineData[0]);i++){
		if(LineData[i]!=NULL)
			delete	LineData[i];
...
void	SettingDialog::ShowData			(int TypeNumber ,LEDControl16Panel::PortData &Data ,int PatternNumber)
{
	for(int i=0;i<Data.UsageLines && i<sizeof(LineData)/sizeof(LineData[0]);i++){
...
void	SettingDialog::GetDataFromWindow(int TypeNumber ,LEDControl16Panel::PortData &Data ,int PatternNumber)
{
	for(int i=0;i<Data.UsageLines && i<sizeof(LineData)/sizeof(LineData[0]);i++){
