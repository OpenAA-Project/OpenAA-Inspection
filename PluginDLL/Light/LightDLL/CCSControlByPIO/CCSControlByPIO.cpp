...
{
	LightCCSControlByPIO	*H=new LightCCSControlByPIO(base,LightParamStr);
	H->Initial();
	return H;
}
...
{
	LightCCSControlByPIO	*H=(LightCCSControlByPIO *)handle;
	if(H!=NULL){
		H->Close();
		delete	H;
	}
...
//	if process fails, it returns false
{
	LightCCSControlByPIO	*p=static_cast<LightCCSControlByPIO *>(handle);
	bool	Ret=p->Load(&str);
	if(Ret==true){
		p->CurrentPattern=0;
