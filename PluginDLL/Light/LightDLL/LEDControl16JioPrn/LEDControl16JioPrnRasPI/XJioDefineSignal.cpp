...
QString	JioPortDefinition::GetTypeName(int PortType,int BitNo)
{
	switch(PortType){
...
QString	JioPortDefinition::GetString(int PortType ,int BitNo)
{
	switch(PortType){
...
void	JioPortDefinition::SetString(int PortType ,int BitNo ,const QString &s)
{
	switch(PortType){
...
bool	JioPortDefinition::GetSignal(int PortType ,int BitNo ,struct  TimingData &d)
{
	switch(PortType){
	case DefPortPIOAIN	:
        if(BitNo==0)	return d.RealData.PIOAIN.Bit0;
        if(BitNo==1)	return d.RealData.PIOAIN.Bit1;
        if(BitNo==2)	return d.RealData.PIOAIN.Bit2;
        if(BitNo==3)	return d.RealData.PIOAIN.Bit3;
		break;
	case DefPortPIOBIN	:
        if(BitNo==0)	return d.RealData.PIOBIN.Bit0;
        if(BitNo==1)	return d.RealData.PIOBIN.Bit1;
        if(BitNo==2)	return d.RealData.PIOBIN.Bit2;
        if(BitNo==3)	return d.RealData.PIOBIN.Bit3;
		break;
	case DefPortPIOCIN	:
        if(BitNo==0)	return d.RealData.PIOCIN.Bit0;
        if(BitNo==1)	return d.RealData.PIOCIN.Bit1;
        if(BitNo==2)	return d.RealData.PIOCIN.Bit2;
        if(BitNo==3)	return d.RealData.PIOCIN.Bit3;
		break;
	case DefPortPIODIN	:
        if(BitNo==0)	return d.RealData.PIODIN.Bit0;
        if(BitNo==1)	return d.RealData.PIODIN.Bit1;
        if(BitNo==2)	return d.RealData.PIODIN.Bit2;
        if(BitNo==3)	return d.RealData.PIODIN.Bit3;
		break;
	case DefPortPIOAOUT	:
        if(BitNo==0)	return d.RealData.PIOAOUT.Bit0;
        if(BitNo==1)	return d.RealData.PIOAOUT.Bit1;
        if(BitNo==2)	return d.RealData.PIOAOUT.Bit2;
        if(BitNo==3)	return d.RealData.PIOAOUT.Bit3;
		break;
	case DefPortPIOBOUT	:
        if(BitNo==0)	return d.RealData.PIOBOUT.Bit0;
        if(BitNo==1)	return d.RealData.PIOBOUT.Bit1;
        if(BitNo==2)	return d.RealData.PIOBOUT.Bit2;
        if(BitNo==3)	return d.RealData.PIOBOUT.Bit3;
		break;
	case DefPortPIOCOUT	:
        if(BitNo==0)	return d.RealData.PIOCOUT.Bit0;
        if(BitNo==1)	return d.RealData.PIOCOUT.Bit1;
        if(BitNo==2)	return d.RealData.PIOCOUT.Bit2;
        if(BitNo==3)	return d.RealData.PIOCOUT.Bit3;
		break;
	case DefPortPIODOUT	:
        if(BitNo==0)	return d.RealData.PIODOUT.Bit0;
        if(BitNo==1)	return d.RealData.PIODOUT.Bit1;
        if(BitNo==2)	return d.RealData.PIODOUT.Bit2;
        if(BitNo==3)	return d.RealData.PIODOUT.Bit3;
		break;
	case DefPortGIN		:
	...
	case DefPortGOUT	:
        ...
        if(BitNo==4)	return d.RealData.GOUT4;
        if(BitNo==5)	return d.RealData.GOUT5;
        if(BitNo==6)	return d.RealData.GOUT6;
        if(BitNo==7)	return d.RealData.GOUT7;
		break;
	case DefPortTRGIN	:
...
bool    JioPortDefinition::Save(QIODevice *f)
{
	int32	Ver=1;
    if(::Save(f,Ver)==false)
        return false;
...
bool    JioPortDefinition::Load(QIODevice *f)
{
	int32	Ver;
    if(::Load(f,Ver)==false)
        return false;
