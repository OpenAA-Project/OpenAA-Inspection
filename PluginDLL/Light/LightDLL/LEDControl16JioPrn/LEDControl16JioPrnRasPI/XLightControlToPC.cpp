...
void	LightControlToPCClient::SlotReadyRead()
{
    ...
    if(Cmd==0x1A){
        ...
        if(Brd>=0){
            Parent->Parent->BoardData[Brd].GetLightData(Parent->Parent->Parent,PatternNo);
            int	Len=0;
            TxData[Len]=0xF3;
            Len++;
            for(int i=0;i<MaxCountLines;i++){
                uint32	d=Parent->Parent->BoardData[Brd].LineInfo[PatternNo].TargetValue[i];
        ...
        if(Brd>=0){
            ...
            Parent->Parent->BoardData[Brd].SetLightData(Parent->Parent->Parent,PatternNo);
        }
    }
    else if(Cmd==0xF5){
        BYTE	TxData[10+4*MaxCountLines];
        ...
        if(Brd>=0){
            Parent->Parent->BoardData[Brd].GetCurrentValue(Parent->Parent->Parent);
            int	Len=0;
            TxData[Len]=0xF3;
            Len++;
            for(int i=0;i<MaxCountLines;i++){
                uint32	d=Parent->Parent->BoardData[Brd].CurrentValue[i];
                TxData[1+Len+0]=d&0xFF;
                TxData[1+Len+1]=(d>>8)&0xFF;
                TxData[1+Len+2]=(d>>16)&0xFF;
                TxData[1+Len+3]=(d>>24)&0xFF;
...
LightControlToPCServer::LightControlToPCServer(ShowLEDControl16JForm *p)
:Parent(p)
{
...
{
    Parent->Parent->JioData.JioFixedData.DelayFrameTriggerA	=Data.TriggerTiming1;
    Parent->Parent->JioData.JioFixedData.DelayFrameTriggerB	=Data.TriggerTiming2;
    //Parent->Parent->GIOData.DropLength			=Data.DropTiming;
    Parent->Parent->SetJioDataToWindow();
    Parent->Parent->WriteJio(Parent->Parent->DipSW_Jio);
    //Parent->WriteEIO();
}
void	LightControlToPCServer::ReqData(struct	TriggerInfo &Data)
{
    Data.TriggerTiming1	=Parent->Parent->JioData.JioFixedData.DelayFrameTriggerA;
    Data.TriggerTiming2	=Parent->Parent->JioData.JioFixedData.DelayFrameTriggerB;
    //Data.DropTiming		=Parent->EIOData.DropLength;
}

void	LightControlToPCServer::ReqData(struct	ResultCounterInfo &Data)
{
    Data.TotalCount	=Parent->Parent->JioData.TotalCount;
    Data.OKCount	=Parent->Parent->JioData.OKCount;
    Data.NGCount	=Parent->Parent->JioData.NGCount;
    Data.TMCount	=Parent->Parent->JioData.TMCount;
}
void    LightControlToPCServer::ResetCounter(void)
{
    Parent->Parent->JioData.ResetCounter=true;
    Parent->Parent->WriteJio(Parent->Parent->DipSW_Jio);
    Parent->Parent->JioData.ResetCounter=false;
    Parent->Parent->WriteJio(Parent->Parent->DipSW_Jio);
}
