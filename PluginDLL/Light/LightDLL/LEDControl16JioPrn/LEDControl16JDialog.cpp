...
#include "ui_LEDControl16JDialog.h"
#include "XTypeDef.h"
...
LineGroup::LineGroup(LEDControl16JDialog *p ,QWidget *parent)
...
LEDControl16JDialog::LEDControl16JDialog(LEDControl16JioPanel *p,QWidget *parent) :
    QDialog(parent)
	,Panel(p)
	,ui(new Ui::LEDControl16JDialog)
{
    ui->setupUi(this);
...
LEDControl16JDialog::~LEDControl16JDialog()
{
	for(int k=0;k<LineGroupDataNumb;k++){
...
void LEDControl16JDialog::SetPatternValue(int PatternNo)
{
	for(int k=0;k<LineGroupDataNumb;k++){
		for(int i=0;i<LineGroupData[k]->LineNumb;i++){
			if(LineGroupData[k]->LineData[i]==NULL)
...
void LEDControl16JDialog::GetPatternValue(int PatternNo)
{
	for(int k=0;k<LineGroupDataNumb;k++){
		for(int i=0;i<LineGroupData[k]->LineNumb;i++){
			if(LineGroupData[k]->LineData[i]==NULL)
...
void	LEDControl16JDialog::SlotClicked(int n)
{
	ui->stackedWidgetSetting->setCurrentIndex(n);
}

QProgressBar	*LEDControl16JDialog::GetBar(void)
{
	return ui->progressBar;
}

void LEDControl16JDialog::on_pushButtonOK_clicked()
{
	GetPatternValue(GetCurrentPatternNo());
	close();
}

void LEDControl16JDialog::on_pushButtonSetAll_clicked()
{
...
void LEDControl16JDialog::on_pushButtonSetTestLED_clicked()
{
...
void LEDControl16JDialog::on_pushButtonTransfer_clicked()
{
	GetPatternValue(GetCurrentPatternNo());
	Panel->ReflectDataInDialog();
}

void LEDControl16JDialog::on_toolButtonON_clicked()
{
	GetPatternValue(GetCurrentPatternNo());
	Panel->ReflectOnOffInDialog(ui->toolButtonON->isChecked());
}

void LEDControl16JDialog::on_pushButtonDefinition_clicked()
{
...
void LEDControl16JDialog::on_pushButtonShowIO_clicked()
{
...
void LEDControl16JDialog::on_pushButtonShowSpecial_clicked()
{
...
void LEDControl16JDialog::on_pushButtonIODeliverer_clicked()
{
