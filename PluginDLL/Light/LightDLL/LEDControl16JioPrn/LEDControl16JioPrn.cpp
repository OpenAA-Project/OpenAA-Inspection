...
{
	CopyrightString="Copyright(c) MEGATRADE 2024.01";
	return true;
}

DEFFUNCEX	const char	*DLL_GetExplain(void)
{
	return "LEDControl16JioPrn";
}
...
{
	LEDControl16JioPanel	*H=new LEDControl16JioPanel(base ,LightParamStr);
...
{
	LEDControl16JioPanel	*H=(LEDControl16JioPanel *)handle;
	if(H!=NULL){
		H->Close();
		delete	H;
	}
...
//	In one case, this fuction is called from Sequence.dat
{
	LEDControl16JioPanel	*p=static_cast<LEDControl16JioPanel *>(handle);
	if(p!=NULL){
