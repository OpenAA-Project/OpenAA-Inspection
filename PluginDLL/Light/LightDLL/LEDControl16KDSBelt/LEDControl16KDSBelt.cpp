...
{
	LEDControl16KDSBeltPanel	*H=new LEDControl16KDSBeltPanel(base ,LightParamStr);
...
{
	LEDControl16KDSBeltPanel	*H=(LEDControl16KDSBeltPanel *)handle;
	if(H!=NULL){
		H->Close();
		delete	H;
	}
...
//	In one case, this fuction is called from Sequence.dat
{
	LEDControl16KDSBeltPanel	*p=static_cast<LEDControl16KDSBeltPanel *>(handle);
	if(p!=NULL){
