...
{
	CopyrightString="Copyright(c) MEGATRADE 2021.02";
	return true;
}

DEFFUNCEX	const char	*DLL_GetExplain(void)
{
	return "LEDControl16H";
}
...
{
	LEDControl16HPanel	*H=new LEDControl16HPanel(base ,LightParamStr);
...
{
	LEDControl16HPanel	*H=(LEDControl16HPanel *)handle;
	if(H!=NULL){
		H->Close();
		delete	H;
	}
...
//	In one case, this fuction is called from Sequence.dat
{
	LEDControl16HPanel	*p=static_cast<LEDControl16HPanel *>(handle);
	if(p!=NULL){
