...
{
	CopyrightString="Copyright(c) MEGATRADE 2017.08";
	return true;
}

DEFFUNCEX	const char	*DLL_GetExplain(void)
{
	return "LEDControl16B";
}
...
{
	LEDControl16BPanel	*H=new LEDControl16BPanel(base ,LightParamStr);
...
{
	LEDControl16BPanel	*H=(LEDControl16BPanel *)handle;
	if(H!=NULL){
		H->Close();
		delete	H;
	}
...
//	In one case, this fuction is called from Sequence.dat
{
	LEDControl16BPanel	*p=static_cast<LEDControl16BPanel *>(handle);
	if(p!=NULL){
