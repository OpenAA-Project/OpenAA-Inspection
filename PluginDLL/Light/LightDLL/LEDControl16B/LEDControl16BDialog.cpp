...
#include "ui_LEDControl16BDialog.h"
...
LineGroup::LineGroup(LEDControl16BDialog *p ,QWidget *parent)
	:Parent(p)
	,Button(false,parent)
{
...
void	LineGroup::AddLine(LEDControl16BPanel::DefUnit::DefBoard::DefLine	*L ,QWidget *parent)
{
	for(int i=0;i<sizeof(LineData)/sizeof(LineData[0]);i++){
		if(LineData[i]==NULL){
...
LEDControl16BDialog::LEDControl16BDialog(LEDControl16BPanel *p,QWidget *parent) :
    QDialog(parent)
	,Panel(p)
    ,ui(new Ui::LEDControl16BDialog)
{
    ui->setupUi(this);
...
LEDControl16BDialog::~LEDControl16BDialog()
{
	for(int k=0;k<LineGroupDataNumb;k++){
...
void LEDControl16BDialog::SetPatternValue(int PatternNo)
{
	for(int k=0;k<LineGroupDataNumb;k++){
		for(int i=0;i<sizeof(LineGroupData[k]->LineData)/sizeof(LineGroupData[k]->LineData[0]);i++){
			if(LineGroupData[k]->LineData[i]==NULL)
				break;
			LEDControl16BPanel::DefUnit::DefBoard::DefLine	*L=LineGroupData[k]->LineData[i]->PointerForDefLine;

			for(int h=0;h<Panel->LightData[PatternNo].Pattern.AllocatedCountOfLines;h++){
				LEDControl16BPanel::DefUnit::DefBoard::DefLine	*D=Panel->LightData[PatternNo].Pattern.LineValueDim[h].PointerForDefLine;
				if(L!=NULL && D!=NULL 
				&& D->BoardNo==L->BoardNo
				&& D->UnitNo==L->UnitNo
				&& D->LineNo==L->LineNo){
...
void LEDControl16BDialog::GetPatternValue(int PatternNo)
{
	for(int k=0;k<LineGroupDataNumb;k++){
		for(int i=0;i<sizeof(LineGroupData[k]->LineData)/sizeof(LineGroupData[k]->LineData[0]);i++){
			if(LineGroupData[k]->LineData[i]==NULL)
...
void	LEDControl16BDialog::SlotClicked(int n)
{
	ui->stackedWidgetSetting->setCurrentIndex(n);
}

QProgressBar	*LEDControl16BDialog::GetBar(void)
{
	return ui->progressBar;
}

void LEDControl16BDialog::on_pushButtonOK_clicked()
{
	GetPatternValue(GetCurrentPatternNo());
	close();
}

void LEDControl16BDialog::on_pushButtonSetAll_clicked()
{
...
void LEDControl16BDialog::on_pushButtonTransfer_clicked()
{
	GetPatternValue(GetCurrentPatternNo());
	Panel->ReflectDataInDialog();
}

void LEDControl16BDialog::on_toolButtonON_clicked()
{
	GetPatternValue(GetCurrentPatternNo());
	Panel->ReflectOnOffInDialog(ui->toolButtonON->isChecked());
}

void LEDControl16BDialog::on_pushButtonDefinition_clicked()
{
...
void LEDControl16BDialog::on_pushButtonWriteEPROM_clicked()
{

}

void LEDControl16BDialog::on_pushButtonShowIO_clicked()
{
	ShowIODialog	D(Panel);
	D.exec();
}

void LEDControl16BDialog::on_pushButtonShowAD_clicked()
{
	ShowADDialog	D(Panel);
	D.exec();
}


void LEDControl16BDialog::on_pushButtonReqVolume_clicked()
{
	int	n=Panel->ReqVolume();
	ui->spinBoxVolume->setValue(n);
}

void LEDControl16BDialog::on_toolButtonManual_clicked()
{
	Panel->SetManual(ui->toolButtonManual->isChecked());
}

void LEDControl16BDialog::on_pushButtonShowSpecial_clicked()
{
