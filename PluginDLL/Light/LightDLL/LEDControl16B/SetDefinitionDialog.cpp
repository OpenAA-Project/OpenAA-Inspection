...
SetDefinitionDialog::SetDefinitionDialog(LEDControl16BPanel *p,QWidget *parent) :
    QDialog(parent)
	,Panel(p)
    ,ui(new Ui::SetDefinitionDialog)
{
	...
	ui->spinBoxBaseCounter	->setValue(Panel->BaseCounter);
...
void SetDefinitionDialog::on_pushButton_clicked()
{
	...
	Panel->BaseCounter	=ui->spinBoxBaseCounter	->value();
	close();
}
