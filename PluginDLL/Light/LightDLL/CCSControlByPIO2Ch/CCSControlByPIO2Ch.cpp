...
{
	CopyrightString="Copyright(c) MEGATRADE 2015.10";
	return true;
}

DEFFUNCEX	const char	*DLL_GetExplain(void)
{
	return "CCS Control 2Ch by PIO";
}
...
{
	LightCCSControlByPIO2Ch	*H=new LightCCSControlByPIO2Ch(base,LightParamStr);
	H->Initial();
	return H;
}
...
{
	LightCCSControlByPIO2Ch	*H=(LightCCSControlByPIO2Ch *)handle;
	if(H!=NULL){
		H->Close();
		delete	H;
	}
...
//	if process fails, it returns false
{
	LightCCSControlByPIO2Ch	*p=static_cast<LightCCSControlByPIO2Ch *>(handle);
	bool	Ret=p->Load(&str);
	if(Ret==true){
		p->CurrentPattern=0;
