...
{
	LEDControl16GPanel	*H=new LEDControl16GPanel(base ,LightParamStr);
...
{
	LEDControl16GPanel	*H=(LEDControl16GPanel *)handle;
	if(H!=NULL){
		H->Close();
		delete	H;
	}
...
//	In one case, this fuction is called from Sequence.dat
{
	LEDControl16GPanel	*p=static_cast<LEDControl16GPanel *>(handle);
	if(p!=NULL){
