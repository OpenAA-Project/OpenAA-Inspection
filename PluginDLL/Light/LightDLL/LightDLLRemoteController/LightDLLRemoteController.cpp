#include "LightDLLRemoteController.h"
#include "XTypeDef.h"
#include "XDLLType.h"
...
{
	CopyrightString="Copyright(c) MEGATRADE 2014.11";
	return true;
}

DEFFUNCEX	const char	*DLL_GetExplain(void)
{
	return "Light for RemoteController";
}
...
{
	XLightDLLRemoteController	*H=new XLightDLLRemoteController(base ,LightParamStr);
	H->InitialPhysical(LightParamStr);
	if(H->ReflectPhysicalTurnOn(OutputMode)==false){
		QMessageBox::warning(NULL,"Error"
								,"Connection is not established. Please restart LightController");
	}
	return H;
}
...
{
	XLightDLLRemoteController *H=(XLightDLLRemoteController *)handle;
	if(H!=NULL){
		delete	H;
	}
...
//	In one case, this fuction is called from Sequence.dat
{
	XLightDLLRemoteController	*p=static_cast<XLightDLLRemoteController *>(handle);
	if(p!=NULL){
		p->ReflectPhysical();
	}
	return true;
