...
{
	LEDControl16APanel	*H=new LEDControl16APanel(base ,LightParamStr);
...
{
	LEDControl16APanel	*H=(LEDControl16APanel *)handle;
	if(H!=NULL){
		H->Close();
		delete	H;
	}
...
//	In one case, this fuction is called from Sequence.dat
{
	LEDControl16APanel	*p=static_cast<LEDControl16APanel *>(handle);
	if(p!=NULL){
