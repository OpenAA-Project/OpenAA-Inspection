    ...
    ui(new Ui::MainWindow)
{
    ...
    BoardData[Brd].LightPatternData[0].LineData[15].ON=true;
    WriteEOnOffData(DipSW ,Brd,0);
    BoardData[Brd].CmdSetLineOn=true;
    WriteEForBit(DipSW ,Brd);
    BoardData[Brd].CmdSetLineOn=false;
...
void    MainWindow::ReceiveData(struct  LightPacketData &data)
{
...
void MainWindow::on_comboBoxBoard_currentIndexChanged(int index)
{
    ShowCurrentData();
}

void MainWindow::on_spinBoxPattern_valueChanged(int arg1)
{
    ShowCurrentData();
}
...
int MainWindow::GetDipSW_InE(int boardnumber)
{
    return DipSWTable[boardnumber];
}

void MainWindow::ShowCurrentData(void)
{
