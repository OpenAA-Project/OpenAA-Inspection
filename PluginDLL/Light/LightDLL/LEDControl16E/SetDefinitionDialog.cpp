...
SetDefinitionDialog::SetDefinitionDialog(LEDControl16EPanel *p,QWidget *parent) :
    QDialog(parent)
	,Panel(p)
	...
	for(int r=0;r<Panel->CountOfUnit;r++){
		for(int b=0;b<Panel->DefUnitDim[r].CountOfBoard;b++){
			for(int L=0;L<Panel->DefUnitDim[r].DefBoardDim[b].CountOfLine;L++,Row++){
				...
				Panel->DefUnitDim[r].DefBoardDim[b].DefLineDim[L].BoardNo	=b;
				Panel->DefUnitDim[r].DefBoardDim[b].DefLineDim[L].LineNo	=L;
			}
		}
	}
	close();
