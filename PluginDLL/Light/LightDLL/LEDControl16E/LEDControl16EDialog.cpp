...
#include "ui_LEDControl16EDialog.h"
...
LineGroup::LineGroup(LEDControl16EDialog *p ,QWidget *parent)
	:Parent(p)
	,Button(false,parent)
{
...
void	LineGroup::AddLine(LEDControl16EPanel::DefUnit::DefBoard::DefLine	*L ,QWidget *parent)
{
	for(int i=0;i<sizeof(LineData)/sizeof(LineData[0]);i++){
		if(LineData[i]==NULL){
...
LEDControl16EDialog::LEDControl16EDialog(LEDControl16EPanel *p,QWidget *parent) :
    QDialog(parent)
	,Panel(p)
    ,ui(new Ui::LEDControl16EDialog)
{
    ui->setupUi(this);
	ui->lineEditVersion->setText(Panel->VersionStr);
	ui->pushButtonShowIO		->setVisible(false);
	ui->pushButtonShowSpecial	->setVisible(false);
	ui->pushButtonIODeliverer	->setVisible(false);
...
LEDControl16EDialog::~LEDControl16EDialog()
{
	for(int k=0;k<LineGroupDataNumb;k++){
...
void LEDControl16EDialog::SetPatternValue(int PatternNo)
{
	for(int k=0;k<LineGroupDataNumb;k++){
		for(int i=0;i<sizeof(LineGroupData[k]->LineData)/sizeof(LineGroupData[k]->LineData[0]);i++){
			if(LineGroupData[k]->LineData[i]==NULL)
				break;
			LEDControl16EPanel::DefUnit::DefBoard::DefLine	*L=LineGroupData[k]->LineData[i]->PointerForDefLine;

			for(int h=0;h<Panel->LightData[PatternNo].Pattern.AllocatedCountOfLines;h++){
				LEDControl16EPanel::DefUnit::DefBoard::DefLine	*D=Panel->LightData[PatternNo].Pattern.LineValueDim[h].PointerForDefLine;
				if(L!=NULL && D!=NULL 
				&& D->BoardNo==L->BoardNo
				&& D->UnitNo==L->UnitNo
				&& D->LineNo==L->LineNo){
...
void LEDControl16EDialog::GetPatternValue(int PatternNo)
{
	for(int k=0;k<LineGroupDataNumb;k++){
		for(int i=0;i<sizeof(LineGroupData[k]->LineData)/sizeof(LineGroupData[k]->LineData[0]);i++){
			if(LineGroupData[k]->LineData[i]==NULL)
...
void	LEDControl16EDialog::SlotClicked(int n)
{
	ui->stackedWidgetSetting->setCurrentIndex(n);
}

QProgressBar	*LEDControl16EDialog::GetBar(void)
{
	return ui->progressBar;
}

void LEDControl16EDialog::on_pushButtonOK_clicked()
{
	GetPatternValue(GetCurrentPatternNo());
	close();
}

void LEDControl16EDialog::on_pushButtonSetAll_clicked()
{
...
void LEDControl16EDialog::on_pushButtonTransfer_clicked()
{
	GetPatternValue(GetCurrentPatternNo());
	Panel->ReflectDataInDialog();
}

void LEDControl16EDialog::on_toolButtonON_clicked()
{
	GetPatternValue(GetCurrentPatternNo());
	Panel->ReflectOnOffInDialog(ui->toolButtonON->isChecked());
}

void LEDControl16EDialog::on_pushButtonDefinition_clicked()
{
...
void LEDControl16EDialog::on_pushButtonShowIO_clicked()
{
...
void LEDControl16EDialog::on_pushButtonShowSpecial_clicked()
{
...
void LEDControl16EDialog::on_pushButtonIODeliverer_clicked()
{
