...
TestStencilDialog::TestStencilDialog(LEDControl16EPanel *p,QWidget *parent) :
    QDialog(parent)
	,Panel(p)
...
void TestStencilDialog::showEvent ( QShowEvent * event )
{
	Panel->SetEncoderMode(false);
}

void TestStencilDialog::closeEvent ( QCloseEvent * event )
{
	Panel->SetEncoderMode(true);
}
