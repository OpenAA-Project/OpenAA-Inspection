...
{
	CopyrightString="Copyright(c) MEGATRADE 2018.02";
	return true;
}

DEFFUNCEX	const char	*DLL_GetExplain(void)
{
	return "LEDControl16E";
}
...
{
	LEDControl16EPanel	*H=new LEDControl16EPanel(base ,LightParamStr);
...
{
	LEDControl16EPanel	*H=(LEDControl16EPanel *)handle;
	if(H!=NULL){
		H->Close();
		delete	H;
	}
...
//	In one case, this fuction is called from Sequence.dat
{
	LEDControl16EPanel	*p=static_cast<LEDControl16EPanel *>(handle);
	if(p!=NULL){
