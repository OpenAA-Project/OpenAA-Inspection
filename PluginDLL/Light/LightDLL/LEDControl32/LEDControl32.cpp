...
{
	LEDControl32	*H=new LEDControl32(base ,LightParamStr);
...
{
	LEDControl32	*H=(LEDControl32 *)handle;
	if(H!=NULL){
		H->Close();
		delete	H;
	}
...
//	In one case, this fuction is called from Sequence.dat
{
	LEDControl32	*p=static_cast<LEDControl32 *>(handle);
	if(p!=NULL){
		p->OutputNumber=PatternNo;
		p->CommSetOutputNumber(p->OutputNumber);
	}
	return true;
}
