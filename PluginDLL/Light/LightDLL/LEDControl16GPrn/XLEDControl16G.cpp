...
QString	LEDControl16GPanel::DefFileName		=/**/"LEDControl16GPrnDef.dat";
QString	LEDControl16GPanel::DefaultFileName	=/**/"LEDControl16GPrnDefault.dat";
...
bool	LEDControl16GPanel::SaveDefault(void)
{
...
bool	LEDControl16GPanel::LoadDefault(void)
{
	QFile	DFile(DefaultFileName);
...
bool	LEDControl16GPanel::ShowSettingDialog(void)
{
	...
	GIOForm=new LEDControl16GPrnDialog(this);
...
bool	LEDControl16GPanel::WriteRegister(BYTE DipSW,BYTE Cmd ,BYTE Data[] ,unsigned short DataLen)
{
	static	bool	ReEntrant=false;
...
bool	LEDControl16GPanel::ReadRegister (BYTE DipSW,BYTE Cmd ,BYTE Data[] ,unsigned short SendDataLen
...
bool	LEDControl16GPanel::SendGio(void)
{
	BYTE	Data[100];
	memcpy(Data,&GIOFixedData,sizeof(GIOFixedData));
	int	Len=sizeof(GIOFixedData);
...
bool	LEDControl16GPanel::RequireGio(void)
{
	BYTE	Data[100];
	int ReceivedByte;
	BYTE	RecCmd;
	ReadRegister (-1,'B',Data ,1,RecCmd,ReceivedByte ,sizeof(Data));
	if(RecCmd=='B' && ReceivedByte==sizeof(GIOFixedData)){
		memcpy(&GIOFixedData,Data,sizeof(GIOFixedData));
		return true;
	}
	return false;
}
void	LEDControl16GPanel::ShowTimingChart(void)
{
	if(TimingChart!=NULL){
		TimingChart->show();
	}
...
int	LEDControl16GPanel::FindTiming(DWORD st)	//stÇÊÇËëÂÇ´Ç¢ç≈èâÇÃóvëfÇãÅÇﬂÇÈ
{
...
struct  TimingData	*LEDControl16GPanel::GetTiming(int point)
{
