...
TitlePanelWidget::TitlePanelWidget(TimingSignalPort *parent)
	:LabelPortType(this),LabelPortName(this),Parent(parent)
{
}
...
void	TitlePanelWidget::Initial(void)
{
	...
	LabelPortType.setText(Parent->Parent->Panel->GIOPortName.GetTypeName(Parent->PortType,Parent->BitNo));
	LabelPortName.setText(Parent->Parent->Panel->GIOPortName.GetString(Parent->PortType,Parent->BitNo));
}
...
void SignalPanelWidget::paintEvent(QPaintEvent *event)
{
	...
	if(0<k){
		...
		CurrentData=Parent->Parent->Panel->GIOPortName.GetSignal(Parent->PortType,Parent->BitNo,*a);
	}
	if(0<=k){
		int	LastX=0;
		Pnt.setPen(Qt::red);
		...
		for(int x=0;x<XLen;x++){
			...
			if(NextP!=NULL){
				...
				while(s>=NextP->CurrentMiliSec){
					bool	b=Parent->Parent->Panel->GIOPortName.GetSignal(Parent->PortType,Parent->BitNo,*NextP);
					if(CurrentData!=b){
						Both=true;
					}
					LastData=b;
...
TimingChartForm::TimingChartForm(LEDControl16GPanel *p,QWidget *parent) :
    QWidget(parent),
