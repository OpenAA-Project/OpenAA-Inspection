...
void    TitlePanelWidget::ShowTimeLine(DWORD TimeLine)
{
    ...
    if(t!=NULL){
        bool    b=Parent->Parent->Panel->GIOPortName.GetSignal(Parent->PortType,Parent->BitNo ,*t);
...
void SignalPanelWidget::paintEvent(QPaintEvent *event)
{
    ...
    bool	LastB=Parent->Parent->Panel->GIOPortName.GetSignal(Parent->PortType,Parent->BitNo,*LastT);
    int     LastX=0;
...
TimingChartForm::TimingChartForm(LEDControl16GPrnRasPi *p,QWidget *parent) :
