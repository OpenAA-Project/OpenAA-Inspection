...
{
	LEDControl16HioPanel	*H=new LEDControl16HioPanel(base ,LightParamStr);
...
{
	LEDControl16HioPanel	*H=(LEDControl16HioPanel *)handle;
	if(H!=NULL){
		H->Close();
		delete	H;
	}
...
//	In one case, this fuction is called from Sequence.dat
{
	LEDControl16HioPanel	*p=static_cast<LEDControl16HioPanel *>(handle);
	if(p!=NULL){
