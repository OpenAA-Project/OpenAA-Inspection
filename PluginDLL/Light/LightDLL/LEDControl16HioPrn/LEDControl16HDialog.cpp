...
#include "ui_LEDControl16HDialog.h"
#include "XTypeDef.h"
...
LineGroup::LineGroup(LEDControl16HDialog *p ,QWidget *parent)
...
LEDControl16HDialog::LEDControl16HDialog(LEDControl16HioPanel *p,QWidget *parent) :
    QDialog(parent)
	,Panel(p)
	,ui(new Ui::LEDControl16HDialog)
{
    ui->setupUi(this);
...
LEDControl16HDialog::~LEDControl16HDialog()
{
	for(int k=0;k<LineGroupDataNumb;k++){
...
void LEDControl16HDialog::SetPatternValue(int PatternNo)
{
	for(int k=0;k<LineGroupDataNumb;k++){
		for(int i=0;i<LineGroupData[k]->LineNumb;i++){
			if(LineGroupData[k]->LineData[i]==NULL)
...
void LEDControl16HDialog::GetPatternValue(int PatternNo)
{
	for(int k=0;k<LineGroupDataNumb;k++){
		for(int i=0;i<LineGroupData[k]->LineNumb;i++){
			if(LineGroupData[k]->LineData[i]==NULL)
...
void	LEDControl16HDialog::SlotClicked(int n)
{
	ui->stackedWidgetSetting->setCurrentIndex(n);
}

QProgressBar	*LEDControl16HDialog::GetBar(void)
{
	return ui->progressBar;
}

void LEDControl16HDialog::on_pushButtonOK_clicked()
{
	GetPatternValue(GetCurrentPatternNo());
	close();
}

void LEDControl16HDialog::on_pushButtonSetAll_clicked()
{
...
void LEDControl16HDialog::on_pushButtonSetTestLED_clicked()
{
...
void LEDControl16HDialog::on_pushButtonTransfer_clicked()
{
	GetPatternValue(GetCurrentPatternNo());
	Panel->ReflectDataInDialog();
}

void LEDControl16HDialog::on_toolButtonON_clicked()
{
	GetPatternValue(GetCurrentPatternNo());
	Panel->ReflectOnOffInDialog(ui->toolButtonON->isChecked());
}

void LEDControl16HDialog::on_pushButtonDefinition_clicked()
{
...
void LEDControl16HDialog::on_pushButtonShowIO_clicked()
{
...
void LEDControl16HDialog::on_pushButtonShowSpecial_clicked()
{
...
void LEDControl16HDialog::on_pushButtonIODeliverer_clicked()
{
