...
SettingDialog::SettingDialog(ImacControlByLAN *p ,LayersBase *base ,QWidget *parent) :
    QDialog(parent),ServiceForLayers(base),
    ui(new Ui::SettingDialog),Panel(p)
{
    ui->setupUi(this);
	CurrentChannel	=0;
	Data=new ImacControlByLANParam();
...
void	SettingDialog::LoadFromWindow(int currentChannel)
{
	...
	Data->ChannelInfoData[currentChannel].Delay		=ui->spinBoxDelay		->value();
	Data->ChannelInfoData[currentChannel].OutputTime=ui->spinBoxOutputTime	->value();
}
void	SettingDialog::SetToWindow(int NewChannel)
{
	...
	ui->spinBoxDelay		->setValue	(Data->ChannelInfoData[NewChannel].Delay);
	ui->spinBoxOutputTime	->setValue	(Data->ChannelInfoData[NewChannel].OutputTime);
}
