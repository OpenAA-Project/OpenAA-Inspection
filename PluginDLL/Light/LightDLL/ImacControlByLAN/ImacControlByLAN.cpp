...
{
	ImacControlByLAN	*H=new ImacControlByLAN(base,LightParamStr);
	if(H->Initial()==false){
		QMessageBox::warning(NULL,"エラー"
							 ,"Leimac 照明コントローラーと通信できません");
		return NULL;
	}
	return H;
}
...
{
	ImacControlByLAN	*H=dynamic_cast<ImacControlByLAN *>(handle);
	if(H!=NULL){
		H->Close();
		delete	H;
	}
...
//	if process fails, it returns false
{
	ImacControlByLAN	*p=static_cast<ImacControlByLAN *>(handle);
	bool	Ret=p->Load(&str);
	if(Ret==true){
		p->Transfer(0,NULL);
	}
