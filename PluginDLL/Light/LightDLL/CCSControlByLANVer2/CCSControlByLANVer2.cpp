...
{
	CopyrightString="Copyright(c) MEGATRADE 2024.9";
	return true;
}

DEFFUNCEX	const char	*DLL_GetExplain(void)
{
	return "CCS Control by LAN Ver2";
}
...
{
	CCSControlByLANVer2	*H=new CCSControlByLANVer2(base,LightParamStr);
	if(H->Initial()==false)
		return NULL;
	return H;
}
...
{
	CCSControlByLANVer2	*H=dynamic_cast<CCSControlByLANVer2	*>(handle);
	if(H!=NULL){
		H->Close();
		delete	H;
	}
...
//	if process fails, it returns false
{
	CCSControlByLANVer2	*p=static_cast<CCSControlByLANVer2 *>(handle);
	bool	Ret=p->Load(&str);
	if(Ret==true){
		p->Transfer(0,NULL);
	}
