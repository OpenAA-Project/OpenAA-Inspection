...
#include "ui_LEDControl16CDialog.h"
...
LineGroup::LineGroup(LEDControl16CDialog *p ,QWidget *parent)
...
void	LineGroup::AddLine(LEDControl16CPanel::DefUnit::DefBoard::DefLine	*L ,QWidget *parent)
{
	for(int i=0;i<sizeof(LineData)/sizeof(LineData[0]);i++){
		if(LineData[i]==NULL){
...
LEDControl16CDialog::LEDControl16CDialog(LEDControl16CPanel *p,QWidget *parent) :
    QDialog(parent)
	,Panel(p)
    ,ui(new Ui::LEDControl16CDialog)
{
    ui->setupUi(this);
...
LEDControl16CDialog::~LEDControl16CDialog()
{
	for(int k=0;k<LineGroupDataNumb;k++){
...
void LEDControl16CDialog::SetPatternValue(int PatternNo)
{
	for(int k=0;k<LineGroupDataNumb;k++){
		for(int i=0;i<sizeof(LineGroupData[k]->LineData)/sizeof(LineGroupData[k]->LineData[0]);i++){
			if(LineGroupData[k]->LineData[i]==NULL)
				break;
			LEDControl16CPanel::DefUnit::DefBoard::DefLine	*L=LineGroupData[k]->LineData[i]->PointerForDefLine;

			for(int h=0;h<Panel->LightData[PatternNo].Pattern.AllocatedCountOfLines;h++){
				LEDControl16CPanel::DefUnit::DefBoard::DefLine	*D=Panel->LightData[PatternNo].Pattern.LineValueDim[h].PointerForDefLine;
				if(L!=NULL && D!=NULL 
				&& D->BoardNo==L->BoardNo
				&& D->UnitNo==L->UnitNo
				&& D->LineNo==L->LineNo){
...
void LEDControl16CDialog::GetPatternValue(int PatternNo)
{
	for(int k=0;k<LineGroupDataNumb;k++){
		for(int i=0;i<sizeof(LineGroupData[k]->LineData)/sizeof(LineGroupData[k]->LineData[0]);i++){
			if(LineGroupData[k]->LineData[i]==NULL)
...
void	LEDControl16CDialog::SlotClicked(int n)
{
	ui->stackedWidgetSetting->setCurrentIndex(n);
}

QProgressBar	*LEDControl16CDialog::GetBar(void)
{
	return ui->progressBar;
}

void LEDControl16CDialog::on_pushButtonOK_clicked()
{
	GetPatternValue(GetCurrentPatternNo());
	close();
}

void LEDControl16CDialog::on_pushButtonSetAll_clicked()
{
...
void LEDControl16CDialog::on_pushButtonTransfer_clicked()
{
	GetPatternValue(GetCurrentPatternNo());
	Panel->ReflectDataInDialog();
}

void LEDControl16CDialog::on_toolButtonON_clicked()
{
	GetPatternValue(GetCurrentPatternNo());
	Panel->ReflectOnOffInDialog(ui->toolButtonON->isChecked());
}

void LEDControl16CDialog::on_pushButtonDefinition_clicked()
{
...
void LEDControl16CDialog::on_pushButtonShowIO_clicked()
{
	ShowIODialog	D(Panel);
	D.exec();
}

void LEDControl16CDialog::on_pushButtonShowAD_clicked()
{
	ShowADDialog	D(Panel);
	D.exec();
}

void LEDControl16CDialog::on_pushButtonShowSpecial_clicked()
{
...
void LEDControl16CDialog::on_pushButtonIODeliverer_clicked()
{
