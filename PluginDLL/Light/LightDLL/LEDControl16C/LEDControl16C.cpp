...
{
	LEDControl16CPanel	*H=new LEDControl16CPanel(base ,LightParamStr);
...
{
	LEDControl16CPanel	*H=(LEDControl16CPanel *)handle;
	if(H!=NULL){
		H->Close();
		delete	H;
	}
...
//	In one case, this fuction is called from Sequence.dat
{
	LEDControl16CPanel	*p=static_cast<LEDControl16CPanel *>(handle);
	if(p!=NULL){
