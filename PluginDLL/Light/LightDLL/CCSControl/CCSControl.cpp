...
{
	LightCCSControl	*H=new LightCCSControl(base,LightParamStr);
...
{
	LightCCSControl *H=(LightCCSControl *)handle;
	if(H!=NULL){
		H->Close();
		delete	H;
	}
...
{
	return handle->ReflectPhysicalTurnOn(true);
}
...
//	In one case, this fuction is called from Sequence.dat
{
	return handle->ReflectPhysicalTurnOn(false);
}
