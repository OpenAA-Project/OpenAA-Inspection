...
void	mtLineGraph::paintEvent(QPaintEvent *event)
{
	...
	if(!OnSTypeFlag){
		if(STypeX==mtConstant){
			...
			uWidth=wuWidth=uGraphMaxX-uGraphMinX;	//�f�[�^���W��X���̈敝
			if(uWidth<=0.0){
				int AAA=0;
			}
		...
		if(STypeY==mtConstant){
			...
			uHeight=wuHeight=uGraphMaxY-uGraphMinY;		//�f�[�^���W��Y���̈敝
			if(uHeight<=0.0){}
			...
			double	nH=uGraphMaxY-uGraphMinY;		//�f�[�^���W��Y���̈敝
			if(nH>0.0){
				uHeight=wuHeight=nH;
	...
	if(STypeX==mtConstant){
		...
		for(int XXX=0;XXX<=WLen;XXX++){
			...
			if(GraphList.at(0)!=NULL){
				...
				Pnt.drawLine(gMeasureX,height()-OffsetY,gMeasureX,0);	//���؂��Ԗڐ�
			}
		}
	}
	else if(STypeX==mtLog){
	...
	if(STypeY==mtConstant){
		...
		for(int YYY=0;YYY<=HLen;YYY++){
			...
			if(GraphList.at(0)!=NULL){
				...
				Pnt.drawLine(OffsetX,gMeasureY,width(),gMeasureY);	//���؂��Ԗڐ�
			}
		}
	}
	else if(STypeY==mtLog){
...
//	Pnt.setBrush(QBrush(QColor(Qt::lightGray)));
	...
	if(STypeX==mtConstant){
		...
		for(int XXX=0;XXX<=WLen;XXX++){
			...
			if(GraphList.at(0)!=NULL){
				...
				//Pnt.drawText(0,height()-(OffsetY>>1),gMeasureX-1,OffsetY>>1,Qt::AlignRight,QString().setNum(uMeasureX));	//�ڐ��萔�l
				Pnt.drawText(0,height()+CharY,gMeasureX-1,CharacterHeight,Qt::AlignRight,QString().setNum(uMeasureX));	//�ڐ��萔�l
			}
		}
	}
	else if(STypeX==mtLog){
		Pnt.drawText(0,height()-(OffsetY>>1),OffsetX-1,OffsetY>>1,Qt::AlignRight,QString().setNum(0));	//�ڐ��萔�l
		for(int XXX=0;XXX<uLogXDbl;XXX++){
			if(GraphList.at(0)!=NULL){
	...
	if(STypeY==mtConstant){
		...
		for(int YYY=0;YYY<=HLen;YYY++){
			...
			if(GraphList.at(0)!=NULL){
				...
				if(LastPosY-CharacterHeight>=gMeasureY){
					Pnt.drawText(1,gMeasureY+1,OffsetX,height(),Qt::AlignLeft,QString().setNum(uMeasureY));	//�ڐ��萔�l
					LastPosY=gMeasureY;
				}
			}
		}
		...
		Pnt.drawText(1,height()-OffsetY+1,OffsetX,height(),Qt::AlignLeft,QString().setNum(0));	//�ڐ��萔�l
		int	LastPosY=height()+CharacterHeight;
		for(int YYY=0;YYY<uLogYDbl;YYY++){
			if(GraphList.at(0)!=NULL){
...
//�f�[�^���W��X���W(ux)�����ʍ��W��X���W(gx)�ɕϊ������i�ΐ��\���j
void	mtLineGraph::GetCanvasToUniverseLogX(double gx,int &ux)
{
	ux=pow(10.0,(uLogXDbl*(gx-OffsetX)/(width()-OffsetX))-1);
}

//�f�[�^���W��Y���W(uy)�����ʍ��W��Y���W(gy)�ɕϊ������i�ΐ��\���j
