	...
	if(ExpandedPasteBtn!=NULL && ExpandedPasteBtn->isChecked()==true){
		...
		if(ExpandedPasteStart.SelectedItemLists.GetCount()==0){
			QMessageBox::warning(NULL,/**/"Alarm"
									,LangDISolver.GetString(XDisplayImage_LS,LID_192)/*"Selected items are necessary"*/);
			return;
		}
...
void	DisplayImage::BroadcastShiftAll(void)
{
	if(ReEntrant==true){
		return;
	}
	...
	if(MainCanvas!=NULL){
		...
		for(int i=0;i<RelatedItems.count();i++){
			...
			if(r!=NULL){
				DisplayImage	*Img=dynamic_cast<DisplayImage *>(r);
				if(Img!=NULL){
					Img->SetDrawPosition(rect.left(),rect.top());
				}
				else{
					r->Repaint();
				}
...
void	DisplayImage::ExeCopyRect(FlexArea &Area)
{
	...
	if(0<=ShowOnePage && ShowOnePage<GetPageNumb()){
		ExeCopyRect(Area ,ShowOnePage);
	}
	else{
		for(int page=0;page<GetLayersBase()->GetPageNumb();page++){
			ExeCopyRect(Area ,page);
		}
	}
	if(MainCanvas!=NULL){
		MainCanvas->Clear();
...
void	DisplayImage::ExeCopyRect(FlexArea &Area ,int page)
{
	FlexArea	area;
	area=Area;
...
void	DisplayImage::ExeDrawRect(FlexArea &Area)
{
	...
	if(0<=ShowOnePage && ShowOnePage<GetPageNumb()){
		ExeDrawRect(Area ,ShowOnePage);
	}
	else{
		for(int page=0;page<GetLayersBase()->GetPageNumb();page++){
			ExeDrawRect(Area ,page);
		}
	}
	if(MainCanvas!=NULL){
		MainCanvas->Clear();
...
void	DisplayImage::ExeDrawRect(FlexArea &Area ,int page)
{
	FlexArea	MArea;
	MArea=Area;
...
{
	if(0<=ShowOnePage && ShowOnePage<GetPageNumb()){
		ExeDrawDot(ShowOnePage ,globalX,globalY);
	}
	else{
		for(int page=0;page<GetLayersBase()->GetPageNumb();page++){
			ExeDrawDot(page ,globalX,globalY);
		}
	}
	if(MainCanvas!=NULL){
		MainCanvas->Clear();
...
void	DisplayImage::ExeDrawDot(int page ,int globalX,int globalY)
{
	int localX ,localY;
...
void	DisplayImage::SetDrawPosition(int datax1 ,int datay1)
{
	if(MainCanvas!=NULL){
		MainCanvas->ZoomDraw(-datax1, -datay1, GetZoomRate());
		emit	SignalChangedDrawingPosition();
	}
}
...
{
	int	GlobalPage	=(ShowOnePage<0 || GetPageNumb()<=ShowOnePage)?GetLayersBase()->GetGlobalPage(universalX,universalY):ShowOnePage;
	int	LocalPage	=GetParamComm()->GetLocalPageFromGlobal	(*GetParamGlobal(),GlobalPage);
	if(GlobalPage>=0 && LocalPage>=0 && LocalPage<GetPageNumb()){
...
void	DisplayImage::SetFModeRepaintOnMouseMove(bool b)
{
	if(GetCanvas()!=NULL){
		GetCanvas()->SetFModeRepaintOnMouseMove(b);
	}
	FModeRepaintOnMouseMove=b;
}
...
void	DisplayImage::ExePasteImage(int dx,int dy)
{
	...
	if(0<=ShowOnePage && ShowOnePage<GetPageNumb()){
		ExePasteImage(ShowOnePage ,dx,dy);
	}
	else{
		for(int page=0;page<GetPageNumb();page++){
			ExePasteImage(page ,dx,dy);
		}
	}
}
void	DisplayImage::ExePasteImage(int page ,int dx,int dy)
{
	int	GlobalPage=GetLayersBase()->GetGlobalPageFromLocal(page);
...
void	DisplayImage::ExePickupColor(int globalX,int globalY)
{
	int	GlobalPage=-1;
	if(0<=GetShowOnePage() && GetShowOnePage()<GetPageNumb()){
		GlobalPage=GetLayersBase()->GetGlobalPageFromLocal(GetShowOnePage());
	}
	else{
		GlobalPage=GetLayersBase()->GetGlobalPage(globalX,globalY);
	}
	if(GlobalPage>=0){
		AllocInnerBuff();
...
void	DisplayImage::ToolMenuBtnDown()
{
	...
	&& GetParamGlobal()->AllocateTargetBuff==true){
		Menu.addAction ("Copy Target -> BackGround"	, this, SLOT(SlotCopyImageToBackGround()));
	}
	...
	if(GetParamGlobal()->AllocateBackGroundBuff==true){
		Menu.addAction (LangDISolver.GetString(XDisplayImage_LS,LID_178)/*"Clear Master Image"*/	, this, SLOT(SlotClearBackGroundImage()));
	}
...
void	DisplayImage::SlotClearBackGroundImage()
{
	for(int page=0;page<GetLayersBase()->GetPageNumb();page++){
		int	GlobalPage=GetLayersBase()->GetGlobalPageFromLocal(page);
		GUICmdClearImage	Cmd(GetLayersBase(),EmitterRoot,EmitterName ,GlobalPage);
		...
		Cmd.Color=Qt::black;
		if(Cmd.SendOnly(GlobalPage,0)==false){
			SetError(Error_Comm , /**/"Send error :ClearBackGroundImage",ErrorCodeList::_Alart);
		}
	}
	BroadcastShowInEdit();
}
...
void	DisplayImage::SlotCopyImageToBackGround()
{
	for(int page=0;page<GetLayersBase()->GetPageNumb();page++){
		int	GlobalPage=GetLayersBase()->GetGlobalPageFromLocal(page);
		GUICmdCopyImageToBackGround	Cmd(GetLayersBase(),EmitterRoot,EmitterName ,GlobalPage);
		if(Cmd.SendOnly(GlobalPage,0)==false){
			SetError(Error_Comm , /**/"Send error :SlotCopyImageToBackGround",ErrorCodeList::_Alart);
		}
	}
	//Repaint();
	BroadcastShowInEdit();
...
{
	QMenu	Menu(this);
	Menu.addAction (LangDISolver.GetString(XDisplayImage_LS,LID_204)/*"Rectangle"*/			, this, SLOT(SlotSelectRectangle()));
	Menu.addAction ("Line"				, this, SLOT(SlotSelectLine()));
	Menu.addAction (LangDISolver.GetString(XDisplayImage_LS,LID_205)/*"Ellipse-Center"*/	, this, SLOT(SlotSelectEllipseCenter()));
	Menu.addAction (LangDISolver.GetString(XDisplayImage_LS,LID_206)/*"Ellipse-4Points"*/	, this, SLOT(SlotSelectEllipse4Points()));
	Menu.addAction (LangDISolver.GetString(XDisplayImage_LS,LID_207)/*"Long hole"*/			, this, SLOT(SlotSelectLongCircle()));
	Menu.addAction (LangDISolver.GetString(XDisplayImage_LS,LID_208)/*"Rotated rect"*/		, this, SLOT(SlotSelectRotatedRect()));
	Menu.addAction (LangDISolver.GetString(XDisplayImage_LS,LID_209)/*"Ring"*/				, this, SLOT(SlotSelectRing()));
	Menu.addAction (LangDISolver.GetString(XDisplayImage_LS,LID_210)/*"Polygon"*/			, this, SLOT(SlotSelectPolygon()));
	Menu.addAction (LangDISolver.GetString(XDisplayImage_LS,LID_211)/*"Free hand"*/			, this, SLOT(SlotSelectFreeHand()));
	Menu.addAction (LangDISolver.GetString(XDisplayImage_LS,LID_212)/*"Pour"*/				, this, SLOT(SlotSelectPour()));
	Menu.addAction (LangDISolver.GetString(XDisplayImage_LS,LID_213)/*"Replace color"*/		, this, SLOT(SlotSelectReplaceColor()));
...
void	DisplayImage::SlotSelectLine()
{
...
{
	CmdReqImageLayer	DrawMode(GetLayersBase(),GetLayersBase()->GetMaxLayerNumb());
	...
	for(int Layer=0;Layer<GetLayersBase()->GetLayerNumb(0);Layer++){
		LayerList.Add(Layer);
	}
}
...
double	DisplayImage::GetScaleLen(double RealLen)
{
	...
	if(Dp!=NULL){
		...
		if(ExistRegulation==false){
			return GetParamGlobal()->TransformUnitDistanceToPixel(ShowOnePage,0 ,0  ,RealLen ,0)*GetZoomRate();
		}
