...
void ShowColorDifferenceResultListForm::on_pushButtonAddMeasure_clicked()
{
	int N = CurrentResults.GetCount();
	for(int row=0;row<N;row++){
		ColorDifferenceResultInfo	*R=CurrentResults.GetItem(row);
...
void	GUICmdReqColorDifferenceResult::Receive(int32 localPage, int32 cmd ,QString &EmitterRoot,QString &EmitterName)
{
	...
	ColorDifferenceBase *RBBase=(ColorDifferenceBase *)GetLayersBase()->GetAlgorithmBase(/**/"Basic",/**/"ColorDifference");
	if(RBBase!=NULL){
		ColorDifferenceInPage	*PData=dynamic_cast<ColorDifferenceInPage	*>(RBBase->GetPageData(localPage));
		if(PData!=NULL){
