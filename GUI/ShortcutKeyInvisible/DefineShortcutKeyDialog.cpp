...
#include "ui_DefineShortcutKeyDialog.h"
#include "ShortcutKeyInvisible.h"
#include "XGeneralFunc.h"
#include "SearchControlDialog.h"
#include <QComboBox>

QString	ConvertKeyCodeToString(DWORD KeyCode);


CellInComponentControl::CellInComponentControl(DefineShortcutKeyDialog *p ,int row)
{
	Parent=p;
	Row=row;
...
void	CellInComponentControl::Set( const QString &_DLLRoot
									,const QString &_DLLName
									,const QString &_DLLInst
									,const QString &_ObjPath
									,const QString &_ObjName)
{
	...
	ObjPath	=_ObjPath;
	ObjName	=_ObjName;
	setText(DLLRoot + QString(/**/"-") +DLLName + QString(/**/"-") +ObjName + QString(/**/"-") +ObjPath);
}
	
void	CellInComponentControl::SlotClicked()
{
	SearchControlDialog	E(Parent->GetLayersBase());
	if(E.exec()==(int)true){
		Set(E.DLLRoot
			,E.DLLName
			,E.DLLInst
...
DefineShortcutKeyDialog::DefineShortcutKeyDialog(struct	ShortcutKeyInfoList	 _ShortcutKeyDim[] , int _CountOfShortcut ,LayersBase *base,QWidget *parent) :
    QDialog(parent),ServiceForLayers(base)
    ,ui(new Ui::DefineShortcutKeyDialog)
{
    ui->setupUi(this);
	...
	ShortcutKeyDim	=_ShortcutKeyDim;
	CountOfShortcut	=_CountOfShortcut;

	ShowDataToWindow(ShortcutKeyDim, CountOfShortcut);
}

DefineShortcutKeyDialog::~DefineShortcutKeyDialog()
{
    delete ui;
}

void DefineShortcutKeyDialog::on_tableWidget_doubleClicked(const QModelIndex &index)
{

}

void DefineShortcutKeyDialog::on_pushButtonOK_clicked()
{
	StoreDataFromWindow(ShortcutKeyDim , CountOfShortcut);
	close();
}

void DefineShortcutKeyDialog::StoreDataFromWindow(struct ShortcutKeyInfoList tShortcutKeyDim[] , int tCountOfShortcut)
{
	for(int i=0;i<tCountOfShortcut;i++){
		if(tShortcutKeyDim[i].DLLRoot!=NULL)
			delete	[]tShortcutKeyDim[i].DLLRoot;
		tShortcutKeyDim[i].DLLRoot=NULL;
		if(tShortcutKeyDim[i].DLLName!=NULL)
			delete	[]tShortcutKeyDim[i].DLLName;
		tShortcutKeyDim[i].DLLName=NULL;
		if(tShortcutKeyDim[i].DLLInst!=NULL)
			delete	[]tShortcutKeyDim[i].DLLInst;
		tShortcutKeyDim[i].DLLInst=NULL;
		if(tShortcutKeyDim[i].ObjPath!=NULL)
			delete	[]tShortcutKeyDim[i].ObjPath;
		tShortcutKeyDim[i].ObjPath=NULL;
		if(tShortcutKeyDim[i].ObjName!=NULL)
			delete	[]tShortcutKeyDim[i].ObjName;
		tShortcutKeyDim[i].ObjName=NULL;

		CellInComponentControl	*bb=dynamic_cast<CellInComponentControl *>(ui->tableWidget->cellWidget(i,1));
		if(bb!=NULL){
			int	Len=bb->DLLRoot.count()+1;
			tShortcutKeyDim[i].DLLRoot=new char[Len];
			::QString2Char(bb->DLLRoot,tShortcutKeyDim[i].DLLRoot ,Len);

			Len=bb->DLLName.count()+1;
			tShortcutKeyDim[i].DLLName=new char[Len];
			::QString2Char(bb->DLLName,tShortcutKeyDim[i].DLLName ,Len);

			Len=bb->DLLInst.count()+1;
			tShortcutKeyDim[i].DLLInst=new char[Len];
			::QString2Char(bb->DLLInst,tShortcutKeyDim[i].DLLInst ,Len);

			Len=bb->ObjPath.count()+1;
			tShortcutKeyDim[i].ObjPath=new char[Len];
			::QString2Char(bb->ObjPath,tShortcutKeyDim[i].ObjPath ,Len);

			Len=bb->ObjName.count()+1;
			tShortcutKeyDim[i].ObjName=new char[Len];
			::QString2Char(bb->ObjName,tShortcutKeyDim[i].ObjName ,Len);
		}

		QComboBox	*b1=dynamic_cast<QComboBox *>(ui->tableWidget->cellWidget(i,2));
		if(b1!=NULL){
			tShortcutKeyDim[i].Action=(ShortcutKeyInfoList::ActionType)b1->currentIndex();
		}

		QString	EStr;
		switch(tShortcutKeyDim[i].Action){
		case ShortcutKeyInfoList::_NoAction				:	break;
		case ShortcutKeyInfoList::_PushButton			:	break;
		case ShortcutKeyInfoList::_DoubleClickList		:
			EStr=GetDataToTable(ui->tableWidget ,3 ,i);
			tShortcutKeyDim[i].Numb=EStr.toInt();
			break;
		case ShortcutKeyInfoList::_SelectComboboxList	:
			EStr=GetDataToTable(ui->tableWidget ,3 ,i);
			tShortcutKeyDim[i].Numb=EStr.toInt();
			break;
		case ShortcutKeyInfoList::_Check				:	break;
		case ShortcutKeyInfoList::_Uncheck				:	break;
		case ShortcutKeyInfoList::_ToggleCheck			:	break;
		case ShortcutKeyInfoList::_InputInEdit			:
			EStr=GetDataToTable(ui->tableWidget ,3 ,i);
			::QString2Char(EStr ,tShortcutKeyDim[i].ExtraData ,sizeof(tShortcutKeyDim[0].ExtraData));
			break;
...
void DefineShortcutKeyDialog::ShowDataToWindow(struct ShortcutKeyInfoList tShortcutKeyDim[] , int tCountOfShortcut)
{
	...
	ui->tableWidget->setRowCount(tCountOfShortcut);
	for(int i=0;i<tCountOfShortcut;i++){
		QString	AStr;
		switch(tShortcutKeyDim[i].AddedKey){
		case ShortcutKeyInfoList::_None:			AStr=/**/"";			break;
		case ShortcutKeyInfoList::_Ctrl		:		AStr=/**/"Ctrl+";		break;
		case ShortcutKeyInfoList::_Shift	:		AStr=/**/"Shift+";		break;
		case ShortcutKeyInfoList::_Alt		:		AStr=/**/"Alt+";		break;
		case ShortcutKeyInfoList::_CtrlShift:		AStr=/**/"Ctrl+Shift+";	break;
		case ShortcutKeyInfoList::_AltShift	:		AStr=/**/"Alt+Shift+";	break;
		case ShortcutKeyInfoList::_AltCtrl	:		AStr=/**/"Alt+Ctrl+";	break;
		}
		QString		AStr2=ConvertKeyCodeToString(tShortcutKeyDim[i].KeyCode.QtKeyCode);
		QString		AStr3=AStr+AStr2;
		::SetDataToTable(ui->tableWidget ,0 ,i ,AStr3);

		QString	DLLObj;
		if(tShortcutKeyDim[i].DLLRoot!=NULL)
			DLLObj+=QString(tShortcutKeyDim[i].DLLRoot);
		DLLObj+=/**/"-";
		if(tShortcutKeyDim[i].DLLName!=NULL)
			DLLObj+=QString(tShortcutKeyDim[i].DLLName);
		DLLObj+=/**/"-";
		if(tShortcutKeyDim[i].DLLInst!=NULL)
			DLLObj+=QString(tShortcutKeyDim[i].DLLInst);
		DLLObj+=/**/"-";
		if(tShortcutKeyDim[i].ObjPath!=NULL)
			DLLObj+=QString(tShortcutKeyDim[i].ObjPath);
		DLLObj+=/**/"-";
		if(tShortcutKeyDim[i].ObjName!=NULL)
			DLLObj+=QString(tShortcutKeyDim[i].ObjName);

		QWidget *wn=ui->tableWidget->cellWidget(i,1);
		if(wn==NULL || dynamic_cast<QComboBox *>(wn)==NULL){
			CellInComponentControl	*b=new CellInComponentControl(this,i);
			ui->tableWidget->setCellWidget(i,1,b);
		}
		CellInComponentControl	*bn=dynamic_cast<CellInComponentControl *>(ui->tableWidget->cellWidget(i,1));
		if(bn!=NULL){
			bn->Set(tShortcutKeyDim[i].DLLRoot
					,tShortcutKeyDim[i].DLLName
					,tShortcutKeyDim[i].DLLInst
					,tShortcutKeyDim[i].ObjPath
					,tShortcutKeyDim[i].ObjName);
		}

		QWidget *w1=ui->tableWidget->cellWidget(i,2);
		if(w1==NULL || dynamic_cast<QComboBox *>(w1)==NULL){
			QComboBox	*b=new QComboBox();
			b->addItems(ActionList);
			ui->tableWidget->setCellWidget(i,2,b);
		}
		QComboBox	*b1=dynamic_cast<QComboBox *>(ui->tableWidget->cellWidget(i,2));
		if(b1!=NULL){
			b1->setCurrentIndex((int)tShortcutKeyDim[i].Action);
		}

		QString	EStr;
		switch(tShortcutKeyDim[i].Action){
		case ShortcutKeyInfoList::_NoAction				:	EStr=/**/"";				break;
		case ShortcutKeyInfoList::_PushButton			:	EStr=/**/"";			break;
		case ShortcutKeyInfoList::_DoubleClickList		:	EStr=QString::number(tShortcutKeyDim[i].Numb);	break;
		case ShortcutKeyInfoList::_SelectComboboxList	:	EStr=QString::number(tShortcutKeyDim[i].Numb);	break;
		case ShortcutKeyInfoList::_Check				:	EStr=/**/"";				break;
		case ShortcutKeyInfoList::_Uncheck				:	EStr=/**/"";				break;
		case ShortcutKeyInfoList::_ToggleCheck			:	EStr=/**/"";		break;
		case ShortcutKeyInfoList::_InputInEdit			:	EStr=tShortcutKeyDim[i].ExtraData;				break;
		case ShortcutKeyInfoList::_OutSequenceSystemWord:	EStr=QString::number(tShortcutKeyDim[i].Numb);	break;
		}
		//_heapchk();
		::SetDataToTable(ui->tableWidget ,3 ,i ,EStr);
		//_heapchk();
	}
}

void DefineShortcutKeyDialog::on_pushButtonSaveFile_clicked()
{
	...
	if(FileName.isEmpty()==false){
		...
		struct ShortcutKeyInfoList tShortcutKeyDim[1000];
		for(int i=0;i<CountOfShortcut;i++){
			tShortcutKeyDim[i].CopyFrom(ShortcutKeyDim[i]);
		}
		StoreDataFromWindow(tShortcutKeyDim , CountOfShortcut);

		QFile	File(FileName);
		if(File.open(QIODevice::WriteOnly)==true){
			SaveToFile(&File , tShortcutKeyDim, CountOfShortcut);
		}
	}
}

void DefineShortcutKeyDialog::on_pushButtonLoadFile_clicked()
{
	QString FileName=QFileDialog::getOpenFileName ( 0
							, "Load shortcut information"
							, QString()
							, /**/"dat (*.dat);;All files (*.*)");
	if(FileName.isEmpty()==false){
		...
		if(File.open(QIODevice::ReadOnly)==true){
			LoadFromFile(&File , tShortcutKeyDim, CountOfShortcut);
		}
		ShowDataToWindow(tShortcutKeyDim , CountOfShortcut);
	}
}

bool	DefineShortcutKeyDialog::SaveToFile(QIODevice *f , struct ShortcutKeyInfoList tShortcutKeyDim[] , int tCountOfShortcut)
{
	int32	Ver=1;
	...
	if(::Save(f,tCountOfShortcut)==false)
		return false;
	for(int i=0;i<tCountOfShortcut;i++){
		if(tShortcutKeyDim[i].Save(f)==false)
			return false;
	}
	return true;
}

bool	DefineShortcutKeyDialog::LoadFromFile(QIODevice *f , struct ShortcutKeyInfoList tShortcutKeyDim[] , int tCountOfShortcut)
{
	int32	Ver;
	...
	if(::Load(f,Count)==false)
		return false;
	for(int i=0;i<tCountOfShortcut && i<Count;i++){
		if(tShortcutKeyDim[i].Load(f)==false)
			return false;
	}
	return true;
}

struct	KeyCodeNameMap
{
	DWORD	KeyCode;
	const	char	*KeyName;
};

static	struct	KeyCodeNameMap	KeyMap[]={
