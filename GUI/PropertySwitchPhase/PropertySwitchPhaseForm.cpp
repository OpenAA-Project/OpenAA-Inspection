...
void	PropertySwitchPhaseForm::ShowPhaseList(void)
{
	...
	if(ABase!=NULL){
		...
		QStringList	Header;
		Header.append(LangSolver.GetString(PropertySwitchPhaseForm_LS,LID_0)/*"Use"*/);
		Header.append(LangSolver.GetString(PropertySwitchPhaseForm_LS,LID_1)/*"Phase Name"*/);
		for(int page=0;page<GetParamGlobal()->PageNumb;page++){
			Header.append(QString(LangSolver.GetString(PropertySwitchPhaseForm_LS,LID_2)/*"Page"*/)+QString::number(page));
		}
		Header.append(LangSolver.GetString(PropertySwitchPhaseForm_LS,LID_3)/*"Result"*/);
...
void	PropertySwitchPhaseForm::ShowMarkList(void)
{
	...
	for(ItemInfoList *a=MarkList.GetFirst();a!=NULL;a=a->GetNext(),Row++){
		...
		::SetDataToTable(ui->tableWidgetMarkList ,1 ,Row,QString::number(a->Cx) + QString(LangSolver.GetString(PropertySwitchPhaseForm_LS,LID_4)/*","*/)+QString::number(a->Cy) );
...
void	PropertySwitchPhaseForm::DeleteAllPhases(void)
{
	for(int page=0;page<GetParamGlobal()->PageNumb;page++){
		int	globalPage=GetParamComm()->GetGlobalPageFromLocal(*GetParamGlobal(),page);
		GUICmdDeleteAllPhases	RCmd(GetLayersBase(),sRoot,sName,globalPage);
		RCmd.Send(NULL,globalPage,0);
	}
}
void	PropertySwitchPhaseForm::DeleteAllMarks(void)
{
	on_pushButtonDeleteAll_clicked();
}

void	PropertySwitchPhaseForm::ReUseMark(void)
{
	for(int page=0;page<GetParamGlobal()->PageNumb;page++){
		int	globalPage=GetParamComm()->GetGlobalPageFromLocal(*GetParamGlobal(),page);
		GUICmdReUseMark	RCmd(GetLayersBase(),sRoot,sName,globalPage);
		RCmd.Send(NULL,globalPage,0);
	}
}
