...
static	bool	MacroSelectAreaByPriority(GUIFormBase *Instance ,QStringList &Args);
...
DEFFUNCEX	bool	DLL_Initial(LayersBase *Base)
{
	...
	(*Base)=new GUICmdReqAlignmentLargeAngleList	(Base, QString(sRoot), QString(sName));
	(*Base)=new GUICmdAckAlignmentLargeAngleList	(Base, QString(sRoot), QString(sName));
	return true;
}
...
DEFFUNCEX	int32	DLL_RegistMacroFunction(ExportFuncForMacro Functions[],int MaxBuffer)
{
	...
	if(MaxBuffer>ret){
		...
		Functions[ret].Explain.append(/**/"Select area by position");
		Functions[ret].ArgName.append(/**/"X");
		Functions[ret].ArgName.append(/**/"Y");
	...
	if(MaxBuffer>ret){
		Functions[ret].FuncName	=/**/"SelectArea";
		Functions[ret].Explain.append(/**/"Select area by priority");
		Functions[ret].ArgName.append(/**/"Priority");
		Functions[ret].DLL_ExcuteMacro	=MacroSelectAreaByPriority;
		ret++;
	}
...
static	bool	MacroAutoCreatePoint(GUIFormBase *Instance ,QStringList &Args)
{
	...
	int	SearchAreaDot=Args[1].toInt(&ok);
	if(ok==false){
		return false;
	}
	int	SearchAreaDot2=Args[2].toInt(&ok);
	if(ok==false){
		return false;
	}
	...
	V->AutoCreatePoint(Layer,PointSize,SearchAreaDot,SearchAreaDot2,AreaPriority
...
static	bool	MacroAutoCreatePointWithLib	(GUIFormBase *Instance ,QStringList &Args)
{
	...
	V->AutoCreatePoint(Layer,PointSize,SearchAreaDot,SearchAreaDot2
						,AreaPriority
						,GeneratePartialAlignment
...
static	bool	MacroSelectAreaByPriority(GUIFormBase *Instance ,QStringList &Args)
{
	...
	V->SelectAreaByPriority(Priority);
