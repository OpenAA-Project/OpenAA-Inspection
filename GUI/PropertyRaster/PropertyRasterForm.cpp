...
void	PropertyRasterForm::LoadPDF(const QString &FileName)
{
	QFile	File(FileName);
...
void PropertyRasterForm::LoadRaster(int FileNo,QByteArray &Data ,const QString &FileName)
{
...
void PropertyRasterForm::on_pushButtonStartMoveToMatch_clicked()
{
	RasterImagePanel	*DProp	=(RasterImagePanel	*)GetImagePanel();
	if(DProp!=NULL){
		GetLayersBase()->ShowProcessingForm(LangSolver.GetString(PropertyRasterForm_LS,LID_11)/*"Processing..."*/);
...
void	PropertyRasterForm::CommandMakeImageToMaster(void)
{
	MakeImageToMaster();
}
...
{
	GetLayersBase()->ShowProcessingForm(LangSolver.GetString(PropertyRasterForm_LS,LID_12)/*"Processing..."*/);
	for(int page=0;page<GetPageNumb();page++){
		GUICmdConvertColorProfiles	SCmd(GetLayersBase(),sRoot,sName,0);
		SCmd.Send(NULL,page,0);
	}
...
void PropertyRasterForm::on_listWidgetRasterMask_doubleClicked(const QModelIndex &index)
{
	...
	if(QMessageBox::question(NULL,LangSolver.GetString(PropertyRasterForm_LS,LID_13)/*"Delete"*/
							,LangSolver.GetString(PropertyRasterForm_LS,LID_14)/*"Delete mask?"*/)==QMessageBox::Yes){
		int	FileLayerID=GetCurrentFileLayerID();
...
{
	QString	FileName=QFileDialog::getSaveFileName(NULL,LangSolver.GetString(PropertyRasterForm_LS,LID_15)/*"Save transform parameter"*/
												,QString()
...
{
	QString	FileName=QFileDialog::getOpenFileName(NULL,LangSolver.GetString(PropertyRasterForm_LS,LID_16)/*"Load transform parameter"*/
												,QString()
												,/**/"TPR file(*.tpr);;All files(*.*)");
	if(FileName.isEmpty()==false){
...
void PropertyRasterForm::on_pushButtonLoadColorProfile_clicked()
{
	QString	FileName=QFileDialog::getOpenFileName(NULL
												,LangSolver.GetString(PropertyRasterForm_LS,LID_17)/*"Load color profile"*/
												,QString()
												,/**/"dat(*.dat);;All files(*.*)");
	if(FileName.isEmpty()==false){
		QFile	File(FileName);
