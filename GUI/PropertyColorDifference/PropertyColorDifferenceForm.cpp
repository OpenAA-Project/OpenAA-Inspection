...
{
	QString	FileName=QFileDialog::getSaveFileName(NULL,LangSolver.GetString(PropertyColorDifferenceForm_LS,LID_8)/*"Save Flow data"*/
												,QString()
...
{
	QString	FileName=QFileDialog::getOpenFileName(NULL,LangSolver.GetString(PropertyColorDifferenceForm_LS,LID_9)/*"Load Flow data"*/
												,QString()
												,/**/"Flow data(*.flw);;All data(*.*)");
	if(FileName.isEmpty()==false){
		QFile	File(FileName);
