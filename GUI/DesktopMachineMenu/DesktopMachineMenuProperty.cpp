...
**
** This file is C:\Regulus64v5\GUI\DesktopMachineMenu\DesktopMachineMenuProperty.cpp
** Author : YYYYYYYYYY
****************************************************************************-**/
...
void	DesktopMachineMenuProperty::SlotBlickTimer()
{
	...
	if(ExecuteType==_None){
		...
		if(Param!=NULL && Param->StartScanOnly==false){
			...
			NextMasterForm	DUp(GetLayersBase(),this);
			if(((bool)DUp.exec())==true){
				...
				GetLayersBase()->ShowProcessingForm (LangSolver.GetString(DesktopMachineMenuProperty_LS,LID_0)/*"Initializing after edit"*/);
...
void DesktopMachineMenuProperty::on_pushButtonDetail_clicked()
{
	...
	CurrentPath+=QDir::separator()+QString(/**/"/InspectionPlayerDesktopDetail.gui");
