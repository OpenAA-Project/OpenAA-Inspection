    ...
    ui(new Ui::SelectAlignmentLargeDialog)
{
	...
	if(GetParamGlobal()->AllocBitBuffer==true && ((AlignmentLargeBase *)Ab)->UseBitBuff==true){
		ui->labelCautionForOrigin->setText(LangSolver.GetString(SelectAlignmentLargeDialog_LS,LID_1)/*"Pick up/Generate from BitBuffer"*/);
	}
	else{
		ui->labelCautionForOrigin->setText(LangSolver.GetString(SelectAlignmentLargeDialog_LS,LID_2)/*"Pick up/Generate from Master image"*/);
	}
...
void SelectAlignmentLargeDialog::on_tableWidgetLibList_clicked(const QModelIndex &Index)
{
	...
	if(Ab!=NULL){
		...
		if(Packet.Success==true){
			...
			ui->checkBoxJudgeWithBrDif->setChecked(ALib->JudgeWithBrDif);
			...
			QMessageBox::critical(NULL	,LangSolver.GetString(SelectAlignmentLargeDialog_LS,LID_3)/*"Library error"*/
										,LangSolver.GetString(SelectAlignmentLargeDialog_LS,LID_4)/*"Not loaded this library record"*/);
		}
	}
}
void	SelectAlignmentLargeDialog::GetDataFromWindow(XAlignmentLarge &data)
{
	...
	data.GetThresholdW()->JudgeWithBrDif=ui->checkBoxJudgeWithBrDif	->isChecked();
}
