...
**
** This file is C:\Regulus64v5\GUI\PropertyHoughInspection\PropertyHoughInspection.cpp
** Author : YYYYYYYYYY
****************************************************************************-**/
#include "PropertyHoughInspectionFormResource.h"
#include "XGUIDLL.h"
#include "PropertyHoughInspection.h"
#include "PropertyHoughInspectionForm.h"
#include "XHoughInspectionPacket.h"
#include "XMacroFunction.h"

const	char	*sRoot=/**/"Button";
const	char	*sName=/**/"PropertyHoughInspection";
...
{
	return(/**/"Show Property for HoughInspection");
}
...
DEFFUNCEX	bool	DLL_Initial(LayersBase *Base)
{
	...
	(*Base)=new GUIReqHoughListForPacketPack	(Base,QString(sRoot),QString(sName));
	(*Base)=new GUIAckHoughListForPacketPack	(Base,QString(sRoot),QString(sName));
	(*Base)=new GUIReqGenerateHoughItems		(Base,QString(sRoot),QString(sName));
	return true;
}
...
{
	return(new PropertyHoughInspectionForm(Base,parent));
}
...
{
	return(new QIcon(QPixmap(/**/":Resources/PropertyHoughInspection.png")));
}
...
static	bool	MacroGenerateBlocks(GUIFormBase *Instance ,QStringList &Args)
{
	PropertyHoughInspectionForm	*V=dynamic_cast<PropertyHoughInspectionForm *>(Instance);
	if(V==NULL){
		return false;
	}
	if(Args.count()<5){
