#include "ManualPixelInspectionNGListResource.h"
/*******************************************************************************
...
**
** This file is C:\Regulus64v5\GUI\ManualPixelInspectionNGList\ManualPixelInspectionNGList.cpp
** Author : YYYYYYYYYY
****************************************************************************-**/

#include "PixelInspectionNGList.h"
#include "XGUIDLL.h"
#include "XDLLOnly.h"
...
static	const	char	*sName=/**/"ManualPixelInsNGList";
...
{
	return(/**/"NG List for Pixel Inspection");
}

DEFFUNCEX	GUIFormBase	*DLL_CreateInstance(LayersBase *Base,QWidget *parent)
{
	return(new PixelInspectionNGList(Base,parent));
}
...
DEFFUNCEX	int32	DLL_GetPropertyString(void	*Instance ,struct	PropertyClass Data[] ,WORD	maxDataDim)
{
	...
	Data[0].Pointer				 =&((PixelInspectionNGList *)Instance)->WidthLineNumber;
	Data[1].Type				 =/**/"int32";
	Data[1].VariableNameWithRoute=/**/"WidthPageNumber";
	Data[1].Pointer				 =&((PixelInspectionNGList *)Instance)->WidthPageNumber;
	Data[2].Type				 =/**/"int32";
	Data[2].VariableNameWithRoute=/**/"WidthPosition";
	Data[2].Pointer				 =&((PixelInspectionNGList *)Instance)->WidthPosition;
	Data[3].Type				 =/**/"int32";
	Data[3].VariableNameWithRoute=/**/"Attribute";
	Data[3].Pointer				 =&((PixelInspectionNGList *)Instance)->WidthAttribute;
	Data[4].Type				 =/**/"QString";
	Data[4].VariableNameWithRoute=/**/"TargetImagePanelName";
	Data[4].Pointer				 =&((PixelInspectionNGList *)Instance)->TargetImagePanelName;
...
{
	return(new QIcon(QPixmap(/**/":Resources/ManualPixelInspectionNGList.png")));
}
...
//==================================================================================================
PixelInspectionNGList::PixelInspectionNGList(LayersBase *Base ,QWidget *parent)
...
void	PixelInspectionNGList::Prepare(void)
{
	...
	NGList.setHorizontalHeaderItem ( 0, new QTableWidgetItem(LangSolver.GetString(ManualPixelInspectionNGList_LS,LID_0)/*"Pg"*/));
	NGList.setHorizontalHeaderItem ( 1, new QTableWidgetItem(LangSolver.GetString(ManualPixelInspectionNGList_LS,LID_1)/*"Position"*/));
	ShowInPlayer(GetLayersBase()->GetCurrentInspectIDForDisplay());
}
void	PixelInspectionNGList::ResizeAction()
{
	NGList.resize(width(),height());
}
void	PixelInspectionNGList::ShowInPlayer(int64 shownInspectionID)
{
	NGData.RemoveAll();
	AlgorithmBase	*ABase=GetLayersBase()->GetAlgorithmBase(/**/"Basic",/**/"PixelInspection");
	if(ABase!=NULL){
		AlgorithmLibraryContainer	*LContainer=ABase->GetLibraryContainer();
		if(LContainer!=NULL){
			LContainer->EnumLibrary(PixelInspectionLibType,LibList);
		}
	}
	...
	for(int page=0;page<GetPageNumb();page++){
		...
		if(CmdReq.Send(page ,0,CmdAck)==true){
			for(PixelInspectionNGDataList *e=CmdAck.NGData.GetFirst();e!=NULL;e=e->GetNext()){
				e->Page=page;
			}
	...
	int	row=0;
	for(PixelInspectionNGDataList *e=NGData.GetFirst();e!=NULL;e=e->GetNext(),row++){
		QTableWidgetItem *W;
		int	N=0;
		if(WidthPageNumber>0){
			W=NGList.item ( row, N);
		...
		if(WidthPosition>0){
			...
			W->setText(QString::number(e->CenterX) + LangSolver.GetString(ManualPixelInspectionNGList_LS,LID_2)/*","*/ +QString::number(e->CenterY));
			N++;
		}
		if(WidthAttribute>0){
			W=NGList.item ( row, N);
			...
			AlgorithmLibraryList	*L=LibList.SearchLibrary(PixelInspectionLibType, e->LibID);
			if(L!=NULL)
...
void	PixelInspectionNGList::ListItemSelectionChanged ()
{
	int row	=NGList.currentRow();
	PixelInspectionNGDataList *e=NGData.GetItem(row);
	if(e!=NULL){
...
void	PixelInspectionNGList::ListItemClicked ( QTableWidgetItem *)
{
	int row	=NGList.currentRow();
	PixelInspectionNGDataList *e=NGData.GetItem(row);
	if(e!=NULL){
...
//=======================================================================
bool	PixelInspectionNGDataList::Save(QIODevice *f)
{
...
bool	PixelInspectionNGDataList::Load(QIODevice *f)
{
...
bool	PixelInspectionNGDataListContainer::Save(QIODevice *f)
{
	int32	N=GetNumber();
	if(::Save(f,N)==false)
		return false;
	for(PixelInspectionNGDataList *a=GetFirst();a!=NULL;a=a->GetNext()){
		if(a->Save(f)==false)
			return false;
	}
	return true;
}
bool	PixelInspectionNGDataListContainer::Load(QIODevice *f)
{
	int32	N;
	if(::Load(f,N)==false)
		return false;
...
void	GUICmdReqNGList::Receive(int32 localPage, int32 cmd ,QString &EmitterRoot,QString &EmitterName)
{
	...
	AlgorithmBase	*ABase=GetLayersBase()->GetAlgorithmBase(/**/"Basic",/**/"PixelInspection");
	if(ABase!=NULL){
		CmdCreateNGListPacket	Cmd(GetLayersBase());
