...
void PropertyDentForm::on_ButtonPickupTest_clicked()
{
	...
	GetLayersBase()->ShowProcessingForm(LangSolver.GetString(PropertyDentForm_LS,LID_0)/*"Pickup Test"*/);
...
void PropertyDentForm::on_ButtonGenerateLibs_clicked()
{
	...
	GetLayersBase()->ShowProcessingForm(LangSolver.GetString(PropertyDentForm_LS,LID_1)/*"Generating items"*/,GetLayersBase()->GetPageNumb()*10);
	for(int page=0;page<GetLayersBase()->GetPageNumb();page++){
		int	GlobalPage=GetLayersBase()->GetGlobalPageFromLocal(page);
