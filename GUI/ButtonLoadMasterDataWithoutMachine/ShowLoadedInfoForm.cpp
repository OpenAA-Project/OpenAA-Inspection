...
void	ShowLoadedInfoForm::ShowList(void)
{
	...
	for(int Row=0;Row<RowCount;Row++){
		...
		if(IndexPointer[Row].Pointer->Error==LoadedInfoList::_NoFile)
			SetDataToTable(ui.tableWidget,4,Row,LangSolver.GetString(ShowLoadedInfoForm_LS,LID_37)/*"No file"*/);
		else if(IndexPointer[Row].Pointer->Error==LoadedInfoList::_Error)
			SetDataToTable(ui.tableWidget,4,Row,LangSolver.GetString(ShowLoadedInfoForm_LS,LID_38)/*"Error"*/);
		else if(IndexPointer[Row].Pointer->ItemCount==0)
			SetDataToTable(ui.tableWidget,4,Row,LangSolver.GetString(ShowLoadedInfoForm_LS,LID_39)/*"No Items"*/);
		else if(IndexPointer[Row].Pointer->Error==LoadedInfoList::_Success)
			SetDataToTable(ui.tableWidget,4,Row,LangSolver.GetString(ShowLoadedInfoForm_LS,LID_40)/*"OK"*/);
	}
	delete	[]IndexPointer;
}
