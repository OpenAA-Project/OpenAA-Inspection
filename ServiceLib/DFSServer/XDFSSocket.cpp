...
void	DFSSocket::ReadyRead ()
{
	...
	do{
		if(RxState==_NowOnWaiting || (RxState==_NowOnReceiving && (time(NULL)-LastReceivingTime>5))){
			...
			if(N!=0){
				if(NowOnXByte>=N){
					Len=Sock->read((char *)(OnRBuff->GetPointerPointer()+NowOnPoint), N);
					if(Len<0){
						FastCommLeafRxReEntrant--;
						return;
					}
					...
					if(NowOnXByte!=0){
						Len=Sock->read((char *)(OnRBuff->GetPointerPointer()+NowOnPoint), NowOnXByte);
						if(Len<0){
							FastCommLeafRxReEntrant--;
							return;
						}
			...
			if(RByte!=0){
				int	Len=Sock->read((char *)(OnRBuff->GetPointerPointer()+NowOnPoint),RByte);
