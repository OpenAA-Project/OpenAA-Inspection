...
//	int OffsetY2=IDataUnit->MImageClassList[SType]->ImageList[GetLastPiecePage()]	->OrgOffsetY;
	//PieceCoordinates[0]��(X,Y)���W��������PieceCoordinates[1]��(X,Y)���W������
	int dX,dY;
...
{
	//XML�f�[�^�������ւ����O�ɁA���߂̃f�[�^�iIST�j�Q�����`�F�b�N����
	int Ret=0;
	if(Type==Front){
...
void RepairBase::ShowHistoryList(QTableWidget *TableWidget,int MasterCount,QStringList &HeaderLabels,int InsDataDialogFlag)
{
	...
	if(GetShowInsDataMsg()==true){
		InsDataDialog.setWindowFlags(Qt::WindowStaysOnTopHint);	//���ɑO�ʂɕ\��
...
///						}
	...
	//�Џ����f�[�^�̃Z�b�g�̑O��cbPieceListEditor��delete
	...
	if(GetRetPCEDataList(SType)->count()>0){
		PieceNameList.append(LangSolver.GetString(RepairBase_LS,LID_656)/*""*/);	//�Ж��̂Ȃ��̂��̂��\�ߊ܂߂Ă���
	}
...
void RepairBase::xmlMacGet(QByteArray Info, QByteArray &MAC)
{
	...
	while (!n.isNull()) {	
		if (n.tagName()==LangSolver.GetString(RepairBase_LS,LID_666)/*"IST"*/){
			MAC=n.attribute(LangSolver.GetString(RepairBase_LS,LID_667)/*"MAC"*/).toLocal8Bit().data();//2:�������u�ԍ�
		}
		return ;
	}
}
...
//	NGPointList[1].clear();
	...
	//NG���X�g�̃N���A�̑O��cbPieceListEditor��delete
	if(cbPieceListEditor!=NULL){
		delete cbPieceListEditor;
		cbPieceListEditor=NULL;
	}
...
///		int RowCnt	=0;
		RowCnt	=0;
		QString TagNGI=LangSolver.GetString(RepairBase_LS,LID_671)/*"Init"*/;		//Init:�����l
		if(BothCnt==0){
			if(XMLIndex1==-1){
				SetListIndex(BothCnt,XMLIndex1);
				continue;
		...
		if(MachineID[BothCnt]!=MAC.toInt()){
			...
			if(GetOutlineOffset(MachineID[BothCnt],&OffsetXList,&OffsetYList)==true){
				for(int i=0; i<Pagenumb && i<OffsetXList.count(); i++){
					IDataUnit->MImageClassList[SType]->ImageList[i]->OrgOffsetX	=OffsetXList.at(i);	//OutlineOffset�̎擾
					IDataUnit->MImageClassList[SType]->ImageList[i]->OrgOffsetY	=OffsetYList.at(i);
					IDataUnit->MImageClassList[SType]->ImageList[i]->OffsetX	=(IDataUnit->MImageClassList[SType]->ImageList[i]->OrgOffsetX)>>IDataUnit->MImageClassList[SType]->ImgRate;	//OutlineOffset�̎擾
...
//			int SumImgCnt=0;
			...
			//NGP�f�[�^�̃\�[�g�O�����i�\�[�g��NG��X���W��Y���W���؂��Ԃ����悤�Ɂj
...
///				for(Cnt=0;Cnt<NGPCount;Cnt++){
					FindNGCntList[BothCnt][Cnt]=false;	//false�ɏ�����
...
///				//NG�`�F�b�N�ς݂��ǂ����̔��f<RCH>�^�O��SELECT
...
///			}
			...
			for(int CompCnt=BaseCnt+1;CompCnt<NGPCount;CompCnt++){
				...
				//NG�|�C���g���ŏ��̓_�������͗אڂ����_���܂߂Ă����ɕ`���G���A���Ɋ܂܂��邩�ǂ����̔��f
...
void RepairBase::SetSigmoid(double Volume,int Bright,int BrightArray[])
{
	...
	if(Volume>1){
		//Volume���P�����傫���l�̎��́A�V�O���C�h�֐��ŃO���t���\��
...
//���l���X�g�̒��̐��l�����ׂăr�b�g�ɂ��ĕԂ�
//���F���l���X�g[2,3,5,6]��54�i=32+16+4+2�i110110))
...
QString RepairBase::GetEmptyARA(int X,int Y)
{
	...
	return PieceInfoDataList[SType][MinimumDistanceInt].GetPieceName() + LangSolver.GetString(RepairBase_LS,LID_804)/*"�O"*/;
}

bool RepairBase::OpenDebugLogFile(QString OutputDir,QString OutputFileName)
{
	//�f�o�b�O���O�t�@�C����Open���Ă����ꍇ�Aclose����
