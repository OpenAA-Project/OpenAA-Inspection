...
int64	GetComputerMicrosec(void)
{
	...
	if(QueryPerformanceCounter(&PerformanceCount)==TRUE){   // �J�E���^�̒l
...
void	InitialMicrosecTimer(struct	InitializedMicrosecTimerStruct &IData)
{
	...
	*Dif1Milisec=0;
	...
	if(QueryPerformanceCounter(&LastPerformanceCount)==TRUE){   // �J�E���^�̒l
		::Sleep(100);
		LARGE_INTEGER CurrentPerformanceCount;
		QueryPerformanceCounter(&CurrentPerformanceCount);   // �J�E���^�̒l
