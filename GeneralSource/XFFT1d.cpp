...
void FFTClass::factorize(int n, int *nFact, int fact[])
{
    int i,j,k;
    int nRadix;
    int radices[7];
...
bool FFTClass::transTableSetup(int sofar[], int actual[], int remain[],
                     int *nFact,
                     int *nPoints)
{
    int i;
    ...
    if (actual[1] > maxPrimeFactor){
        //AnsiString  A=AnsiString("Prime factor of FFT length too large : ")
        //              +AnsiString(actual[1]);
        //Application->MessageBox(A.c_str(),"Error",MB_OK);
        return(false);
        }
    remain[0]=*nPoints;
    sofar[1]=1;
...
void FFTClass::permute(int nPoint, int nFact,
             int fact[], int remain[],
...
void FFTClass::initTrig(int radix)
{
    int i;
    double w,xre,xim;
...
void FFTClass::fft_4(double aRe[], double aIm[])
{
...
void FFTClass::fft_5(double aRe[], double aIm[])
{    
...
void FFTClass::fft_odd(int radix)
{
...
void FFTClass::twiddleTransf(int sofarRadix, int radix, int remainRadix,
                    DoubleComplex y[])
    ...
    for (dataNo=0; dataNo<sofarRadix; dataNo++){
        ...
        for (groupNo=0; groupNo<remainRadix; groupNo++){
            if ((sofarRadix>1) && (dataNo > 0)){
                ...
                do {
                    adr = adr + sofarRadix;
                    zRe[blockNo]=  twiddleRe[blockNo] * y[adr].real()
                                 - twiddleIm[blockNo] * y[adr].imag();
                    zIm[blockNo]=  twiddleRe[blockNo] * y[adr].imag()
                                 + twiddleIm[blockNo] * y[adr].real();
            ...
            adr=groupOffset;
            for (blockNo=0; blockNo<radix; blockNo++){
                y[adr]=DoubleComplex(zRe[blockNo],zIm[blockNo]);
                adr=adr+sofarRadix;
                }
...
bool FFTClass::fft(int n, DoubleComplex x[] ,DoubleComplex y[])
{
    ...
    for(;;){
        ...
        if(transTableSetup(sofarRadix, actualRadix, remainRadix, &nFactor, &n)==true)
            break;
        N--;
        }
    permute(n, nFactor, actualRadix, remainRadix, x, y);
...
bool FFTClass::fftPower(int n, BYTE x[] ,double y[])
{
    DoubleComplex *Xi=new DoubleComplex[n];
    DoubleComplex *Yo=new DoubleComplex[n];

    for(int i=0;i<n;i++){
        Xi[i]=DoubleComplex(x[i],0);
        }
    if(fft(n, Xi ,Yo)==false){
        delete  []Xi;
        delete  []Yo;
        return(false);
        }
...
bool FFTClass::fftPower(int n, double x[] ,double y[])
{
    DoubleComplex *Xi=new DoubleComplex[n];
    DoubleComplex *Yo=new DoubleComplex[n];
...
bool FFTClass::fftPower(int n, int x[] ,double y[])
{
    DoubleComplex *Xi=new DoubleComplex[n];
    DoubleComplex *Yo=new DoubleComplex[n];
...
DoubleComplex   Expi(double s)
{
    return DoubleComplex(cos(s),sin(s));
}

bool FFTClass::reversefft(int n, DoubleComplex InpData[] ,DoubleComplex ResultOut[])
{
    for(int i=0;i<n;i++){
        ResultOut[i].set(0,0);
    }
    for(int i=0;i<n;i++){
        for(int j=0;j<n;j++){
            ResultOut[i]+=InpData[j]*Expi(i*2*M_PI*j/n);
        }
        ResultOut[i]/=n;
    }
    return true;
