...
bool	XDateTime::IsAvailable(void)	const
{
	if(	Year	==0
	&& Month	==0
	&& Day		==0
...
XDateTime	&XDateTime::operator=(const QDateTime &d)
{
	lock();
	Year	=d.date().year();
	Month	=d.date().month();
...
XDateTime	XDateTime::fromString(const QString &str,const QString &format)
{
...
qint64		XDateTime::to64(void) const
{
	QDateTime	K=GetQDateTime();
	return K.toSecsSinceEpoch();
}
void		XDateTime::set64(qint64 t)
{
	QDateTime	K;
	K.setSecsSinceEpoch(t);
	operator=(K);
}

qint64 XDateTime::secsTo ( const XDateTime & other ) const
{
	QDateTime	d=GetQDateTime();
...
QDate	XDateTime::date(void)const
{
	QDate	d(year(),month(),day());
	return d;
}

QTime	XDateTime::time(void)const
{
	QTime	d(hour(),minute(),second());
	return d;
}
