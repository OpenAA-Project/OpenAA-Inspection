...
bool clsDb::isConnected() {
	if ( db.isOpen() == false ) {
		if ( log != NULL ) {
			log->log("not connected",clsLog::CLSLOG_DEBUG);
		}
		return false;
	} else {
		if ( log != NULL ) {
			log->log("connected",clsLog::CLSLOG_DEBUG);
		}
	}
	return true;
}
...
bool clsDb::makePrepare() {
	...
	/*
	if ( queryReadLidEid->prepare("SELECT * FROM V_"+tblHead+"_TOPLIDEID WHERE lid=:lid AND eid=:eid" ) == false ) {
		QSqlError err = queryReadLidEid->lastError();
		mess = "Read Lid Eid:" + err.text();
		queryReadLidEid->clear();
		log->log(mess, clsLog::CLSLOG_DEBUG);
		return false;
	}
	*/
