...
bool clsDbCtrl::newDbDef(clsDbDef *dbDef) {
	...
	db->log->log("mstid=["+dbDef->MstId+"] mac=["+dbDef->Mac+"] day=["+dbDef->dDay+"]",clsLog::CLSLOG_DEBUG);
	if (db->openQuery() == false) {
		db->closeQuery(query);
	...
	if (db->next() == 0) {
		db->log->log("hitt",clsLog::CLSLOG_DEBUG);
	...
	db->log->log("Not Found",clsLog::CLSLOG_DEBUG);
...
bool clsDbCtrl::chkDbFileName(const clsDbDef *dbDef, clsDb *db) {
	if ( db->dbnam != prm->dbFolder + "/" + dbDef->dbFile ) {
		return false;
	}
	return true;
}
