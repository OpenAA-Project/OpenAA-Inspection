...
clsXmlOutLex::clsXmlOutLex(const QByteArray &_buf) :clsXmlLexBase(_buf) {
	bFullFlg = false;
...
QByteArray clsXmlOutLex::whrSecOutTag() {
	if ( bFullFlg == true ) {
		return "";
	}
	QByteArray whr = "";
...
bool clsXmlOutLex::isOutTag(const QByteArray &tag) {
	if ( bFullFlg == true ) {
		return true;
	}
	bool flg = false;
...
bool clsXmlOutLex::isOutTagAttr(const QByteArray &tag, const QByteArray &attr) {
	if ( bFullFlg == true ) {
		return true;
	}
	bool flg = false;
...
bool clsXmlOutLex::isTagInAttr(clsXMLRec *rec, const QByteArray &attr) {
	bool flg = false;
	if ( rec->attrs == NULL ) {
		return false;
	}
