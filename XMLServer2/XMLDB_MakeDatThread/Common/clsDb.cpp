...
void clsDb::setPrm() {
	...
	setPrm("QIBASE", "localhost", "/home/firebird/xmldb2.fdb", "sysdba",
			"masterkey");
}
...
bool clsDb::Connect(QString nam) {
	db = QSqlDatabase::addDatabase(dbtype,nam);
...
bool clsDb::DisConnect() {
	if ( queryTop != NULL ) queryTop->clear();
	if ( queryTopAttr != NULL ) queryTopAttr->clear();
	if ( querySec != NULL ) querySec->clear();
	if ( querySecAttr != NULL ) querySecAttr->clear();
	db.close();
	return true;
}
...
bool clsDb::setTable(QString tbl) {
	...
	if (createIndex(tblHead + "_d_sec_ngp", "topid", tblHead
			+ "_sec_ngp_idx2") == false) {
		return false;
	}
	if (createIndex(tblHead + "_d_sec_ngp", "nid", tblHead
			+ "_sec_ngp_idx3") == false) {
		return false;
	}
	...
	if (createIndex(tblHead + "_d_sec_ngi", "topid", tblHead
			+ "_sec_ngi_idx2") == false) {
		return false;
	}
	if (createIndex(tblHead + "_d_sec_ngi", "nid", tblHead
			+ "_sec_ngi_idx3") == false) {
		return false;
	}
