...
void clsDb::setPrm() {
	...
	setPrm("QIBASE","localhost","C:\\Data\\xmldb.fdb","sysdba","masterkey");
}

void clsDb::setPrm(QString _dbtyp, QString _dbhost, QString _dbnam, QString _dbusr, QString _dbpwd) {
	dbtype = _dbtyp;
	dbhost = _dbhost;
	dbnam  = _dbnam;
	dbusr  = _dbusr;
...
bool clsDb::Connect() {
	...
	if ( !db.open() ) {
		...
		mess = err.text()+"["+dbtype+":"+dbnam+":"+dbhost+":"+dbusr+":"+dbpwd+"]";
		return false;
	}
	return true;
}
...
QString clsDb::maxId(QString tbl) {
	...
	*/
	if ( query.exec("SELECT NEXT VALUE FOR "+tbl+"_id_seq FROM RDB$DATABASE") == false ) {
		QSqlError err = query.lastError();
		mess =err.text();
		return "";
	}
...
bool clsDb::addSecNode(int no, QString topid, QString tag, QString val, QString &parid) {
...
bool clsDb::addSecAttr(int no, QString topid, QString nid, QString tag, QString val) {
	if ( no == -1 ) {
...
bool clsDb::delTop(QString whr) {
	...
	QString sql = "DELETE FROM "+tblHead+"_d_topnode_attr WHERE topid IN ("+whr+")";
	if ( updExecSql(sql) == false ) {
		return false;
	}
...
bool clsDb::delSec(QString whr) {
	...
	QString sql = "DELETE FROM "+tblHead+"_d_secnode_attr WHERE topid IN ("+whr+")";
	if ( updExecSql(sql) == false ) {
		return false;
	}
...
bool clsDb::setTable(QString tbl) {
	...
	if ( createIndex(tblHead+"_d_topnode","tag,val",tblHead+"_topnode_idx1") == false ) {
		return false;
	}
	...
	if ( createIndex(tblHead+"_d_topnode_attr","itm,val",tblHead+"_topnode_attr_idx1") == false ) {
		return false;
	}
	if ( createIndex(tblHead+"_d_topnode_attr","topid",tblHead+"_topnode_attr_idx2") == false ) {
		return false;
	}
	if ( createIndex(tblHead+"_d_topnode_attr","srt",tblHead+"_topnode_attr_idx3") == false ) {
		return false;
	}
	...
	if ( createIndex(tblHead+"_d_secnode","tag,val",tblHead+"_secnode_idx1") == false ) {
		return false;
	}
	if ( createIndex(tblHead+"_d_secnode","topid",tblHead+"_secnode_idx2") == false ) {
		return false;
	}
	...
	if ( createIndex(tblHead+"_d_secnode_attr","itm,val",tblHead+"_secnode_attr_idx1") == false ) {
		return false;
	}
	if ( createIndex(tblHead+"_d_secnode_attr","topid",tblHead+"_secnode_attr_idx2") == false ) {
		return false;
	}
	if ( createIndex(tblHead+"_d_secnode_attr","nid",tblHead+"_secnode_attr_idx3") == false ) {
		return false;
	}
	if ( createIndex(tblHead+"_d_secnode_attr","srt",tblHead+"_secnode_attr_idx4") == false ) {
		return false;
	}
...
bool clsDb::createSequence(QString name) {
	...
	sql += "begin ";
	sql += "   if (NEW.ID IS null) then NEW.ID = next value for "+name+"_id_seq;";
	sql += "END; ";
	//sql += "SET TERM ; ^";
	if ( updExecSql(sql) == false ) {
		return false;
...
bool clsDb::setTag(QString tag, int lvl) {
	...
	lst->append(tag);
	QString sql = "INSERT INTO d_taglvl(tag,lvl) VALUES ('"+tag+"',"+QString::number(lvl)+")";
	if ( updExecSql(sql) == false ) {
		return false;
	}
	return true;
